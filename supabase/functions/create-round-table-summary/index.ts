...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!record || !record.module_id || !record.user_id) {
      console.log(
        "[create-round-table-summary] Invalid payload, skipping architect ingestion.",
      );
      return new Response(
    ...
    const result = await ingestArchitectMemorySource({
      ...
      tableName: "user_round_table_entries",
      record,
      oldRecord,
      requestId: ctx.requestId,
      triggerCoreIdentity: false,
    });
    ...
    console.error(
      `[create-round-table-summary] request_id=${ctx.requestId} user_id=${ctx.userId ?? "null"}`,
      error,
    );
