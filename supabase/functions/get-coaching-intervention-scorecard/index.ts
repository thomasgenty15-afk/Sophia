...
import { loadCoachingInterventionTraceWindow } from "../sophia-brain/lib/coaching_intervention_trace.ts";
import { buildCoachingInterventionTraceScorecard } from "../sophia-brain/lib/coaching_intervention_scorecard.ts";
...
function deriveWindow(body: z.infer<typeof BodySchema>): {
  ...
  return {
    to,
    from: new Date(new Date(to).getTime() - hours * 60 * 60 * 1000).toISOString(),
  };
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!window) return badRequest(req, requestId, "Invalid or missing time window");
    ...
    if (!isInternal) {
      ...
      const { data: adminRow } = await userClient
        ...
        .maybeSingle();
      if (!adminRow) {
        return jsonResponse(req, { error: "Forbidden", request_id: requestId }, {
          status: 403,
        });
      }
    }
    ...
    const trace = await loadCoachingInterventionTraceWindow({
    ...
    const scorecard = buildCoachingInterventionTraceScorecard({ trace, annotations: [] });
    ...
    console.error(`[get-coaching-intervention-scorecard] request_id=${requestId}`, error);
