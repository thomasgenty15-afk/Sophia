...
const MESSAGE_SCAN_LIMIT =
  Number((Deno.env.get("SOPHIA_MEMORIZER_SCAN_LIMIT") ?? "8000").trim()) ||
  8000;
const USERS_LIMIT =
  Number((Deno.env.get("SOPHIA_MEMORIZER_USERS_LIMIT") ?? "500").trim()) || 500;
const BATCH_PER_USER_MAX_MESSAGES = Number(
  (Deno.env.get("SOPHIA_MEMORIZER_MAX_MESSAGES_PER_USER") ?? "250").trim(),
) || 250;
const LOOKBACK_HOURS =
  Number((Deno.env.get("SOPHIA_MEMORIZER_LOOKBACK_HOURS") ?? "24").trim()) ||
  24;
...
function trimTranscript(
  ...
  return picked.map((m) =>
    `[${m.created_at}] ${String(m.role).toUpperCase()}: ${
      String(m.content ?? "")
    }`
  ).join("\n");
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const disabled =
      (Deno.env.get("SOPHIA_MEMORIZER_DISABLED") ?? "").trim() === "1";
    if (disabled) {
    ...
    const cutoffIso = new Date(Date.now() - LOOKBACK_HOURS * 60 * 60 * 1000)
      .toISOString();
    ...
    for (const c of candidates) {
      try {
        ...
        const temp = ((state as any)?.temp_memory &&
            typeof (state as any).temp_memory === "object")
          ? { ...(state as any).temp_memory }
          : {};
        const jobState =
          (temp.__job_state && typeof temp.__job_state === "object")
            ? { ...(temp.__job_state as Record<string, unknown>) }
            : {};
        const memorizerState =
          (jobState.memorizer && typeof jobState.memorizer === "object")
            ? { ...(jobState.memorizer as Record<string, unknown>) }
            : {};

        const latest = String(rows[rows.length - 1]?.created_at ?? "");
        const already = String(memorizerState.last_message_at ?? "");
        ...
        temp.__job_state = {
          ...
          memorizer: {
            ...
            last_counts: {
              ...
              globals_created: result.globalMemoriesCreated,
              globals_updated: result.globalMemoriesUpdated,
              globals_noop: result.globalMemoriesNoop,
              globals_pending_compaction:
                result.globalMemoriesPendingCompaction,
            },
          },
        };
        ...
        globalCreated += result.globalMemoriesCreated;
        globalUpdated += result.globalMemoriesUpdated;
        globalNoop += result.globalMemoriesNoop;
        globalPendingCompaction += result.globalMemoriesPendingCompaction;
        details.push({
          user_id: c.user_id,
          scope: c.scope,
          created: result.topicsCreated,
          ...
          globals_updated: result.globalMemoriesUpdated,
          globals_noop: result.globalMemoriesNoop,
          globals_pending_compaction: result.globalMemoriesPendingCompaction,
        });
      } catch (e) {
        skipped++;
        details.push({
    ...
    return jsonResponse(req, {
      ...
      created_global_memories: globalCreated,
      updated_global_memories: globalUpdated,
      noop_global_memories: globalNoop,
      pending_global_memory_compaction: globalPendingCompaction,
      skipped,
      details,
    }, { includeCors: false });
  } catch (error) {
