...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const checkin of checkins) {
      ...
      // Follow-ups on yesterday's plan actions should not open a closed
      // WhatsApp conversation through a generic template.
      if (isActionMorningFollowup && !in24hConversationWindow) {
        await supabaseAdmin
          .from("scheduled_checkins")
          .update({
            status: "cancelled",
            processed_at: new Date().toISOString(),
            delivery_last_error:
              "action_morning_followup_closed_whatsapp_window",
      ...
      if (isLevelReviewReminder || isLevelAutoGenerated) {
        ...
        if (!message) {
          await markScheduledCheckinDeliveryState({
            ...
            errorMessage: `${purpose}_template_missing_closed_window`,
            requestId,
          });
          continue;
        }
