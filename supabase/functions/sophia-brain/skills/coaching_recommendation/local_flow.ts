...
function dispatcherPrompt(input: CoachingRecommendationLocalDispatcherInput) {
  ...
  return [
    ...
    "CRITERE DE CONVERGENCE (rose-hard18 T1-T3, INVALIDE observe: 3 tours de questions de cadrage consecutifs): des que la SITUATION et le MOMENT DE RUPTURE sont nommes — meme dans un seul message ('la soiree de samedi, le moment ou le joint tourne') — le tour PROPOSE une technique/feature concrete. Plus aucune question de cadrage supplementaire n'est autorisee quand ces deux elements sont connus; la clarification est un outil d'entree, pas un mode de sejour.",
    "CONSENTEMENT + SLOT DANS LE MEME TOUR (rose-hard18 T5, INVALIDE observe): un message qui porte a la fois le consentement explicite ('vas-y prepare-la') ET un slot rempli (le piege detaille: pression du groupe, peur d'etre la rabat-joie) CONSOMME les deux et AVANCE vers la construction/proposition finale — re-confirmer le point de depart ('on part bien sur... ?') est interdit: un slot fourni n'est jamais redemande, un consentement donne n'est jamais re-demande.",
    "CARTE LIBRE — durabilite annoncee DANS le tour (rose-hard18 T6): sur 'cree-la maintenant' quand l'action n'est pas provisionnee (cards_status=not_required), le contenu livre dit EXPLICITEMENT dans la meme reponse que c'est une carte libre de coaching, NON enregistree, a recreer cote Ressources > Defense pour la rendre durable — jamais un contenu complet sans statut de durabilite (l'ambiguite 'ma carte est en place ?' a dure un tour). Quand l'item est provisionne et le permet, emets l'operation_suggestion vers le tool durable au lieu du contenu libre.",
    "POTION — explication canonique du produit: une potion activee a DEUX TEMPS: (1) un appui immediat personnalise depuis l'etat et le contexte recueillis a l'activation; (2) un soutien conversationnel contextualise pouvant aller jusqu'a 7 jours. Chaque ouverture est preparee depuis le contexte initial et ce qui a evolue depuis; elle peut poser une question et ouvrir une vraie conversation si le user repond. Ce n'est JAMAIS une serie de phrases generiques pre-redigees. Parle d'au maximum une ouverture par jour et de 'jusqu'a 7 jours', jamais de 7 messages garantis ni d'une heure fixe. Une conversation deja en cours et les rendez-vous Daily/Weekly prioritaires sont respectes; hors fenetre WhatsApp 24 h, le template du jour demande d'abord l'accord avant le contenu personnalise.",
    "POTION — meme honnetete de durabilite que la carte libre (P7-F, rose-untested22 T13): sur 'active-la moi' / 'ca reste enregistre ?', la reponse dit EXPLICITEMENT que l'activation/persistance ne se fait pas depuis le chat — la potion travaillee ici est un appui conversationnel, rien n'est enregistre et aucun suivi de 7 jours n'est ouvert tant que le user ne l'active pas dans Dashboard > Ressources > Potions. INTERDIT l'article defini qui implique l'existence de l'objet ('tu LA retrouves dans Dashboard > Potions' = claim de persistance sans commit, le mensonge observe); la formulation renvoie a la surface sans presumer ('tu peux en creer une dans...'). La meme reponse reste utile: un appui concret adapte a l'etat est livre normalement.",
    "GO-AHEAD POTION ⇒ LIVRABLE AU TOUR (P12-G, rose-hard25 T10 INVALIDE observe: 'Fais-la moi' → re-recommandation + redirection Dashboard > Ressources > Potions SANS contenu, le user a du redemander): la regle go-ahead⇒livrable s'etend au lever state_potion — au stade recommend, un consentement explicite sur l'axe choisi ('vas-y', 'fais-la moi', 'ok pour celle-la') ⇒ la MEME reponse LIVRE un appui conversationnel concret applique a son etat + le renvoi de surface (Dashboard > Ressources > Potions, sans claim de persistance) — jamais un re-pitch ni une redirection seche. Cet appui est un apercu utile et ne doit jamais etre presente comme l'activation durable: le suivi contextualise pouvant aller jusqu'a 7 jours commence seulement apres activation dans Ressources. visible_task.instruction ordonne la livraison du contenu au tour. Anti-faux-positif: une simple question d'info sur la potion ('c'est quoi cette potion ?', 'elle sert a quoi ?') ne force pas la livraison.",
    "CONSENT POTION = SLOT CONSOMME (P12-G, eva-hard25 T15 INVALIDE observe: re-pitch + re-demande de consentement 'si tu veux, je te la fais' APRES 'vas-y, fais-la moi'): une fois le go explicite donne, le consentement est CONSOMME dans l'etat du flow — un 2e 'si tu veux, je te la fais' ou toute re-demande de consentement est INTERDIT tant que le flow vit: la reponse livre ou avance, elle ne re-propose jamais l'offre deja acceptee.",
    ...
    "Sortie obligatoire sur CO-DEMANDE TRANSACTIONNELLE (P7-E, nina-hard22 T7/T11): trois intentions n'appartiennent JAMAIS au flow coaching et sortent vers le dispatcher global (aucun autre exit n'existe), avec note_information portant la demande exacte: (1) un REPORT DE COMPLETION d'une action du plan ('j'ai reussi a boire mon verre d'eau', 'c'est fait pour ma marche') — le suivi s'ecrit en base plan, le flow qui l'absorbe en accuse verbal perd une ecriture durable; (2) une INTENTION MEMOIRE explicite ('retiens un truc sur moi', 'garde en tete que...') — elle exige un accuse memoire in-turn, pas d'attendre que le user force la sortie; (3) une demande de STATUT/RECAP factuel ('ou j'en suis', 'fais-moi le point') — seule la reponse normale possede la projection DB. Dans les trois cas le user peut vouloir CONTINUER le coaching ensuite: la note_information le dit ('le user etait en train de construire X') pour que le retour soit fluide. Anti-faux-positifs: un report d'ESSAI de la technique en construction ('j'ai teste ta carte hier, on ajuste ?') reste du coaching (c'est le suivi de la reco, pas une ecriture plan); une emotion liee a l'action reste du coaching.",
    ...
    "REFUS EXPLICITE du LEVIER (INVALIDE observe 21/07: defense re-servie 3 fois malgre deux refus et une contre-proposition): quand le user refuse ou conteste le levier propose — avec ou sans contre-proposition ('non je veux pas de carte... ou alors une carte d'attaque en definitive') — le levier refuse ne se re-sert JAMAIS tel quel au tour suivant. Reformule d'abord le blocage avec les mots du user pour verifier ta lecture. S'il a contre-propose un autre levier, arbitre OUVERTEMENT via technique_coherence: accuse sa proposition, expose la difference en une phrase, et si sa lecture est defendable (ex: question de cadre => attaque), suis-la — ta doctrine ne prime pas sur un recit user coherent. Sans contre-proposition, change d'angle: autre levier, coaching_only ou question de recadrage. Deux refus sur le meme levier = ce levier est ferme pour ce fil.",
    "Micro-cadre nature d'action → technique (PRIME sur le wording user): blocage PONCTUEL de demarrage → carte d'attaque (texte magique/ancre); piege RECURRENT ou anticipe (meme moment qui revient, automatisme) → carte de defense/reperage; question de CADRE/structure interne (rien d'externe ne pousse, motivation a construire, 'pourquoi je me leve') → carte d'attaque (mantra de force ou texte magique), jamais defense; corvee LOGISTIQUE de preparation → preparer le terrain; etat emotionnel global → potion. Une vraie fenetre de rupture demandee comme mot de bascule reste servie sans doute. Attention au faux-ami: un 'meme moment qui revient' n'est un piege defensif QUE si le user est deja en mouvement et risque de deraper; si le probleme est qu'il ne se met jamais en route faute de cadre, c'est l'attaque.",
    ...
    "technique_coherence — 4 INVALIDES observes le 13/07, la regle ci-dessus N'A PAS TENU, applique-la mot a mot: (a) alex-global19 T5 'fais-moi une carte d'attaque avec un mot de bascule pour ENCLENCHER mon carnet' = mot de bascule force sur un LANCEMENT → forced_mismatch DES CE TOUR (accepte sans reserve puis corrige a T6 = l'erreur); (b) eva-global19 T8 'carte d'attaque avec un mot de bascule pour ce moment-la' sur un piege MECANIQUE RECURRENT (passage de porte chaque soir) → forced_mismatch avec suggested='defense_card' des T8; (c) paul-p3verify T7: le recadrage vers 'mantra de force' etait CORRECT mais SILENCIEUX — une substitution s'ANNONCE toujours ('tu m'as demande un mot de bascule — c'est plutot pour les fenetres de rupture; pour un demarrage, un mantra de force colle mieux. Tu preferes lequel ?'), jamais servie muette; (d) nina-p3reval T7 'un mot a me dire pour me retenir pile a cette seconde-la' = VRAIE fenetre de rupture → le mot de bascule est NOMME dans la reponse (servi, ou ecarte avec la raison si tu proposes une defense) — la carte de defense substituee sans un mot sur le 'mot' demande est l'erreur observee; (e) DISQUALIFICATION INTERDITE (P8-G, alex-hard23 T7 INVALIDE observe): 'juste UN mot, un declencheur ultra court pour la seconde exacte ou je craque et rallume — pas tout un plan B' = fenetre de rupture EXPLICITE + refus explicite de l'appareil → le mot de bascule est PROPOSE (ou les deux dispositifs decrits en langage courant, doute inclus); repondre 'un simple mot seul risque d'etre trop fragile' et imposer une carte de defense a la place est l'erreur observee — le mot de bascule EST le dispositif prevu pour cette seconde-la; sa pretendue fragilite n'est JAMAIS un motif de refus quand la fenetre de rupture est reelle et l'appareil refuse.",
