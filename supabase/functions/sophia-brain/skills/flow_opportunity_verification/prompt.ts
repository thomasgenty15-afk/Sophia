...
export function flowOpportunityLocalDispatcherSystemPrompt(): string {
  return [
    ...
    "Tu utilises `exit_to_global_dispatcher` si le user veut juste arreter ou repousser cette verification sans nouveau sujet clair.",
    ...
    "Priorites: safety, commande explicite, question produit/status inline, acceptation/refus/revision, repetition, sortie globale.",
    ...
    "",
    "`flow_action`: decision principale du tour. Elle doit refleter le message courant dans l'etat actif, pas seulement l'etat precedent. Utilise seulement les actions du contrat: `offer_opportunity`, `insufficient_response`, `get_info_product`, `get_info_db`, `repeat_current_state`, `revise_focus`, `correct_target_flow`, `handoff_to_local_flow`, `blocked_or_unsupported`, `exit_to_global_dispatcher`, `cancel_flow`, `defer_flow`, `complete_flow`, `safety_preempt`. N'invente jamais une action absente du contrat ou une action generique. Cette action determine la suite reducer: prompt visible local, inline roundtrip, handoff, issue locale, exit global ou safety.",
    ...
    "`note_information`: objet `{ needed, note }`. Mets `needed=true` et une `note` complete pour tout changement de dispatcher: `get_info_product`, `get_info_db`, `handoff_to_local_flow`, `exit_to_global_dispatcher`, `safety_preempt`. Mets `needed=false`, `note=null` quand le flow reste local ou se clot via cancel/defer/complete sans changement de dispatcher. La note garde strictement la structure source_flow_id, target_dispatcher, handoff_reason, handoff_context_for_next_dispatcher, user_words, structured_context, confidence si utile. user_words contient 1 a 3 fragments du message courant. structured_context est succinct et non vide: ancre de confirmation, target_flow, contraintes user, unresolved_questions et recommended_next_focus. Ne mets pas source_flow_presentation, source_flow_state_summary, target_local_dispatcher_hint, risk_score ou committed_effects dans la note. La note est consommee par le dispatcher cible; elle ne doit jamais etre transmise brute dans `conversation_context`.",
    ...
    "`exit_memo`: resume de sortie pour le reducer. Mets `needed=true` pour `exit_to_global_dispatcher`, `safety_preempt`, stale ou unsupported; indique `reason`, `flow_summary`, cible originale, contexte cible, hint global si utile, et `same_user_message_should_be_reprocessed=true` seulement si le message doit etre reanalyse par le global. Pour cancel/defer/complete local: `needed=false` ou reason `cancelled`, et reprocess=false.",
    ...
    "`adjust_plan_item`: ne handoff jamais si le user exprime une contrainte structuree de maintien du plan (`keep_plan`, `no_plan_edit`, `plan_should_stay`, `garder le plan`) ou dit que le probleme est l'elan, la peur, le sens ou l'evitement tout en voulant garder la direction. Dans ce cas, corrige la cible vers demotivation_repair si l'opportunite et l'ancre le permettent, sinon bloque ou repete l'offre sans proposer de modification.",
    "`one_shot_reminder`: ne handoff jamais depuis une demande vague d'appui/support/presence/coup de pouce. Il faut une demande explicite de rappel/notification/programmer et un moment ou delai exploitable dans target_context ou target_flow_input; sinon bloque ou retourne global sans effet.",
...
export function buildFlowOpportunityLocalDispatcherUserPrompt(input: {
  ...
  return JSON.stringify({
    ...
    required_json_shape: {
      flow_action:
        "offer_opportunity|insufficient_response|get_info_product|get_info_db|repeat_current_state|revise_focus|correct_target_flow|handoff_to_local_flow|blocked_or_unsupported|exit_to_global_dispatcher|cancel_flow|defer_flow|complete_flow|safety_preempt",
