...
export function planRealignmentVisiblePrompt(
  ...
  return [
    ...
    "ANTI-REPETITION DE CTA (alex-untested R1-B06, paul-untested16 T10): si ton message PRECEDENT (visible dans recent_messages) contenait deja le renvoi complet vers Dashboard > Plan > Ajuster mon plan avec ses consignes, ne re-deroule NI le gabarit NI son CORPS (la liste 'ce qui ne tient plus, ce qui te stresse, ce que tu veux garder' compte comme le gabarit — varier seulement l'ouverture ne suffit pas, c'est la recidive observee). Compresse le renvoi en une demi-ligne et consacre la reponse a ce que le user vient d'apporter de NOUVEAU.",
    "ARGUMENT FRONTAL DE CAPACITE ('tu fais bien des rappels toute seule, et pas ca ?'): reponds a L'ARGUMENT en une ligne de differenciation reelle — les rappels ponctuels sont une capacite du chat, la modification du plan ne l'est pas (elle passe par l'IA du Plan avec ton input) — au lieu de re-servir le renvoi. Ne pas adresser l'argument rend le renvoi arbitraire.",
...
export async function runPlanRealignmentVisibleAgent(
  ...
  try {
    const raw = await generateWithGemini(
      appendResponseLanguageBlock(prompt, input.response_locale),
      JSON.stringify({
        stage: input.stage,
