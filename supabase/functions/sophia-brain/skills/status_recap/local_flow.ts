...
export function normalizeStatusRecapLocalDispatcherOutput(
  ...
  const noteInformation = transitionNote
    ? normalizeNoteInformation(root.note_information, {
      ...
      source_flow_state_summary: stringValue(intentRoot.summary) ||
        "Status recap local flow is handing off without mutation.",
      ...
      target_local_dispatcher_hint: transitionNote.hint,
  ...
  return {
    ...
    state_updates: {
      status: enumValue(
        ...
        flowAction === "cancel_flow" || flowAction === "stop_local_no_handoff"
          ? "closed"
          : "active",
      ),
...
export function buildStatusRecapConversationContext(args: {
  ...
  return {
    ...
    handoff_data: {
      inbound_note_summary: args.noteInformationInbound
...
export function reduceStatusRecapLocalDispatcherOutput(args: {
  ...
  if (
    ...
    return {
      status: "closed",
      reason_code: output.flow_action === "stop_local_no_handoff"
        ? "status_recap_local_stop_local_no_handoff"
        : "status_recap_local_cancelled",
      local_state: createStatusRecapFlowState({
        previous: args.previous,
        status: "closed",
        lastIntent: args.previous?.last_intent ?? "unclear",
...
export function dispatcherSystemPrompt(): string {
  return [
    ...
    "Tout changement de dispatcher exige note_information: source_flow_id=status_recap, target_dispatcher, handoff_reason, résumé du message user, état collecté, questions non résolues, evidence, confidence et recommended_next_focus dans structured_context.",
    ...
    "Si le user veut juste arrêter le flow sans nouveau sujet clair, utilise stop_local_no_handoff. Ne renvoie pas vers le dispatcher global.",
    ...
    "- flow_action: décision principale du tour courant. Utilise answer_status pour un état factuel global, answer_object_status pour cartes/rappels/potions/actions précis, answer_coach_preferences_status pour préférences coach, answer_cancelled_objects pour annulés, answer_recent_effects pour effets récents, answer_fait_prevu_fragile pour ce format, repeat_last_status pour répéter, explain_sources pour expliquer les sources, no_source_status si la demande est status mais le db_context_pack indique qu'aucune source utile n'existe, narrow_scope si la cible status est trop large ou ambiguë, human_recap_no_db si le user demande un résumé humain de conversation et non un état DB, stop_local_no_handoff ou cancel_flow si le user arrête sans nouveau sujet, exit_to_global_dispatcher si le user change clairement de sujet ou demande une action produit, handoff_to_local_flow seulement si un autre dispatcher local précis doit reprendre, safety_preempt seulement en cas de safety réelle.",
    ...
    "- state_updates.status: active quand status_recap continue, closing quand le tour visible doit probablement clore, closed pour stop_local_no_handoff/cancel_flow, exit_to_global pour exit_to_global_dispatcher, safety pour safety_preempt.",
    ...
    "- note_information: null quand status_recap reste owner ou stoppe localement sans handoff. Obligatoire pour exit_to_global_dispatcher, handoff_to_local_flow et safety_preempt. Elle est consommée par le dispatcher cible, jamais transmise brute au prompt visible. structured_context doit inclure source_flow, target_dispatcher, handoff_reason, user_message_summary, active_flow_summary, collected_state, unresolved_questions, confidence, evidence et recommended_next_focus.",
    "- exit_memo.needed: false pour continuation locale, répétition, sources, no_source, stop local sans nouveau sujet. true pour exit_to_global_dispatcher, handoff_to_local_flow ou safety_preempt.",
    ...
    "- stop_local_no_handoff ou cancel_flow: le user arrête ce flow sans nouveau sujet clair; state_updates.status=closed, visible_task.kind=stop_or_cancel, note_information=null, exit_memo.needed=false.",
    "- exit_to_global_dispatcher: nouveau sujet clair, demande de création/modification/activation/annulation, ou aide produit; state_updates.status=exit_to_global, visible_task.kind=exit_ack, exit_memo.needed=true, note_information obligatoire.",
