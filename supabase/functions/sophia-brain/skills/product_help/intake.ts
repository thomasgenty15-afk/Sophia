...
function includesLocationOu(text: string): boolean {
  return /(^|\s)ou($|\s)/.test(text);
}
...
// Kept only for explicit tests and emergency comparison. Production uses
// runProductHelpStructuredIntake() and never calls this as a normal fallback.
export function legacyProductHelpHeuristicIntake(
  input: ProductHelpStructuredIntakeInput,
): ProductHelpDecision {
  const text = normalizeText(input.user_message);
  const effects = input.recent_object_candidates;
  const intent = legacyInferIntent(text);
  const resolved = legacyResolveObjectType(text, effects);
  const candidateFeature = choosePrimaryCatalogCandidate(
    input.catalog_candidates,
  );
  const objectFeature = pickCatalogFeatureForObject(resolved.objectType ?? "");
  const preferObjectFeature = Boolean(
    objectFeature &&
      (resolved.objectType !== "plan_item" ||
        intent === "tool_action_request" ||
        intent === "object_status_question"),
  );
  const primaryFeature = preferObjectFeature
    ? objectFeature!
    : candidateFeature;
  const explicitObject = Boolean(resolved.objectType);
  const needsRealObjectSource = explicitObject &&
    legacyAsksSpecificExistingObject(text) &&
    [
      "where_is_it",
      "modify_or_cancel_where",
      "object_status_question",
    ].includes(intent);
  const bridgeOperation = intent === "tool_action_request"
    ? bridgeOperationForFeature(primaryFeature, resolved.objectType ?? null)
    : null;
  const preserveActiveFlow = Boolean(input.active_skill_working_state) &&
    input.active_skill_working_state?.skill_id !== "product_help";
  return baseProductHelpDecision({
    intent,
    target: {
      kind: intent === "tool_action_request"
        ? "tool_flow"
        : resolved.source
        ? "recent_effect"
        : explicitObject
        ? "user_object"
        : "feature_catalog",
      feature_id: resolved.objectType === "one_shot_reminder"
        ? "one_shot_reminder.chat"
        : primaryFeature.id,
      object_type: resolved.objectType ?? undefined,
      object_ref: resolved.source?.label,
      confidence_band: resolved.objectType || primaryFeature ? "high" : "low",
    },
    grounding: {
      catalog_feature_ids: catalogFeatureIds([
        primaryFeature,
      ...
      db_sources_required: Boolean(
        needsRealObjectSource || intent === "object_status_question",
      ),
      db_sources_used: [
        ...sourceForCandidate(resolved.source),
        {
          source_type: "catalog",
          id: primaryFeature.id,
          label: primaryFeature.label,
        },
      ],
    },
    ...
    constraints: [
      ...
      "do_not_claim_object_exists_without_source",
      "do_not_render_status_block",
      ...(preserveActiveFlow ? ["preserve_active_flow" as const] : []),
      ...(["where_is_it", "modify_or_cancel_where"].includes(intent)
        ? ["exact_location_requested" as const]
        : []),
      "short_reply",
    ],
    response_contract: {
      max_questions: intent === "unclear" ? 1 : 0,
      allow_operation_suggestion: false,
      allow_status_projection: false,
      allow_generic_catalog_answer: !needsRealObjectSource,
      must_include_location: [
        "where_is_it",
        "how_to",
        "modify_or_cancel_where",
    ...
    state_patch: {
      summary:
        `Legacy product help heuristic intake: ${intent} / ${primaryFeature.id}`,
      decision: "legacy_product_help_heuristic_intake",
    },
  });
}

function legacyCurrentObjectType(text: string): ProductHelpObjectType | null {
  if (includesAny(text, ["carte de defense", "carte defense"])) {
    return "defense_card";
  }
  if (
    includesAny(text, [
  ...
  if (includesAny(text, ["rappel", "rappels", "rappelle"])) {
    if (includesAny(text, ["recurrent", "recurrente", "tous les", "chaque"])) {
      return "recurring_reminder";
    }
    return "one_shot_reminder";
  }
  if (includesAny(text, ["initiative", "initiatives"])) return "initiative";
  if (includesAny(text, ["potion", "potions"])) return "potion";
  if (includesAny(text, ["preference", "preferences", "ton coach"])) {
    return "preference";
  }
  if (includesAny(text, ["plan", "mission", "habitude", "action"])) {
    return "plan_item";
  }
  return null;
}

function legacyResolveObjectType(
  text: string,
  effects: ProductHelpObjectCandidate[],
): {
  objectType: ProductHelpObjectType | null;
  source?: ProductHelpObjectCandidate;
} {
  const current = legacyCurrentObjectType(text);
  if (current) {
    return {
      objectType: current,
      source: effects.find((effect) => effect.object_type === current),
    };
  }
  if (
    includesAny(text, [
    ...
    const unique = [...new Set(effects.map((effect) => effect.object_type))];
    if (unique.length === 1) {
      return { objectType: unique[0], source: effects[effects.length - 1] };
    }
  }
  return { objectType: null };
}

function legacyAsksSpecificExistingObject(text: string): boolean {
...
function legacyInferIntent(text: string): ProductHelpIntent {
  const asksLocation = includesAny(text, [
    ...
    "dans l'app",
    "emplacement",
  ]) || includesLocationOu(text);
  const asksModifyCancelWhere = asksLocation &&
    includesAny(text, ["modifier", "annuler", "supprimer", "corriger"]);
  if (asksModifyCancelWhere) return "modify_or_cancel_where";
  if (
    includesAny(text, [
      "cree-moi",
      "cree moi",
      ...
      "enregistre",
      "sauvegarde",
    ]) && !includesAny(text, ["comment", "je peux"]) &&
    !includesLocationOu(text)
  ...
  if (
    includesAny(text, [
      ...
      "qu est ce qui est actif",
      "quelles prefs",
      "preferences sont enregistrees",
      "preference est enregistree",
      "recap",
      "récap",
    ])
  ) return "object_status_question";
  if (includesAny(text, ["compare", "difference", "différence"])) {
    return "compare_features";
  }
  if (includesAny(text, ["limite", "limites", "je peux"])) {
    return "can_i_do_x";
  }
  if (asksLocation) return "where_is_it";
  if (
    includesAny(text, [
      "comment",
      "utilise",
