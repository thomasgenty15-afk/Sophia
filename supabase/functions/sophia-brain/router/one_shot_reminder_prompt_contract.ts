...
export function oneShotReminderCanonicalDispatcherPromptLines(): string[] {
  return [
    ...
    "   - Utilise direct_effect_time_context comme seule source temporelle: now_utc, user_timezone, user_locale, user_local_datetime, user_local_human et named_day_calendar.",
    "   - UN JOUR NOMME SE LIT, IL NE SE CALCULE PAS. named_day_calendar donne les 8 prochains jours civils de l'eleve avec leur date ISO et leur nom dans les DEUX langues (2026-08-06 = jeudi / thursday (aujourd'hui / today)). Pour 'jeudi', 'ce jeudi', 'jeudi soir', 'thursday': prends la PREMIERE ligne de la table dont les noms contiennent ce jour, et copie son ISO tel quel. Ne recalcule JAMAIS une date depuis user_local_datetime — Un jour nomme FUTUR ressort deja juste ('samedi' -> la bonne date); c'est le jour COURANT qui derape.",
    "   - UN JOUR NOMME QUI EST AUJOURD'HUI VEUT DIRE AUJOURD'HUI. C'est le seul cas ou le modele se trompe, et il s'y trompe SYSTEMATIQUEMENT: MESURE (run reel 2026-08-06, un JEUDI) 'jeudi soir' resolu en 2026-08-07 3 fois sur 3, alors que la meme phrase avec 'samedi soir' ressortait juste. Si l'entree offset 0 porte le jour nomme, prends son ISO — pas offset 1, pas offset 7. Il faut le mot 'prochain'/'next' pour viser la semaine suivante, et dans ce cas SEULEMENT prends la seconde occurrence du nom.",
