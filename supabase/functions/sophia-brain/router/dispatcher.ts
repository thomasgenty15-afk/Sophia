...
type DispatcherLlmRunner = typeof generateWithGemini;
...
export function setDispatcherLlmRunnerForTest(
  runner: DispatcherLlmRunner | null,
): void {
  dispatcherLlmRunner = runner ?? generateWithGemini;
}

export function resolveDispatcherModelSelection(): {
  primaryModel: string;
...
export interface DispatcherSignals {
  ...
  defense_card_win: {
    detected: boolean;
    confidence: number;
    /** Brief description of what the user resisted */
...
export interface SignalHistoryEntry {
  /** Signal type (e.g., "plan_feedback", "plan_item_discussion") */
  signal_type: string;
...
export interface DispatcherPlanItemSnapshotItem {
  id: string;
  title: string;
  dimension: string;
  item_type: string;
...
export interface DispatcherInputV2 {
  ...
  /** Canonical V2 dispatcher snapshot of plan items. */
  plan_item_snapshot?: DispatcherPlanItemSnapshotItem[];
  planItemSnapshot?: DispatcherPlanItemSnapshotItem[];
}
...
- track_progress_plan_item / track_progress_north_star (section TOOLS)
- plan_item_discussion / plan_feedback (section PLAN)
- needs_explanation (si l'utilisateur demande d'expliquer / clarifier)
- needs_research (si question factuelle fraîche / web)
- checkup_intent (si l'utilisateur veut lancer un bilan maintenant, même si les bilans sont surtout cron-driven en V2)
...
- defense_card_win (si user raconte avoir résisté à une pulsion/tentation)
- risk_score (0-10)

IMPORTANT:
- Le chat V2 ne crée pas et ne reconfigure pas le plan dans ce dispatcher.
- plan_item_discussion sert à reconnaître qu'on parle d'un item du plan.
- plan_feedback sert à capter un ressenti/retour utilisateur sur un item du plan.
- Les signaux dashboard_*_intent servent seulement à orienter vers les bons écrans dashboard.
...
- Le SNAPSHOT PLAN ITEMS V2 contient les éléments du plan de l'utilisateur (habitudes, missions, supports).
- Si le snapshot contient deja la cible exacte, recopie l'ID EXACT du snapshot dans "target_item_id".
- Si possible, recopie aussi le TITRE EXACT du snapshot dans "target_title".
- N'invente jamais un ID.
- Si plusieurs cibles du snapshot sont plausibles, mets "target_item_id" a null.
- Si tu reconnais l'intention de tracking mais que la cible reste ambigue, garde detected=true et mets "target_item_id" a null.

1) track_progress_plan_item
- Cas: user dit avoir fait / raté / partiellement fait un plan_item (habitude, mission, tâche, support).
- Renseigne: detected + target_item_id + target_title + status_hint (completed|missed|partial|unknown) + operation_hint + value_hint + date_hint.
- operation_hint:
...
=== SECTION PLAN ===
=== DISCUSSION ET FEEDBACK SUR LE PLAN ===
1) plan_item_discussion (discussion sur un plan_item existant)
- Détecte si le user parle d'un plan_item existant sans forcément demander une opération CRUD.
- Renseigne: plan_item_discussion.detected + plan_item_discussion.target_item_id + plan_item_discussion.item_hint.
- Utilise le SNAPSHOT PLAN ITEMS V2 pour mieux reconnaître l'élément visé (id, titre, dimension, statut).
- Tolère de petites fautes d'orthographe/accents/pluriels sur le nom.
- Exemples:
  * "ma medittion du soir me saoule" -> plan_item_discussion.detected=true, item_hint="méditation du soir"
  * "pour mon sport je bloque" -> plan_item_discussion.detected=true, item_hint="sport"
  * "je veux parler de mon plan" -> plan_item_discussion.detected=false

2) plan_feedback
- Détecte un feedback utilisateur sur un plan_item: trop dur, trop facile, utile, motivant, bloquant, bien avancé, etc.
- Renseigne: plan_feedback.detected + sentiment (positive|negative|neutral) + target_item_id + target_title + detail.
- detail = résumé très court du feedback (max ~120 chars).
- Si tu n'es pas sûr du plan_item ciblé, garde detected=true mais mets target_item_id à null.
- Exemples:
  * "cette habitude est trop dure" -> detected=true, sentiment=negative
  * "j'ai bien avancé sur mon projet" -> detected=true, sentiment=positive
  * "ce support m'aide moyen" -> detected=true, sentiment=neutral
...
4) dashboard_recurring_reminder_intent (redirection Rendez-vous)
...
5) defense_card_win (victoire contre une pulsion)
- Détecte quand le user raconte avoir RÉSISTÉ ou SURMONTÉ une pulsion / tentation / envie.
- Renseigne: detected + confidence + situation_hint (brève description de la situation surmontée).
- Le user doit EXPLICITEMENT dire qu'il a résisté. Une mention de difficulté sans victoire = PAS detected.
- Exemples:
  * "j'ai résisté au grignotage hier soir" -> detected=true, situation_hint="grignotage le soir"
  * "j'ai failli craquer mais j'ai tenu bon" -> detected=true, situation_hint="pulsion non précisée"
  * "j'avais trop envie de fumer mais j'ai bu un verre d'eau" -> detected=true, situation_hint="envie de fumer, remplacée par verre d'eau"
  * "j'ai craqué hier" -> detected=false (c'est un échec, pas une victoire)
  * "c'est dur de résister" -> detected=false (difficulté, pas de victoire explicite)
`;
...
- "j'ai fait X" = track_progress_plan_item, PAS checkup
- Discussion sur un item specifique du plan = PAS checkup
- "j'ai pas fait X" hors bilan = track_progress_plan_item, PAS checkup
`;
...
export function buildPlanItemSnapshotSection(
  planItemSnapshot?: DispatcherInputV2["plan_item_snapshot"],
): string {
  if (Array.isArray(planItemSnapshot) && planItemSnapshot.length > 0) {
    const lines = planItemSnapshot
      .slice(0, 25)
      .map((item) => {
        const title = String(item?.title ?? "").trim().slice(0, 80);
        if (!title) return null;
        const id = String(item?.id ?? "").trim().slice(0, 36);
        const dimension = String(item?.dimension ?? "").trim().slice(0, 12);
        const itemType = String(item?.item_type ?? "").trim().slice(0, 16);
        const status = String(item?.status ?? "").trim().slice(0, 16);
        const streak = Number(item?.streak_current ?? 0);
        const activeLoadScore = Number(item?.active_load_score);
        const lastEntry = item?.last_entry_at
          ? String(item.last_entry_at).slice(0, 10)
          : "jamais";
        const parts = [`- [${dimension}|${itemType}|${status}] ${title}`];
        if (streak > 0) parts.push(`(streak: ${streak})`);
        if (Number.isFinite(activeLoadScore) && activeLoadScore > 0) {
          parts.push(`(load: ${activeLoadScore})`);
        }
        parts.push(`— dernier: ${lastEntry}`);
        if (id) parts.push(`[id:${id}]`);
        return parts.join(" ");
      })
      .filter(Boolean);
    if (lines.length === 0) return "";
    return `\n=== SNAPSHOT PLAN ITEMS V2 ===
Elements du plan de transformation de l'utilisateur (habitudes, missions, supports).
...
function buildDispatcherVolatileSection(opts: {
  ...
  last5Messages: Array<{ role: string; content: string }>;
  signalHistory: SignalHistoryEntry[];
  plan_item_snapshot?: DispatcherInputV2["plan_item_snapshot"];
  planItemSnapshot?: DispatcherInputV2["plan_item_snapshot"];
}): string {
  ...
  const planItemSnapshot = resolvePlanItemSnapshot(opts);

  return `${buildPlanItemSnapshotSection(planItemSnapshot)}${
 ...
 */
export function buildDispatcherPromptV2(opts: {
  activeMachine: string | null;
  userMessage: string;
  last5Messages: Array<{ role: string; content: string }>;
  signalHistory: SignalHistoryEntry[];
  ...
  const resolvedPlanItemSnapshot = resolvePlanItemSnapshot({
    plan_item_snapshot,
    planItemSnapshot,
  });
  ...
  const volatilePrompt = buildDispatcherVolatileSection({
    ...
    plan_item_snapshot: resolvedPlanItemSnapshot,
  });
...
export async function analyzeSignalsV2(
  ...
  const promptParts = buildDispatcherPromptV2({
    ...
    plan_item_snapshot: resolvePlanItemSnapshot(input),
  });
...
=== FORMAT DE SORTIE JSON ===
{
  "signals": {
    ...
    "track_progress_plan_item": { "detected": "boolean", "target_item_id": "string|null", "target_title": "string|null", "status_hint": "completed|missed|partial|unknown", "operation_hint": "add|set|null", "value_hint": "number|null", "date_hint": "YYYY-MM-DD|null" },
    "track_progress_north_star": { "detected": "boolean", "value_hint": "number|null", "note_hint": "string|null", "date_hint": "YYYY-MM-DD|null" },
    "plan_item_discussion": { "detected": "boolean", "target_item_id": "string|null", "item_hint": "string|null" },
    "plan_feedback": { "detected": "boolean", "sentiment": "positive|negative|neutral", "target_item_id": "string|null", "target_title": "string|null", "detail": "string|null" },
    ...
    "defense_card_win": { "detected": "boolean", "confidence": "number", "situation_hint": "string|null" },
    "risk_score": "number"
  },
  "memory_plan": {
    "response_intent": "direct_answer|inventory|problem_solving|reflection|support|planning|tooling",
...
Reponds UNIQUEMENT avec le JSON:`;
  ...
  let dispatcherModelUsedForFailure = resolveDispatcherModelSelection()
    .primaryModel;
  try {
    const { primaryModel, fallbackGeminiModel } =
      resolveDispatcherModelSelection();
    let dispatcherModelUsed = primaryModel;
    dispatcherModelUsedForFailure = primaryModel;
    ...
    try {
      console.log(JSON.stringify({
        ...
        request_id: meta?.requestId ?? null,
        model: primaryModel,
        fallback_gemini_model: fallbackGeminiModel,
      }));
    } catch { /* ignore */ }
    let response: string | { tool: string; args: any };
    try {
      ...
      const shouldFallbackToGemini = /^gpt-/i.test(primaryModel);
      ...
      response = await dispatcherLlmRunner(
        ...
        "auto",
        {
          ...
          forceInitialModel: true,
          disableFallbackChain: true,
          source: "sophia-brain:dispatcher-v2-contextual:fallback-gemini",
        },
      );
    }
    ...
    const trackProgressPlanItemTargetItemId =
      (typeof planItemProgressRaw?.target_item_id === "string" &&
        ...
        ? planItemProgressRaw.target_item_id.trim().slice(0, 80)
        : undefined;
    const trackProgressPlanItemTargetTitle =
      (typeof planItemProgressRaw?.target_title === "string" &&
          planItemProgressRaw.target_title.trim())
        ? planItemProgressRaw.target_title.trim().slice(0, 120)
        : (typeof planItemProgressRaw?.target_hint === "string" &&
            planItemProgressRaw.target_hint.trim())
        ? planItemProgressRaw.target_hint.trim().slice(0, 120)
        : undefined;
    const trackProgressPlanItemStatusHintRaw = String(
      planItemProgressRaw?.status_hint ?? "unknown",
    ).toLowerCase();
    const trackProgressPlanItemStatusHint =
      (["completed", "missed", "partial", "unknown"] as const).includes(
          trackProgressPlanItemStatusHintRaw as any,
        )
        ? trackProgressPlanItemStatusHintRaw as
          | "completed"
          | "missed"
          | "partial"
          | "unknown"
        : "unknown";
    const trackProgressPlanItemOperationHintRaw = String(
    ...
    const trackProgressPlanItemValueHint =
      (typeof planItemProgressRaw?.value_hint === "number" &&
          !isNaN(planItemProgressRaw.value_hint))
        ? planItemProgressRaw.value_hint
        : undefined;
    const trackProgressPlanItemDateHint =
      (typeof planItemProgressRaw?.date_hint === "string" &&
          /^\d{4}-\d{2}-\d{2}$/.test(planItemProgressRaw.date_hint.trim()))
        ? planItemProgressRaw.date_hint.trim()
        : undefined;
    ...
    const planItemDiscussionDetected = Boolean(
      planItemDiscussionRaw?.detected,
    );
    const planItemDiscussionTargetItemId =
      (typeof planItemDiscussionRaw?.target_item_id === "string" &&
          planItemDiscussionRaw.target_item_id.trim())
        ? planItemDiscussionRaw.target_item_id.trim().slice(0, 80)
        : undefined;
    const planItemDiscussionHint =
      (typeof planItemDiscussionRaw?.item_hint === "string" &&
          planItemDiscussionRaw.item_hint.trim())
        ? planItemDiscussionRaw.item_hint.trim().slice(0, 120)
        : (typeof planItemDiscussionRaw?.action_hint === "string" &&
            planItemDiscussionRaw.action_hint.trim())
        ? planItemDiscussionRaw.action_hint.trim().slice(0, 120)
        : undefined;
    const planFeedbackDetected = Boolean(planFeedbackRaw?.detected);
    const planFeedbackSentimentRaw = String(
      planFeedbackRaw?.sentiment ?? "neutral",
    ).toLowerCase();
    const planFeedbackSentiment =
      (["positive", "negative", "neutral"] as const).includes(
          planFeedbackSentimentRaw as any,
        )
        ? planFeedbackSentimentRaw as "positive" | "negative" | "neutral"
        : "neutral";
    const planFeedbackTargetItemId =
      (typeof planFeedbackRaw?.target_item_id === "string" &&
          planFeedbackRaw.target_item_id.trim())
        ? planFeedbackRaw.target_item_id.trim().slice(0, 80)
        : undefined;
    const planFeedbackTargetTitle =
      (typeof planFeedbackRaw?.target_title === "string" &&
    ...
    // Parse defense_card_win signal
    const defenseCardWinDetected = Boolean(
      signalsObj?.defense_card_win?.detected,
    );
    const defenseCardWinConf = Math.max(
      0,
      Math.min(
        1,
        Number(signalsObj?.defense_card_win?.confidence ?? 0.5) || 0.5,
      ),
    );
    const defenseCardWinSituationHint =
      (typeof signalsObj?.defense_card_win?.situation_hint === "string" &&
          signalsObj.defense_card_win.situation_hint.trim())
        ? signalsObj.defense_card_win.situation_hint.trim().slice(0, 160)
        : undefined;
    ...
    return {
      signals: {
        ...
        track_progress_plan_item: {
          detected: trackProgressPlanItemDetected,
          target_item_id: trackProgressPlanItemTargetItemId,
          target_title: trackProgressPlanItemTargetTitle,
          status_hint: trackProgressPlanItemStatusHint,
          operation_hint: trackProgressPlanItemOperationHint,
          value_hint: trackProgressPlanItemValueHint,
          date_hint: trackProgressPlanItemDateHint,
        },
        track_progress_north_star: {
        ...
        plan_item_discussion: {
          detected: planItemDiscussionDetected,
          target_item_id: planItemDiscussionTargetItemId,
          item_hint: planItemDiscussionHint,
        },
        plan_feedback: {
          detected: planFeedbackDetected,
          sentiment: planFeedbackSentiment,
          target_item_id: planFeedbackTargetItemId,
          target_title: planFeedbackTargetTitle,
          detail: planFeedbackDetail,
        },
        dashboard_preferences_intent: {
        ...
        defense_card_win: {
          detected: defenseCardWinDetected,
          confidence: defenseCardWinConf,
          situation_hint: defenseCardWinSituationHint,
        },
      ...
      enrichments,
      machine_signals: machineSignals,
      model_used: dispatcherModelUsed,
    };
  } catch (e) {
    ...
    return {
      ...
      model_used: dispatcherModelUsedForFailure,
    };
  }
}
