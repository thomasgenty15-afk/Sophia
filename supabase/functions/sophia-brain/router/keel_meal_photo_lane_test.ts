...
import {
  ...
  KEEL_MEAL_PHOTO_FLOW_STATE_KEY,
...
import type { MealPhotoIntent } from "../../_shared/keel/meal_photo_flow.ts";
...
function memoryWithOpenFlow(question: string | null = "Was this cooked in oil?") {
  return applyMealPhotoFlowState({
    tempMemory: { some_other_key: 1 },
    flow: openMealPhotoFlow({ eventId: EVENT, question, now: OPENED_AT }),
    detectedFoods: ["pork", "rice"],
    now: OPENED_AT,
  });
}

const classifyAs = (intent: MealPhotoIntent) =>
...
Deno.test("lane: une correction amende ET désarme log_protocol_event", async () => {
  ...
  const out = await runMealPhotoCorrectionLane({
    supabase: db,
    userId: USER,
...
Deno.test("lane: un autre sujet ferme le flow et laisse le tour écrire", async () => {
  ...
  assertEquals(readMealPhotoFlowState(out.tempMemory), null, "le flow est fermé");
});
...
Deno.test("lane: un tour de crise ferme le flow, sans rien amender", async () => {
  ...
  assertEquals(readMealPhotoFlowState(out.tempMemory), null);
});
...
Deno.test("lane: l'incertitude laisse le tour à son chemin normal", async () => {
  ...
  assert(readMealPhotoFlowState(out.tempMemory) !== null);
});
...
Deno.test("état: un flow fermé ou déformé n'est jamais relu comme ouvert", () => {
  ...
  assertEquals(readMealPhotoFlowState(null), null);
  assertEquals(readMealPhotoFlowState({}), null);
  assertEquals(
    readMealPhotoFlowState({
      [KEEL_MEAL_PHOTO_FLOW_STATE_KEY]: { flow: { state: "closed", eventId: "e", openedAt: "x" } },
    }),
    null,
  );
