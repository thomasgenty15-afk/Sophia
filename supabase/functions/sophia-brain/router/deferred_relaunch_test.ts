...
function makeTopic(overrides?: Partial<DeferredTopicV2>): DeferredTopicV2 {
  return {
    ...
    signal_summaries: [{ summary: "flemme chronique", created_at: new Date().toISOString() }],
    trigger_count: 1,
...
Deno.test("processRelaunchConsentResponse: first unclear schedules re-ask", () => {
  ...
  assertEquals(askFlag?.action_target, "Faire du sport", "ask flag action_target")
})
...
Deno.test("processRelaunchConsentResponse: second unclear drops gracefully", () => {
  ...
  const first = processRelaunchConsentResponse({
    ...
    userMessage: "bof",
    profileConfirmDeferredKey: "__profile_deferred",
    dispatcherConsentSignal: { value: "unclear", confidence: 0.9 },
  })
  ...
  assertEquals(second.declineMessage!.length > 0, true, "decline message not empty")
...
Deno.test("processRelaunchConsentResponse: yes after first unclear initializes machine", () => {
  ...
  const second = processRelaunchConsentResponse({
    ...
    profileConfirmDeferredKey: "__profile_deferred",
    dispatcherConsentSignal: { value: true, confidence: 0.95 },
  })
...
Deno.test("processRelaunchConsentResponse: no after first unclear declines", () => {
  ...
  const second = processRelaunchConsentResponse({
    ...
    profileConfirmDeferredKey: "__profile_deferred",
    dispatcherConsentSignal: { value: false, confidence: 0.95 },
  })

  assertEquals(second.handled, true, "handled")
  assertEquals(second.shouldInitMachine, false, "no machine init")
  assertEquals(typeof second.declineMessage, "string", "has decline message")
  assertEquals(second.droppedAfterUnclear, undefined, "not dropped-after-unclear (clean decline)")
...
// ═══════════════════════════════════════════════════════════════════════════════
// Regex fallback still works for unclear
// ═══════════════════════════════════════════════════════════════════════════════

Deno.test("processRelaunchConsentResponse: unclear via regex fallback (no dispatcher signal)", () => {
  const topic = makeTopic()
  const { tempMemory: tm0 } = setPendingRelaunchConsent({ tempMemory: {}, topic })

  // Message that is neither yes nor no → unclear via regex fallback
  const result = processRelaunchConsentResponse({
    tempMemory: tm0,
    userMessage: "je vais au cinéma ce soir",
    profileConfirmDeferredKey: "__profile_deferred",
    // No dispatcher signal
  })

  assertEquals(result.handled, true, "handled")
  assertEquals(result.unclearReaskScheduled, true, "reask scheduled via regex fallback")
})
