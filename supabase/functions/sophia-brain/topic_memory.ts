...
import type { MemoryLayerScope } from "../_shared/v2-types.ts";
...
const MATCH_HIGH =
  Number((safeEnvGet("SOPHIA_TOPIC_MATCH_HIGH") ?? "0.72").trim()) || 0.72;
const KEYWORD_STRONG =
  Number((safeEnvGet("SOPHIA_TOPIC_KEYWORD_STRONG") ?? "0.86").trim()) ||
  0.86;
const SYNTH_STRONG =
  Number((safeEnvGet("SOPHIA_TOPIC_SYNTH_STRONG") ?? "0.84").trim()) || 0.84;
const NOOP_SEMANTIC_SIM =
  Number((safeEnvGet("SOPHIA_TOPIC_NOOP_SEMANTIC_SIM") ?? "0.90").trim()) ||
  0.90;
const TITLE_STRONG =
  Number((safeEnvGet("SOPHIA_TOPIC_TITLE_STRONG") ?? "0.88").trim()) || 0.88;
const MAX_KEYWORDS_PER_TOPIC_UPDATE = Number(
  (safeEnvGet("SOPHIA_TOPIC_MAX_KEYWORDS_PER_UPDATE") ?? "12").trim(),
) || 12;
const ALLOW_KEYWORD_REASSIGN =
  (safeEnvGet("SOPHIA_TOPIC_ALLOW_KEYWORD_REASSIGN") ?? "").trim() === "1";
const AUTO_MERGE_SYNTH_SIM = Number(
  (safeEnvGet("SOPHIA_TOPIC_AUTO_MERGE_SYNTH_SIM") ?? "0.93").trim(),
) || 0.93;
const AUTO_MERGE_TITLE_SIM = Number(
  (safeEnvGet("SOPHIA_TOPIC_AUTO_MERGE_TITLE_SIM") ?? "0.95").trim(),
) || 0.95;
const AUTO_MERGE_TITLE_JACCARD = Number(
  (safeEnvGet("SOPHIA_TOPIC_AUTO_MERGE_TITLE_JACCARD") ?? "0.60").trim(),
) || 0.60;
const MAX_TIMELINE_ITEMS =
  Number((safeEnvGet("SOPHIA_TOPIC_MAX_TIMELINE_ITEMS") ?? "10").trim()) ||
  10;
const TOPIC_DEBUG = (safeEnvGet("SOPHIA_TOPIC_DEBUG") ?? "").trim() === "1";
const TOPIC_COMPACTION_PENDING_COUNT = Number(
  (safeEnvGet("SOPHIA_TOPIC_COMPACTION_PENDING_COUNT") ?? "3").trim(),
) || 3;
const TOPIC_COMPACTION_PENDING_CHARS = Number(
  (safeEnvGet("SOPHIA_TOPIC_COMPACTION_PENDING_CHARS") ?? "420").trim(),
) || 420;
const MEMORY_ANALYSIS_MODEL =
  (safeEnvGet("SOPHIA_MEMORY_ANALYSIS_MODEL") ?? "gpt-5.3").trim() ||
  "gpt-5.3";
const MEMORY_VALIDATION_MODEL =
  (safeEnvGet("SOPHIA_MEMORY_VALIDATE_MODEL") ?? "gemini-3-flash-preview")
    .trim() || "gemini-3-flash-preview";
const TOPIC_COMPACTION_MODEL =
  (safeEnvGet("SOPHIA_TOPIC_COMPACTION_MODEL") ?? "gemini-3-flash-preview")
    .trim() || "gemini-3-flash-preview";
...
function readV2RuntimeRefs(
  provenance?: Record<string, unknown> | null,
): V2RuntimeRefs {
  const cycleId = String(
    provenance?.cycle_id ?? provenance?.cycleId ?? "",
  ).trim();
  const transformationId = String(
    provenance?.transformation_id ?? provenance?.transformationId ?? "",
  ).trim();
  return {
...
async function tryLogMemoryPersisted(params: {
  ...
  memoryType: "topic" | "global" | "event" | "core_identity";
  memoryId: string | null;
}): Promise<void> {
  try {
    await logV2Event(
      params.supabase,
      V2_EVENT_TYPES.MEMORY_PERSISTED,
      buildPersistedPayload({
    ...
    console.warn("[TopicMemory] memory_persisted_v2 log failed", {
...
function summarizeObservedTopic(
...
export async function enrichTopicSynthesis(opts: {
  ...
  const sourceType = opts.sourceType ?? "chat";
  const runtimeRefs = readV2RuntimeRefs(provenance);
  ...
  if (
    ...
    await supabase
      ...
      .eq("id", topic.id);
    await tryLogMemoryPersisted({
      supabase,
      userId,
      cycleId: runtimeRefs.cycleId,
      transformationId: runtimeRefs.transformationId,
      layer: "execution",
      action: "noop",
      memoryType: "topic",
      memoryId: topic.id,
  ...
  try {
    ...
    await supabase
      ...
      .update({
        ...
        metadata: {
          ...baseMetadata,
          cycle_id: runtimeRefs.cycleId,
          transformation_id: runtimeRefs.transformationId,
          timeline,
          source_refs: nextSourceRefs,
          latest_source_ref: provenance ??
            (baseMetadata as any)?.latest_source_ref ??
            null,
        },
        updated_at: now,
      })
...
export async function createTopic(opts: {
  ...
  await tryLogMemoryPersisted({
    ...
    action: "create",
    memoryType: "topic",
    memoryId: String((newTopic as any)?.id ?? ""),
  });
...
export async function retrieveTopicMemories(opts: {
  ...
  if (opts.transformationId) {
    const { data: scopedRows, error: scopedErr } = await supabase
      ...
      .eq("user_id", userId)
      .eq("status", "active")
      .contains("metadata", { transformation_id: opts.transformationId });
    if (scopedErr) {
      console.warn("[TopicMemory] transformation filter lookup failed", {
        user_id: userId,
        transformation_id: opts.transformationId,
        error: scopedErr.message,
      });
    } else {
      allowedTopicIds = new Set(
        (scopedRows ?? []).map((row: any) => String(row?.id ?? "").trim())
          .filter(
            Boolean,
          ),
      );
      if (allowedTopicIds.size === 0) return [];
    }
  }
  ...
  const titleResults =
    (Array.isArray((titleRaw as any)?.data)
      ...
      : []) as TopicSearchResult[];
  const filterByAllowedTopicIds = (rows: TopicSearchResult[]) =>
    allowedTopicIds
      ? rows.filter((row) =>
        allowedTopicIds!.has(String(row.topic_id ?? "").trim())
      )
      : rows;
  ...
  for (const r of filterByAllowedTopicIds(keywordResults)) {
    upsertRow(r, "keyword");
  }
  for (const r of filterByAllowedTopicIds(synthesisResults)) {
    upsertRow(r, "synthesis");
  }
  for (const r of filterByAllowedTopicIds(titleResults)) upsertRow(r, "title");
...
export async function processTopicsFromWatcher(opts: {
  ...
  const allowEventPersistence = sourceType === "chat" ||
    sourceType === "bilan" ||
    sourceType === "module";
  ...
  for (const event of extractedEvents) {
    try {
      ...
      await tryLogMemoryPersisted({
        ...
        transformationId: runtimeRefs.transformationId,
        layer: "event",
        action: result.created ? "create" : result.updated ? "update" : "noop",
        memoryType: "event",
        memoryId: result.eventId ?? null,
      });
    } catch (e) {
      console.error(
  ...
  for (const candidate of extractedGlobalMemories) {
    try {
      const classifiedScope =
        (runtimeRefs.cycleId || runtimeRefs.transformationId)
          ? classifyMemoryScope({
            content: [
