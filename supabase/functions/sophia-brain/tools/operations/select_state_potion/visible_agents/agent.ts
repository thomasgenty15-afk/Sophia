...
export type SelectStatePotionVisibleAgentInput = {
  ...
  handoff_status?: StatePotionHandoffStatus | null;
...
export function visibleContractIssues(
  ...
  const potionLabel = input.selected_potion_label ??
    input.draft?.recommendation.platform_inputs?.potion_title ??
    input.draft?.recommendation.potion_label ??
    null;
  if (
    [
      "potion_selected",
  ...
  if (input.stage === "clarte_task") {
    const task = input.clarte_visible_task;
    const fieldValue = input.clarte_state?.field_state.locked_value ??
      input.clarte_state?.field_state.candidate_value ??
      null;
...
function clarteVisibleTaskInstruction(
  ...
  return [
    ...
    ...(input.stage === "clarte_task" &&
        [
          ...
          "apply_attempt",
          "repeat_handoff",
        ].includes(String(input.clarte_visible_task ?? ""))
      ? [
...
async function generateVisibleMessage(
  ...
  const userPrompt = JSON.stringify({
    ...
    selected_potion_label: input.selected_potion_label,
    current_field_id: input.current_field_id,
    question_intent: input.question_intent,
    ...
    hard_constraints: {
      ...
      validation_errors_to_fix: retryIssues,
    },
    required_json_shape: {
      message: "string",
    },
