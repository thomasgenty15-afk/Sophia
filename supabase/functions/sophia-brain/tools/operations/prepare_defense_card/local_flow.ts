...
export function localDispatcherSystemPrompt(): string {
  return [
    ...
    "Rythme conversationnel V1: par défaut, au premier tour d'entrée dans prepare_defense_card, fais une passe courte d'enrichissement/éclaircissement avec ask_trigger_or_signal, ask_defense_goal_or_response ou ask_risk_situation.",
    "Exception compact intake: si le user demande explicitement une carte courte/simple/avec ce que tu sais/pas trois questions et donne déjà une situation ou moment de risque + une parade/action/phrase exploitable, ne redemande pas slot par slot. Tu peux proposer confirm_support_need_proposal dès ce tour ou handoff_ready si support_need est clairement formulé/validé par le user.",
    "Interdiction premier tour hors compact: aucune restitution plateforme au premier message visible. Ne demande pas confirmation d'une formulation, ne donne pas de phrase à recopier, ne cite pas le label du champ, ne cite pas Cartes de défense ni la destination. La restitution du support_need peut arriver au plus tôt au tour suivant, après une vraie passe d'échange.",
    ...
    "- Compact intake support_need: si le user donne deja le moment de risque et la parade, construis un support_need proposed ou locked a partir de cette situation concrete; ne pose pas une question supplementaire juste pour reformuler ce qui est deja exploitable.",
