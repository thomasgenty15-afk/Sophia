...
function buildPrepareAttackCardConversationContext(args: {
  ...
  const techniqueCandidateOptions = args.visibleTask ===
      "ask_or_confirm_technique"
    ? args.state.technique_state.candidate_options
    : [];
  const recommendedTechnique =
    techniqueCandidateOptions.find((option) => option.recommended) ?? null;
  ...
  return {
    ...
    selected_candidate: {
      ...selectedCandidate,
      candidate_options: techniqueCandidateOptions,
      allowed_technique_labels: techniqueCandidateOptions.map((option) =>
        option.technique_label
      ),
  ...
  if (
    ...
    return {
      ...
      reason_code: output.flow_action === "cancel_flow"
        ...
        : "prepare_attack_card_local_deferred",
      local_state: null,
      draft: null,
...
export function dispatcherSystemPrompt(): string {
  return [
    ...
    "Quand le user demande ce qui a ete cree, enregistre, sauvegarde, ou ce qui est seulement prepare a recopier, c'est une question d'etat/ledger read-only sur la frontiere chat-plateforme. Ce n'est pas une tentative de creation.",
    ...
    "- flow_action: decision principale du tour courant. Choisis-la depuis le message actuel + l'etat actif, pas seulement depuis l'etat precedent. Utilise continue_local/missing_info/confirm_candidate/answer_current_field/confirm_proposed_field/choose_technique/confirm_technique_proposal pour avancer localement; revise_current_field/revise_technique/revise_target/revise pour corriger; repeat_handoff pour repeter les valeurs pretes; platform_destination_followup ou destination_followup pour une question courte de destination; apply_attempt si le user demande vraiment creer/lancer/ajouter maintenant depuis le chat; get_info_db si le user demande ce qui a ete cree/enregistre/sauvegarde ou ce qui existe deja; exit_to_global_dispatcher si le user arrete ce flow ou si un nouveau sujet clair doit etre reanalyse par global; handoff_to_local_flow si le user bascule explicitement vers un autre flow local autorise; safety_preempt pour safety. Erreur a eviter: continuer le flow carte quand le user demande clairement autre chose.",
    ...
    "- visible_task.conversation_context: seul contexte que l'agent visible peut utiliser. Filtre-le: state_summary, user_words, field_or_stage, known_values, missing_or_weak_values, selected_candidate, handoff_data, tone_constraints, do_not_say, context_summary, evidence_used. Pour ask_or_confirm_technique, selected_candidate doit inclure candidate_options, allowed_technique_labels et recommended_technique si disponible; ce sont les seules techniques que le prompt visible peut mentionner. Pas de DB brute, pas de memoire brute, pas de note_information brute. Pendant ask_target/confirm_target_candidate/ask_blocker/ask_or_confirm_technique/ask_platform_field/confirm_platform_field_proposal, handoff_data.platform_steps doit rester vide et ne doit pas encourager a aller dans la plateforme; reserve platform_steps aux stages handoff_ready, repeat_handoff, destination_short et apply_attempt. Inclure contraintes user et limites de ton dans tone_constraints/do_not_say.",
    "- subskill_call: inline product/status temporaire, pas changement definitif de flow. needed=true seulement pour get_info_product/get_info_db/inline_product/inline_status. skill_id=product_help pour question produit; status_recap pour objets existants/DB et questions d'etat sur creation/enregistrement/sauvegarde de la carte en cours. context_for_subskill doit contenir active_flow='prepare_attack_card', question_to_answer, active_flow_context compact (target_state, blocker_state, technique_state, current_field_id, flow_kind si pertinent) et preserve_active_flow=true. Laisser needed=false, skill_id=null et context_for_subskill={} sinon. Influence: le router appelle l'inline tool puis revient au parent flow.",
    ...
    "Cas important: si le user demande ce qui a ete cree, enregistre, sauvegarde, ou ce qui est seulement une preparation a recopier apres une carte d'attaque, retourne flow_action=get_info_db, visible_task.kind=none, subskill_call.skill_id=status_recap. Reformule question_to_answer pour que status_recap explique la frontiere: le chat peut preparer et guider, mais ne doit pas affirmer une creation plateforme sans fait DB.",
    ...directEffectLocalDispatcherPromptLines(),
