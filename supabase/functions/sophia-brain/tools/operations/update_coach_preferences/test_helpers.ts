...
export function structuredCoachPreferencesSlotFiller(
  ...
  return async () => ({
    user_intent: String(statePatch.user_intent ?? "") === "preview_only"
      ...
      ? "status_question"
      : String(statePatch.user_intent ?? "") === "apply_attempt"
      ? "apply_attempt"
      : String(statePatch.user_intent ?? "") === "repeat_handoff"
      ? "repeat_handoff"
      : String(statePatch.user_intent ?? "") === "punctual_instruction"
      ? "punctual_instruction"
      : String(statePatch.user_intent ?? "") === "unsupported_preference"
