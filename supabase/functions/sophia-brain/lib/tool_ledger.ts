...
function stableStringify(x: any): string {
  try {
    ...
    const body = keys.map((k) =>
      `${JSON.stringify(k)}:${stableStringify((x as any)[k])}`
    ).join(",");
    return `{${body}}`;
  } catch {
    try {
...
function summarizeToolResult(toolName: string, result: any): any {
  ...
  if (typeof result === "string") {
    return {
      ...
      text_preview: result.slice(0, 240),
    };
  }
  ...
  const ids: any = {};
  for (
    const k of ["id", "action_id", "plan_id", "submission_id", "framework_id"]
  ) {
...
export async function logToolLedgerEvent(opts: {
  ...
  const argsPreview = toolName
    ? summarizeToolArgs(toolName, opts.toolArgs)
    : null;
  const argsHash = toolName
    ? await sha256Hex(stableStringify(opts.toolArgs))
    : null;
  const resultPreview = toolName
    ? summarizeToolResult(toolName, opts.toolResult)
    : null;
  const resultHash = toolName
    ? await sha256Hex(stableStringify(opts.toolResult))
    : null;
  ...
  try {
    ...
    if (!g.__sophia_tool_ledger_dedup) {
      g.__sophia_tool_ledger_dedup = {
        set: new Set<string>(),
        order: [] as string[],
      };
    }
  ...
  const argsFull = toolName
    ? capJsonValue(opts.toolArgs ?? null, maxBytes)
    : null;
  const resultFull = toolName
    ? capJsonValue(opts.toolResult ?? null, maxBytes)
    : null;
  ...
  try {
    await (opts.supabase as any).rpc("log_conversation_event", {
      ...
      p_payload: safeJsonValue({
        ...
        error: errFull,
        latency_ms: Number.isFinite(Number(opts.latencyMs))
          ? Math.max(0, Math.floor(Number(opts.latencyMs)))
          : null,
