...
export interface GlobalMemory {
  id: string;
  user_id: string;
  scope?: "cycle" | "transformation" | "relational" | null;
...
const GLOBAL_MEMORY_COMPACTION_PENDING_COUNT = Number(
  (safeEnvGet("SOPHIA_GLOBAL_MEMORY_PENDING_COUNT") ?? "5").trim(),
) || 5;
const GLOBAL_MEMORY_COMPACTION_PENDING_CHARS = Number(
  (safeEnvGet("SOPHIA_GLOBAL_MEMORY_PENDING_CHARS") ?? "1200").trim(),
) || 1200;
const GLOBAL_MEMORY_COMPACTION_SUMMARY_CHARS = Number(
  (safeEnvGet("SOPHIA_GLOBAL_MEMORY_SUMMARY_CHARS") ?? "1800").trim(),
) || 1800;
const GLOBAL_MEMORY_EMBED_THRESHOLD = Number(
  (safeEnvGet("SOPHIA_GLOBAL_MEMORY_MATCH_THRESHOLD") ?? "0.42").trim(),
) || 0.42;
const GLOBAL_MEMORY_COMPACTION_MODEL =
  (safeEnvGet("SOPHIA_GLOBAL_MEMORY_COMPACTION_MODEL") ??
...
function normalizeGlobalMemoryScope(
  scope?: MemoryLayerScope | null,
): "cycle" | "transformation" | "relational" {
  if (scope === "cycle") return "cycle";
  if (scope === "relational") return "relational";
...
async function tryLogMemoryPersisted(params: {
  ...
  try {
    ...
    console.warn("[GlobalMemory] memory_persisted_v2 log failed", {
      user_id: params.userId,
      memory_id: params.memoryId,
...
export function buildGlobalMemorySemanticSnapshot(
  ...
  const sections = [
    ...
    memory.canonical_summary
      ? `Résumé consolidé: ${compactText(memory.canonical_summary, 600)}`
      : "",
    ...
    memory.active_issues.length > 0
      ? `Chantiers actifs: ${toSentenceList(memory.active_issues.slice(0, 5))}`
      : "",
...
export function shouldCompactGlobalMemory(
  memory: Pick<
    GlobalMemory,
    "pending_count" | "pending_chars" | "canonical_summary" | "needs_compaction"
  >,
): boolean {
  return Boolean(
...
> {
  ...
  const scope = normalizeGlobalMemoryScope(opts.scope);
  const hasScopedRuntime = scope === "relational" ||
    (scope === "cycle" && Boolean(opts.cycleId)) ||
    (scope === "transformation" && Boolean(opts.transformationId));
  ...
  let existingQuery = supabase
    ...
    .eq("status", "active");
  if (scope === "relational") {
    existingQuery = existingQuery.eq("scope", "relational");
  } else if (scope === "cycle" && opts.cycleId) {
    existingQuery = existingQuery.eq("scope", "cycle").eq(
      "cycle_id",
      opts.cycleId,
    );
  } else if (scope === "transformation" && opts.transformationId) {
    existingQuery = existingQuery.eq("scope", "transformation").eq(
      "transformation_id",
      opts.transformationId,
    );
  }
  const { data: existingRow, error: existingErr } = await existingQuery
    .maybeSingle();
  ...
  if (!existingRow) {
    ...
    const { error: insertErr } = await supabase
      ...
      .insert({
        user_id: userId,
        scope,
        cycle_id: hasScopedRuntime ? (opts.cycleId ?? null) : null,
        transformation_id: hasScopedRuntime
          ? (opts.transformationId ?? null)
          : null,
        theme: candidate.theme,
        subtheme_key: candidate.subtheme_key,
        full_key: candidate.full_key,
    ...
    await tryLogMemoryPersisted({
      ...
      transformationId: opts.transformationId ?? null,
      layer: scope,
      action: "create",
      memoryType: "global",
      memoryId: null,
...
export async function retrieveGlobalMemories(params: {
  ...
  const allowedScopes = (Array.isArray(params.scope) ? params.scope : [])
    .filter((scope) =>
      scope === "cycle" || scope === "transformation" || scope === "relational"
    ) as Array<"cycle" | "transformation" | "relational">;
  let query = params.supabase
    .from("user_global_memories")
    .select(GLOBAL_MEMORY_SELECT_COLUMNS)
  ...
  if (allowedScopes.length > 0) {
    const scopeClauses = allowedScopes.map((scope) => {
      if (scope === "cycle" && params.cycleId) {
        return `and(scope.eq.cycle,cycle_id.eq.${params.cycleId})`;
      }
      if (scope === "transformation" && params.transformationId) {
        return `and(scope.eq.transformation,transformation_id.eq.${params.transformationId})`;
      }
      return `scope.eq.${scope}`;
    });
...
export async function retrieveGlobalMemoriesByThemes(params: {
  ...
  const maxResults = Math.max(
    ...
    Math.min(
      12,
      Math.floor(params.maxResults ?? Math.max(4, normalized.length * 3)),
    ),
  );
