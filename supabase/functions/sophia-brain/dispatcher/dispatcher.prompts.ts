...
export const DISPATCHER_V2_PROMPT_VERSION = "dispatcher_v2_prompt_2026_05_s14";
...
- safety_risk_band: risque deja detecte; c'est un plancher. Tu peux l'elever, jamais l'abaisser.
	- active_skill_state: skill conversationnel deja actif; continue-le si le user reste dans le meme besoin, mais laisse passer une intention plus prioritaire comme safety, confirmation ou tool skill clair. Cas weekly_adaptive_review_v1: continue le weekly par defaut, sauf demande explicite de modification d'organisation a router vers adjust_plan_item; apres cette parenthese le weekly doit pouvoir reprendre.
...
- Exception adjust_plan_item: si le user parle d'un element structurel du plan (rituel, bloc, niveau, semaine, programme, charge du soir, groupe d'actions) et demande ou implique clairement de le rendre plus leger/simple/tenable, c'est une intention adjust_plan_item meme sans le mot "plan".
...
- Si le user dit qu'un rituel/bloc/niveau du plan le fatigue, contient trop de choix/preparation, ou le fait lacher, puis demande de l'alleger/simplifier/rendre tenable, route vers adjust_plan_item avec adjust_plan_scope=current_level. Ne route pas vers une carte sauf demande explicite de carte.
