...
async function getLastAssistantMessage(admin, userId) {
  const { data } = await admin.from("chat_messages").select("content, created_at").eq("user_id", userId).eq("scope", "whatsapp").eq("role", "assistant").order("created_at", {
    ascending: false
  }).limit(1).maybeSingle();
...
async function countRecentAssistantPurpose(params) {
  ...
  const { count } = await params.admin.from("chat_messages").select("id", {
    count: "exact",
    head: true
  }).eq("user_id", params.userId).eq("scope", "whatsapp").eq("role", "assistant").gte("created_at", sinceIso).filter("metadata->>purpose", "eq", params.purpose);
...
export async function handleOnboardingState(params) {
  const { admin, userId, whatsappState, fromE164, requestId, waMessageId, text, siteUrl, replyWithBrain, sendWhatsAppText } = params;
  ...
  if (st === "awaiting_plan_finalization") {
    ...
    if (!saysDoneFast && raw) {
      ...
      const isPlan = Boolean(signals?.topic_depth?.plan_focus) || signals?.create_action?.intent_strength === "explicit" || signals?.create_action?.intent_strength === "implicit" || signals?.update_action?.detected === true;
      if (isAck && ack >= 0.7) {
        decision = "done";
      ...
      const isSafetySignal = signals?.safety?.level && signals.safety.level !== "NONE" || signals?.topic_depth?.value === "NEED_SUPPORT" && (signals.topic_depth?.confidence ?? 0) >= 0.7;
      if (isSafetySignal) {
  ...
  if (st === "awaiting_plan_finalization_support") {
    ...
    if (planTitle) {
      ...
      const { data: obCheck } = await admin.from("profiles").select("onboarding_completed").eq("id", userId).maybeSingle();
