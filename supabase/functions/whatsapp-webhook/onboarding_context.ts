export function buildWhatsAppOnboardingContext(params) {
  ...
  const planLine = params.planPolicy === "no_plan" ? "PLAN: aucun plan actif détecté. Ne prétends pas voir un plan. Ne propose pas de créer/modifier un plan complet sur WhatsApp." : params.planPolicy === "plan_active" ? "PLAN: un plan actif est détecté. Tu peux aider à exécuter la prochaine action (petit pas), pas à refondre le plan complet." : "PLAN: statut incertain.";
  const phaseLine = params.phase === "support_fallback" ? "OBJECTIF: l'utilisateur est bloqué (synchro/bug). Escalade support rapidement, puis continue l'accompagnement hors-app." : "OBJECTIF: onboarding WhatsApp (court, fluide).";
