...
export async function generateWithGemini(
  ...
  const callOpenAI = async (args: { model: string; systemPrompt: string; userMessage: string; temperature: number; jsonMode: boolean; tools: any[]; toolChoice: string; requestId: string; timeoutMs: number; reasoningEffort?: "none" | "minimal" | "low" | "medium" | "high" | "xhigh" }) => {
  ...
  const effectiveTimeoutMsForModel = (m: string): number => {
    ...
    if (isEvalLikeRequest) {
      ...
      if (/\bgemini-3\.1[-.]pro(?:-preview)?\b/i.test(mm)) return Math.min(evalTimeout, 240_000);
  ...
  const pickFallbackChainForAttempt = (startModel: string, attempt: number): string[] => {
    ...
    // - Onboarding mini (gpt-5.4-mini): gpt-5.4-mini → gemini-3.0-flash-preview → gpt-5.4-mini
