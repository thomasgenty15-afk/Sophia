...
LANGUE DE SORTIE (nina-global18 batch): content_text, normalized_summary et topic_hint sont STRICTEMENT en francais — aucun token d'une autre langue ou d'un autre alphabet (une contamination « de plus en plus ხშირად » a ete persistee telle quelle). Si le user a employe un mot etranger, traduis-le ou cite-le entre guillemets.
CONTENU DE CRISE SAFETY (nina-global18 batch): ce qui est dit PENDANT un tour de crise (ideation, hotline, means-removal, messages d'effondrement aigu) n'est JAMAIS persiste en item actif — meme regle que la detresse transitoire ci-dessous, etendue a tout le contenu du pic: au mieux rejected_observation. Les faits de vie STABLES mentionnes incidemment pendant la crise (metier, allergie, date) restent memorisables s'ils sont detachables du pic emotionnel.
