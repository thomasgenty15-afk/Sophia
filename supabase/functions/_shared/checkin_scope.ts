...
export async function fetchCheckinExclusionSnapshot(params: {
  ...
  if (planActionsRes.error) throw planActionsRes.error;
  if (personalActionsRes.error) throw personalActionsRes.error;
  if (frameworksRes.error) throw frameworksRes.error;
  if (vitalSignsRes.error) throw vitalSignsRes.error;
  if (remindersRes.error) throw remindersRes.error;

  const planActionTitles = dedupeStrings(
    ((planActionsRes.data ?? []) as Array<Record<string, unknown>>).map((row) =>
      cleanText(row?.title)
    ),
  );
  const personalActionTitles = dedupeStrings(
    ((personalActionsRes.data ?? []) as Array<Record<string, unknown>>).map((
      row,
    ) => cleanText(row?.title)),
  );
  const frameworkTitles = dedupeStrings(
    ((frameworksRes.data ?? []) as Array<Record<string, unknown>>).map((row) =>
      cleanText(row?.title)
    ),
  );
  const vitalSignTitles = dedupeStrings(
    ((vitalSignsRes.data ?? []) as Array<Record<string, unknown>>).map((row) =>
      cleanText(row?.label)
    ),
  );
