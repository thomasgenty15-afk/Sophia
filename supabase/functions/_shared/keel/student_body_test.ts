...
Deno.test("CONTRE-FACTUEL — l'objectif change la semaine", () => {
  ...
  const recomp = emphasisOf("recomposition", UNKNOWN_BODY);
...
Deno.test("CONTRE-FACTUEL — la bande d'âge change la consigne, et chaque bande a la sienne", () => {
  ...
  for (const ageBand of ["18_29", "30_44", "45_59", "60_plus"] as const) {
    const e = emphasisOf("health", { ...UNKNOWN_BODY, ageBand });
    // Elle change quelque chose...
    assertNotEquals(e.emphasis, focusFor("health").emphasis, ageBand);
...
Deno.test("recomposition: c'est le TOUR DE TAILLE qui porte l'objectif", () => {
  const base = focusFor("recomposition").maxNutrition;

  // La signature: la taille descend pendant que le poids ne descend pas.
  const working = emphasisOf("recomposition", {
    ...UNKNOWN_BODY,
    weightTrend: "stable",
    waistTrend: "falling",
  });
  ...
  // Sans tour de taille, on ne peut pas le dire — et on ne le dit pas.
  const noWaist = emphasisOf("recomposition", {
    ...UNKNOWN_BODY,
    weightTrend: "stable",
  });
  assertEquals(noWaist.applied.directionWorking, false);
  assertEquals(noWaist.maxNutrition, base);
});
...
Deno.test("performance: aucune mesure ne prétend dire qu'elle progresse", () => {
  // Ni le poids ni le tour de taille ne disent qu'une performance progresse.
  // La branche existe pour dire NON, explicitement.
  for (const weightTrend of ["falling", "stable", "rising"] as const) {
    for (const waistTrend of ["falling", "stable", "rising"] as const) {
      const e = emphasisOf("performance", { ...UNKNOWN_BODY, weightTrend, waistTrend });
      assertEquals(e.applied.directionWorking, false, `${weightTrend}/${waistTrend}`);
      assertEquals(e.maxNutrition, focusFor("performance").maxNutrition);
    }
  }
});
