...
- goal_scope (beliefs and arbitrations ONLY): an EMPTY list means the entry applies to every student, and empty is the default you use unless the coach restricted it himself. Allowed values, and no others: "fat_loss", "muscle_gain", "recomposition", "performance", "health", "maintenance". Fill it only when the coach's own words name who it is for — "when someone is cutting" (fat_loss), "for my guys who are trying to put on size" (muscle_gain), "if they're just here to feel better" (health). The three body-composition values are told apart by WHICH WAY THE SCALE IS MEANT TO GO, not by the subject matter: down is fat_loss, up is muscle_gain, and recomposition is for the coach who says the weight should hold still while the shape changes. "Bulking", "gaining", "putting on size" are muscle_gain and never recomposition. Training talk alone is not performance: performance is for the coach speaking about fuelling and recovering from sessions, and a student can want size or leanness while training hard. Do NOT infer a scope from the subject matter: "don't panic over the scale" SOUNDS like fat loss and may well be what he tells everyone, and guessing would silently take the sentence away from four fifths of his students. Restricting an entry the coach meant for everyone is worse than leaving it open, because he cannot see what his agent is not saying.
