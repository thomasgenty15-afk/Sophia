...
Deno.test("AUCUN gabarit ne culpabilise — la porte 4 ne doit jamais mordre", () => {
  ...
  const tout: PlanRationaleFacts = {
    ...
    // gabarits de P2.
    cookingPlan: { sessions: 2, unusedRuns: 1, notes: ["style_caps_sessions"] },
...
Deno.test("A2 — la fenêtre courte se DIT, et ce n'est pas la même phrase", () => {
  for (const locale of ["fr", "en"] as const) {
    ...
    const days = explainPlanChoices({
      facts: {
        ...nominalFacts(),
        cookingPlan: { sessions: 1, unusedRuns: 0, notes: ["days_cap_sessions"] },
      },
      locale,
    }).lines.join(" ");
...
Deno.test("A2 — « une seule course sans congélateur » n'a QU'UNE phrase", () => {
  ...
  for (const locale of ["fr", "en"] as const) {
    const lines = explainPlanChoices({
      facts: {
        ...nominalFacts(),
        cookingPlan: { sessions: 2, unusedRuns: 0, notes: ["runs_1_needs_freezer"] },
