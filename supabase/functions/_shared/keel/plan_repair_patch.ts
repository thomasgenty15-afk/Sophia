...
export function applyRepairPatch<T extends RepairPlanShape>(args: {
  ...
  const sessionsDuPlan = args.best.cooking_sessions ?? [];
  const indexSessionDuJour = (jour: string): number => {
    const j = jour.trim();
    if (j === "") return -1;
    return sessionsDuPlan.findIndex((s) => String(s?.day ?? "").trim() === j);
  };
  for (const id of creees) {
    ...
    // ⛔ LE MODÈLE NE VOIT PAS LES JOURS DE SESSION. Mesuré campagne 2026-09-14
    // tir1-s3 : U3 (lundi soir manquant) arrive avec une casserole neuve
    // `cook_on: sun`, alors que les sessions réelles sont lun/mar. Refuser
    // « sun hors fenêtre » jetait un patch qui avait bien créé le plat manquant.
    // On rattache à une SESSION DÉJÀ LÀ : d'abord le jour déclaré, sinon le
    // jour du plat qui tire ce lot. On n'invente aucune session.
    let i = indexSessionDuJour(String(brut.cookOn ?? brut.cook_on ?? ""));
    if (i === -1) {
      for (const a of aAppliquer) {
        if (!usesOf(a.dish).includes(id)) continue;
        const unite = args.index.byId.get(a.unitId);
        i = indexSessionDuJour(String(unite?.dayToken ?? a.dish.day ?? ""));
        if (i >= 0) break;
      }
    }
    if (i === -1) {
...
export function patchDishPayloads(args: {
  ...
  for (const u of args.envelope.units) {
    ...
    // ══════════════════════════════════════════════════════════════════
    // ⟳ 2026-09-13 — UN COMPLÉMENT ET UN PLAT NE SONT PAS LA MÊME ADRESSE
    // ══════════════════════════════════════════════════════════════════
    //
    // ⛔ LE DÉFAUT, PAYÉ SUR UN APPEL RÉEL (2026-09-13). Le périmètre offre au
    // modèle DEUX unit_id à la même case pour la même bouche — c'est écrit dans
    // la projection qu'il reçoit :
    //
    //     · U5 mon/lunch (efbb3deb…) — an extra dish for efbb3deb…
    //     · U4 mon/lunch (efbb3deb…) "Tofu doré du lot commun…"
    //
    // U5 est une unité de COMPLÉMENT (réservée, `dishIndex: null`), U4 le plat
    // dédié qui existe. La clé `jour|moment|propriétaire` les confondait : le
    // modèle a rendu un patch bien formé, avec la bonne `base_version`, sur les
    // deux unités qu'on lui avait nommées — et on a jeté la seconde en
    // `ambiguous_address`. Notre contrat, pas sa faute.
    //
    // ⚠️ CE QUE LA GARDE PROTÉGEAIT RESTE PROTÉGÉ : deux plats dédiés SERVIS au
    // même créneau (le défaut ② de la revue du 2026-09-12) gardent la même clé
    // et se refusent toujours. `isComplement` est la seule chose qui change,
    // et c'est précisément ce que la table des unités distingue déjà.
    const cle = `${unite.dayToken}|${unite.slot}|${unite.ownerId ?? ""}|${
      unite.isComplement ? "extra" : "plate"
    }`;
