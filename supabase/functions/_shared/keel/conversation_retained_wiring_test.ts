...
Deno.test("ET L'ASSERTION MORD — quatre mutations en mémoire, quatre rouges", async () => {
...
Deno.test("⛔ LE TROU MESURÉ — le SIGNAL `plan_feedback` N'A AUCUN PRODUCTEUR", async () => {
  // ═════════════════════════════════════════════════════════════════════════
  // CE TEST PINGLE UN DÉFAUT QUI N'EST PAS DE CE LOT, ET QUI LE REND INERTE.
  //
  // Le câblage ci-dessus est complet et prouvé. Il est branché sur
  // `dispatcherSignals.plan_feedback`, comme la consigne l'exige (⛔ aucun
  // matcher maison). MAIS ce signal n'est JAMAIS produit:
  //
  //   · `DEFAULT_SIGNALS.plan_feedback` vaut `{ detected: false }`;
  //   · `dispatcherSignalsFromTurnFrame` ne surcharge que `safety`,
  //     `interrupt`, `risk_score`, `needs_research` et
  //     `track_progress_plan_item` — jamais `plan_feedback`;
  //   · le contrat `TurnFrame` ne porte AUCUN champ `plan_feedback`, et
  //     `DispatcherSkillSignals` ne déclare que `plan_question`;
  //   · le prompt du dispatcher dit en toutes lettres « Ne produis jamais de
  //     skill signal hors plan_question ».
  //
  // Conséquence: `handlePlanItemFeedback` prend TOUJOURS sa branche `!detected`
  // et `__plan_feedback_addon` n'est jamais écrit — le lecteur de
  // `context/loader.ts` est mort lui aussi.
  //
  // ⚠️ C'EST DONC UNE « CEINTURE ARMÉE SUR UN COFFRE VIDE », et on l'écrit
  // plutôt que de la masquer. Le renvoi est correct par construction et sortira
  // le jour où le signal existera — la contre-épreuve est dans
  // `conversation_retained_test.ts`, où le même code, nourri d'un signal réel,
  // rend bien les deux phrases.
  //
  // ⚠️ LE JOUR OÙ CE TEST ROUGIT, C'EST UNE BONNE NOUVELLE: quelqu'un a câblé
  // le producteur. Il faut alors REMESURER le renvoi de bout en bout (les deux
  // langues, sur la lane `plan_question` et sur la lane du composeur), puis
  // SUPPRIMER ce test. Ne pas le « réparer » en assouplissant l'assertion.
  // ═════════════════════════════════════════════════════════════════════════
  const runtime = await Deno.readTextFile(
    new URL("../../sophia-brain/router/turn_context_runtime.ts", import.meta.url),
  );
  const mapper = stripComments(runtime).slice(
    stripComments(runtime).indexOf("export function dispatcherSignalsFromTurnFrame("),
  ).slice(0, 900);
  assert(
    !mapper.includes("plan_feedback"),
    "LE PRODUCTEUR DU SIGNAL `plan_feedback` EXISTE MAINTENANT. Le renvoi du " +
      "sizing (lot 2C) devient ATTEIGNABLE: remesure-le de bout en bout, dans " +
      "les DEUX langues, sur la lane `plan_question` ET sur celle du composeur, " +
      "puis supprime ce test.",
  );
  ...
  assert(
    !stripComments(frame).includes("plan_feedback"),
    "LE CONTRAT `TurnFrame` PORTE MAINTENANT `plan_feedback`. Même consigne " +
      "que ci-dessus: remesure, puis supprime ce test.",
  );
});
