...
export type PlanAdjustmentScope =
  | "current_level_only"
  | "future_levels_only"
  | "current_plus_future"
  | "full_plan";

export type PlanAdjustmentGlobalReasoning = {
  main_problem_model: string;
  sequencing_logic: string;
  why_not_faster_initially: string;
  acceleration_signals: string[];
  slowdown_signals: string[];
};

export type PlanAdjustmentPhaseReasoning = {
  phase_id: string;
  phase_order: number;
  role_in_plan: string;
  why_before_next: string;
  user_signals_used: string[];
  prerequisite_for_next_phase: string | null;
  acceleration_signals: string[];
  slowdown_signals: string[];
};

export type PlanAdjustmentContext = {
  global_reasoning: PlanAdjustmentGlobalReasoning;
  phase_reasoning: PlanAdjustmentPhaseReasoning[];
};
