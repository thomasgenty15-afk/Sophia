  ...
  try {
    ...
    for (const profile of (profiles ?? []) as Array<Record<string, unknown>>) {
      ...
      if (
        ...
        const selectedSchedule = hasActionsToday
          ? todaySchedule
          : yesterdaySchedule;
          ...
          .upsert(
            {
              ...
              message_payload: {
                ...
                reviewed_local_date: !hasActionsToday &&
                    hasOpenActionsFromYesterday
        ...
        if (upsertErr) {
          ...
          } else if (hasOpenActionsFromYesterday) {
