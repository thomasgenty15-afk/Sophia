...
import {
  ...
  SUPPORT_CARD_SYSTEM_PROMPT,
  validateSupportCardOutput,
} from "../_shared/v2-prompts/lab-surfaces.ts";
import { getRequestContext } from "../_shared/request_context.ts";
import { loadLabScopeContext } from "../_shared/v2-lab-context.ts";
import type { LabScopeKind, SupportCardContent } from "../_shared/v2-types.ts";
...
class GenerateSupportCardError extends Error {
  status: number;

  constructor(status: number, message: string, options?: { cause?: unknown }) {
    super(message, options);
    this.name = "GenerateSupportCardError";
    this.status = status;
  }
}

export async function generateSupportCardForTransformation(args: {
...
}): Promise<{ card_id: string; content: SupportCardContent }> {
  ...
  if (existing && !args.forceRegenerate) {
    return {
      ...
      content: existing.content as SupportCardContent,
    };
  }
  ...
  const raw = await generateWithGemini(
    ...
    "auto",
    {
      ...
      source: "generate-support-card-v1",
      userId: args.userId,
  ...
  if (typeof raw !== "string") {
    throw new GenerateSupportCardError(500, "LLM returned tool call instead of JSON");
  }
  ...
  try {
    ...
    throw new GenerateSupportCardError(500, "LLM returned invalid JSON", { cause: error });
  }

  const validation = validateSupportCardOutput(parsed);
  if (!validation.valid || !validation.content) {
    throw new GenerateSupportCardError(
      500,
  ...
  if (existing?.id) {
    ...
    if (error) {
      throw new GenerateSupportCardError(500, `Update failed: ${error.message}`, {
        cause: error,
      });
    }
  ...
  if (error) {
    throw new GenerateSupportCardError(500, `Insert failed: ${error.message}`, {
      cause: error,
    });
  }
...
async function handleRequest(req: Request): Promise<Response> {
  ...
  try {
    ...
    if (scopeKind === "transformation" && !parsed.data.transformation_id) {
      throw new GenerateSupportCardError(400, "transformation_id is required");
    }

    const result = await generateSupportCardForTransformation({
      admin,
    ...
    await logEdgeFunctionError({
      functionName: "generate-support-card-v1",
      error,
      requestId,
      userId: ctx.userId,
      source: "edge",
      metadata: { route: "generate-support-card-v1" },
    });

    if (error instanceof GenerateSupportCardError) {
    ...
    return serverError(req, requestId, "Failed to generate support card");
  }
}
...
function getSupabaseEnv(): {
  ...
  if (!url || !anonKey || !serviceRoleKey) {
    throw new GenerateSupportCardError(500, "Supabase environment variables are not configured");
  }
