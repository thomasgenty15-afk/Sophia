-- Schedule daily Memorizer at 00:00.
-- Memorizer handles topic memory updates from daily conversations.
...
declare
  existing_jobid int;
begin
  select jobid into existing_jobid from cron.job where jobname = 'trigger-memorizer-daily' limit 1;
...
select cron.schedule(
  ...
  select
    net.http_post(
      url := (select base_url from cfg) || '/functions/v1/trigger-memorizer-daily',
