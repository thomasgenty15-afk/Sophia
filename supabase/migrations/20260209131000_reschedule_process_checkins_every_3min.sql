-- Reschedule `process-checkins` to run every 3 minutes instead of every 30.
--
-- Goal:
-- - Deliver scheduled checkins (focus follow-ups, bilan reschedules, event
--   reminders) with at most ~3 minutes of latency instead of ~30.
-- - When there are no pending rows the function returns in ~50ms, so the extra
--   invocations are negligible cost-wise.
--
-- Security:
