-- Weekly proactive job claim (Sunday 20:00 local), timezone-aware.
...
with cfg as (
  select
    greatest(1, least(coalesce(p_batch, 120), 500))::int as batch,
...
due as (
  ...
  left join public.proactive_job_state s
    on s.user_id = c.user_id and s.job = 'weekly_bilan'
  cross join cfg
  where c.local_dow = 0 -- Sunday in Postgres extract(dow): Sunday=0
...
claimed as (
  ...
  select d.user_id, 'weekly_bilan', d.local_date, now(), 1
  from due d
