-- Remove historical runner events from the production admin log surface.
-- Historical trace tables are left intact; this only stops exposing them via
-- get_production_log.
...
begin
  return query
  select *
  from public.get_production_log(p_since, p_limit, p_only_errors, p_source, false);
end;
$$;
