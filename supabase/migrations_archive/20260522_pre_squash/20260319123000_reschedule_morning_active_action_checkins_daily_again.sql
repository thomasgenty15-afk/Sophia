-- Return morning active-action scheduling to a daily top-up.
-- The content is now generated as close as possible to send time, so the cron
-- should only provision the next local morning slot.
