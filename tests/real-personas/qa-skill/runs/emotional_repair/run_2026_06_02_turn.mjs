...
const skill = "emotional_repair";
const date = process.env.QA_DATE || "2026-06-02";
...
const scope = `qa-emotional-repair-${date}-${runId}`;
const requestId = `qa-emotional-repair-${date}-${runId}-t${
  ...
  JSON.stringify(
    {
      ...
      memory_candidates:
        Array.isArray(trace?.memory_write_candidates_emitted)
          ? trace.memory_write_candidates_emitted.length
          : null,
