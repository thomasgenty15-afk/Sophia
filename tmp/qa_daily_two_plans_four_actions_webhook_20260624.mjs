...
const runId = process.env.QA_RUN_ID ||
  `daily-two-plans-four-actions-webhook-20260624-${Date.now().toString(36)}`;
...
async function setup(before) {
  ...
  const commonPlan = {
    ...
    content: { qa_run_id: runId, source: "qa_daily_two_plans_four_actions_webhook" },
  };
...
async function createPending() {
  ...
  const pending = await insert("whatsapp_pending_actions", {
    ...
    payload: {
      ...
      grouping_reason: "qa_two_plans_four_actions_webhook",
      local_date: localDate,
      week_start_date: weekStartDate,
      timezone: "Europe/Paris",
      qa_run_id: runId,
...
async function main() {
  ...
  try {
    ...
    const plannedTurns = [
      ...
      {
        label: "t2_action1_relevance",
        text: "Oui, la journée sans fumer reste pertinente pour demain.",
      },
      {
        label: "t3_action2_missed",
        text:
          "La respiration cinq minutes, pas faite non plus : j’ai oublié quand l’envie est montée. Oui, c’est à garder.",
      },
      {
        label: "t4_action3_missed",
        text:
