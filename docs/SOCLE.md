...
⟳ 2026-09-15 — **LA BORNE D'ARRONDI EST CALCULÉE, JAMAIS DÉCRÉTÉE.** `boxNutrition` (`_shared/keel/mouth_energy.ts`) rend, par contenant, ce que l'écriture des items en grammes **entiers** peut déplacer : `proteinRoundingG` = Σ ½ g × protéine au gramme de la casserole de chaque item (+ la part de frais au prorata, + 0,05 g d'affichage), `densityRoundingPer100G` = ½ × Σ|d_i − d̄| / G (`densityRoundingOf`, zéro pour un contenant d'un seul terme). L'audit les porte par case et par jour (`CellNutritionRow`, `DayNutritionRow`), la garde les lit : un plancher est tenu quand `proteinG + proteinRoundingG ≥ plancher` (compté dans `checked.protein_within_rounding`), une case est dans le couloir quand `densité ± borne` y entre (`cellStateOf`, sur les bornes **exactes**). ⛔ **Ce que la borne vaut, mesuré hors ligne sur les 9 plans à écarts des 30 tirs** (`scratchpad/2026-09-15-BETA-PREUVES/borne-hors-ligne.ts`) : **0,11 à 0,44 g** de protéine par jour contre des manques de **1,3 à 5,0 g** ; **≤ 0,11 kcal/100 g** contre des couloirs manqués de **0,7 à 1 point**. Elle ne ferme **aucun** des dix jours protéiques et **aucune** des six cases à coup sûr (trois impossibles, trois indécidables à l'entier du couloir affiché). Les écarts « d'arrondi » du bilan des 30 n'en étaient pas : c'est la recette, et c'est une décision de tolérance (D3) qui reste au propriétaire — pas un seuil à abaisser.

⟳ 2026-09-15 — **LE PALIER D'UNITÉ** (`bumpCountableToReadyMass`, `portion_scaling.ts`). Quand `growIngredientsToReadyMass` n'a déplacé aucun dénombrable après huit pesées (« 2 œufs » × 1,2 s'arrondit à 2), il ajoute **une unité** (une demi-cuillère pour les cuillères) à la ligne dénombrable qui couvre le manque en dépassant le moins, au plus `MAX_UNIT_BUMPS = 3` fois, prose réécrite par le même `rewriteCountableQuantity` (« 1 tin » → 2 reste refusé : pluriel inconnu). La casserole rendue est **mesurée ≥ prélevé**, jamais tolérée. Compté dans `potReconcile.unit_bumped`. Motif : tirs 6 et 13 des 30, plans entiers refusés (422) pour 25 g et 11 g de casserole.

⟳ 2026-09-15 — **LE REFUS DE MASSE EST UN CONSTAT BLOQUANT.** Le constat de `defectsFromQuantities` porte la cause `preparation_quantity_unreconciled` (`POT_MASS_UNRECONCILED_CAUSE`, le mot exact du 422 du handler) et entre dans `collectPlanDefects().blocking` ; il reste `repairable: false`, donc `planRepairDecision` rend `nothing_repairable` — la vérité — là où le journal disait `no_blocking_defect` sur un plan refusé la ligne d'après.
