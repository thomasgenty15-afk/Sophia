# Bug Sheet - prepare_defense_card Local Doctrine R2 Invalid

Cadre: Supabase local, `/functions/v1/test-send-message`, `force_full_ai=true`, persona `alex`, tentatives `prepare-defense-local-doctrine-r2` et `prepare-defense-local-doctrine-r2b`. Aucun fallback utilise. Cleanup cible effectue.
...
- Preuve systeme: `status=502`, `response_owner=null`, `selected_handler=null`, `route_reason=null`; edge runtime observe `Up About a minute` apres les tentatives; logs sans stack exploitable; `deno check` cible passe.
