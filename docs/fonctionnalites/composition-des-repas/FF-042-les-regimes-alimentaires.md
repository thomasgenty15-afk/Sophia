# FF-042 · Les régimes alimentaires — un verrou, pas une préférence
...
### Livré (2026-08-11) — R6, le signalement structurel
...
- `generate-meal-v1` — le régime est dérivé des contraintes **déjà chargées**
  (aucune seconde requête) et passé au verdict.
- **11 tests neufs** : la boucle gardée aux deux niveaux (verdict et
  `correctionPlanFor`), la survie à l'abstention, le désarmement omnivore, le
  nom de colonne inconnu écarté, la jointure régime→verdict pour les trois
  régimes, l'existence réelle des colonnes dans la migration, et l'absence de
  tout mot de supplémentation dans le verdict comme dans les phrases servies au
  modèle.
...
1. ~~`safety_constraints.ts` — `dietRef` dans le type, le `select`, le
...
### R6 — La B12 se signale, ne se prescrit pas — **livré le 2026-08-11**
...
#### Deux canaux, et c'est la séparation qui est la garde

`CompositionVerdict.sentinels` a **deux** champs, et un seul est lu par la
boucle de correction ([FF-040](FF-040-la-boucle-de-correction.md)) :
...
Sans cette séparation, le trou B12 d'un végan produirait une boucle infinie
**facturée** : la correction placerait une recette censée l'apporter, la
génération suivante ne la trouverait pas davantage, et le retry se
redéclencherait à chaque plan, pour toujours. `verdictFor` retire donc le
drapeau incouvrable **avant** de construire `missing`, jamais après — un filtre
posé en aval serait un filtre qu'un futur appelant contournerait.

`uncoverableSentinels` est un **paramètre requis** de `verdictFor` : la casse
de compilation est ce qui recense les appelants. Le désarmement est testé par
égalité — un omnivore garde son trou B12 dans `missing`, exactement comme
avant ce lot.

#### Le canal coach : ce qu'il n'est pas encore, et la règle qui l'attend

**Aucun lecteur n'existe aujourd'hui.** `meal_composition_verdicts` a un
écrivain (`generate-meal-v1`) et une réclamation d'export ; rien ne la relit.
Le drapeau est donc écrit et invisible, ce qui est l'état correct tant que
l'agrégat n'est pas construit — ce dépôt a une cicatrice nommée pour l'inverse.

Quand ce lecteur sera écrit, la requête est :

```sql
select count(distinct v.user_id)
from public.meal_composition_verdicts v
where v.verdict->'sentinels'->'uncoverable' ? 'b12_source'
```

et elle est soumise à l'arbitrage A4 (k=5) sous une forme **plus stricte** que
les autres agrégats. « Combien de mes élèves ont un trou B12 » se lit « combien
sont végans » : la cellule comptée est elle-même l'attribut. Le plancher doit
donc porter sur **les deux cellules**, pas sur la cohorte —
`aggregatePairMayShip(marqués, cohorte − marqués)` et non
`aggregateMayShip(cohorte)`. Une cohorte de vingt dont un seul est végan passe
le second et échoue le premier ; c'est le premier qui a raison, parce que
« un élève sur vingt » est nominatif dans une cohorte que le coach connaît.

**Jamais nominatif** vaut aussi dans l'autre sens : le drapeau ne remonte pas
au coach par le canal individuel. Il n'y a pas de canal individuel.
...
- [x] `dietRef` chargé par le loader, **absent** de `safetyConstraintTokens()`
- [x] Végan → `b12_source` sort des trous réparables, **aucun jeton de
      correction déclenché dessus** (le test qui garde la boucle infinie)
- [x] Végétarien → **aucun** drapeau structurel, et son trou d'œufs reste
      réparable comme celui de n'importe qui
- [x] Omnivore sans B12 → trou **réparable**, recette placée (désarmement)
- [x] Aucune sortie — verdict, phrase de correction, consigne — ne contient de
      conseil de supplémentation
- [x] Le drapeau ne remonte à aucun canal coach : **il n'y en a pas encore**,
      et la règle k=5 renforcée qui l'attend est écrite (§6 R6)
