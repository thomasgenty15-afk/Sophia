...
| ~~3~~ | ✅ **REFERMÉ (chantier 4, D1).** Une personne sans compte en crée un depuis `/join-household`, et **la porte exige le pays** : sans lui, `handle_new_user()` refuse le compte et `keel_household_join` refuse `country_required`. `/auth` **n'a pas rouvert** son inscription élève. Le palier posé est `household_member`, jamais `student`. | migration `20260811060000` · `keel/pages/JoinHouseholdPage.tsx` · `keel/api/householdSignup.ts` | [FF-048](FF-048-reclamer-son-profil.md) §7 |
