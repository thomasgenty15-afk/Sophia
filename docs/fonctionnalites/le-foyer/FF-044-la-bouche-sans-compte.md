...
| R6 | **L'objectif d'un membre vient de la ligne, plus jamais de `student_goals`** | `student_goals` exige un compte : la bifurcation des portions était donc muette pour exactement les gens qu'on veut ajouter. `student_goals` reste lu **pour le compte maître seul** — sa situation, ses contraintes pratiques et sa langue gouvernent la composition entière. |
