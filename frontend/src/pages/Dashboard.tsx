...
const WeekCard = ({ week }: { week: any }) => {
  ...
  return (
    <div 
      onClick={handleClick}
      className={`relative rounded-2xl p-6 border transition-all duration-500 group cursor-pointer h-[180px] flex flex-col justify-center snap-center shrink-0 w-full md:w-[90%] mx-auto ${
      isCurrent 
      ...
      <div className="flex items-center justify-between relative z-10 px-4">
        ...
        <div className={`w-10 h-10 md:w-14 md:h-14 rounded-full flex items-center justify-center flex-shrink-0 transition-all duration-300 ml-4 hidden min-[300px]:flex ${
          isCurrent 
...
const PlanActionCard = ({ action, isLocked, onHelp }: { action: Action, isLocked: boolean, onHelp: (action: Action) => void }) => {
  ...
  return (
    <div className={`relative bg-white border rounded-xl p-4 shadow-sm transition-all duration-300 group ${
      isLocked ? 'opacity-60 grayscale border-gray-100' : 
      ...
      <div className="flex items-start gap-4 mt-2">
        {/* Icône Type */}
        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 hidden min-[350px]:flex ${
              ...
              {isGroupA ? (
                ...
                <div 
                  ...
                  className={`cursor-pointer flex items-center justify-between p-2 rounded-lg border transition-all ${
                    isChecked ? 'bg-amber-50 border-amber-200' : 'bg-gray-50 border-gray-200 hover:border-amber-200'
                  }`}
                >
                  ...
                  <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${
                    isChecked ? 'bg-amber-500 border-amber-500 text-white' : 'bg-white border-gray-300'
                  }`}>
...
const PlanPhaseBlock = ({ phase, isLast, onHelpAction }: { phase: PlanPhase, isLast: boolean, onHelpAction: (action: Action) => void }) => {
  ...
  return (
    <div className="relative pl-0 min-[350px]:pl-4 md:pl-8 pb-10 last:pb-0">
      {/* Ligne Verticale */}
      {!isLast && (
        <div className={`hidden min-[350px]:block absolute left-[11px] top-8 bottom-0 w-0.5 ${
          isActive ? 'bg-emerald-200' : 'bg-gray-100'
        }`} />
      )}
      
      {/* Puce Timeline */}
      <div className={`hidden min-[350px]:flex absolute left-0 top-1 w-6 h-6 rounded-full border-4 items-center justify-center z-10 ${
        phase.status === 'completed' ? 'bg-emerald-500 border-emerald-100' :
      ...
      <div className="mb-6 mt-1">
        <h3 className={`text-sm font-bold uppercase tracking-wide ${
          isActive ? 'text-emerald-700' : isLocked ? 'text-gray-400' : 'text-emerald-900'
        }`}>
          {phase.title}
        </h3>
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex flex-col min-[270px]:flex-row bg-gray-100/10 p-1 rounded-[2rem] min-[270px]:rounded-full border border-gray-200/20 gap-1 min-[270px]:gap-0">
            <button 
              ...
              className={`px-4 py-2 rounded-full text-xs font-bold uppercase tracking-wide transition-all flex items-center justify-center gap-2 ${
                !isArchitectMode 
                ...
                <>
                  ...
                  <div className="mt-8 md:mt-12 pt-8 md:pt-12 border-t border-emerald-800/50 pb-20">
                    ...
                    <div className="grid md:grid-cols-2 gap-6">
                      <div className="bg-gradient-to-br from-emerald-900 to-emerald-950 border border-emerald-800 p-6 md:p-8 rounded-2xl relative overflow-hidden opacity-60 cursor-not-allowed">
                        <div className="hidden min-[350px]:block absolute top-0 right-0 p-4 opacity-10">
                  ...
                  {(() => {
                    ...
                    return (
                      <div 
                        ...
                        className={`relative w-full h-48 md:h-56 rounded-3xl border flex items-center overflow-hidden transition-all duration-500 ${
                          isTableRondeDone 
                        ...
                        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between w-full px-8 md:px-12 gap-6">
                          ...
                          <div className="flex items-center gap-6 text-center md:text-left flex-1">
                            <div className={`w-16 h-16 rounded-2xl flex items-center justify-center shadow-inner transition-colors ${
                              isTableRondeDone ? 'bg-emerald-900/20 text-emerald-700' : 'bg-blue-500/20 text-blue-400 group-hover:bg-blue-500/30 group-hover:scale-110 duration-300'
                            }`}>
