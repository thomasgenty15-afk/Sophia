...
const IdentityArchitect = () => {
  ...
  // Chat
  const WELCOME_MESSAGE: ArchitectChatMessage = { id: 1, sender: 'ai', text: `Salut Architecte. Bienvenue dans le module "${currentWeek?.title}". Par où veux-tu commencer ?` };
  const [messages, setMessages] = useState<ArchitectChatMessage[]>([WELCOME_MESSAGE]);
  const [inputMessage, setInputMessage] = useState("");
  const [isChatLoading, setIsChatLoading] = useState(false);
  const [chatError, setChatError] = useState<string | null>(null);
  // Running synthesis note generated by Sophia, re-injected into the next context call.
  const [filRouge, setFilRouge] = useState<string | null>(null);

  // Load chat history from DB for this module scope
  useEffect(() => {
    if (!user || authLoading) return;
    const scope = `module:${moduleId}`;
    supabase
      .from('chat_messages')
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Detect casual / conversational inputs (greetings, short acks, meta questions).
      const normalized = userText
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
      const GREETING_RE = /^(yo|hey+|salut|hello|hi|coucou|bonjour|bonsoir|re|wesh|allo|ola|hola)\b/;
      const SHORT_ACK_RE = /^(ok|oki|okay|d'?ac|dac|daccord|merci|thx|thanks|nickel|super|cool|top|got it|ouais|ouep|yep|yes|oui|non|nope)\b/;
      const isShortLength = userText.length <= 20;
      const isPunctuationOnly = /^[\s?!.…]+$/.test(userText);
      const isCasualMessage =
        isPunctuationOnly ||
        GREETING_RE.test(normalized) ||
        (isShortLength && SHORT_ACK_RE.test(normalized));

      // Build the module context that goes into contextOverride (injected as system
      // context by sophia-brain, not mixed into the user message).
      // For casual messages: minimal context so Sophia can respond naturally.
      // For substantive messages: full week context so Sophia can anchor on the exercise.
      const moduleContextLines: string[] = [
        `Module identité — Semaine ${weekNumber} : ${currentWeek.title}`,
        `Thème actif (Q${activeQuestionIndex}) : ${activeQ?.question || 'N/A'}`,
        // placeholder = the real exercise prompt (full question + instructions)
        activeQ?.placeholder
          ? `Question clé & consignes :\n${activeQ.placeholder}`
          : "",
      ];
      if (!isCasualMessage) {
        if (activeQ?.helperText) {
          moduleContextLines.push(`Conseils du module :\n${activeQ.helperText}`);
        }
        if (activeAnswer.trim().length) {
          moduleContextLines.push(
            `Réponse actuelle de l'utilisateur : ${trunc(oneLine(activeAnswer), 600)}`,
          );
        }
        if (answeredQuestionsSummary) {
          moduleContextLines.push(
            `Autres réponses de la semaine (contexte) :\n${answeredQuestionsSummary}`,
          );
        }
      }
      if (filRouge) {
        moduleContextLines.push(`Fil rouge (synthèse du coaching en cours) :\n${filRouge}`);
      }
      const moduleContextOverride = trunc(
        moduleContextLines.filter(Boolean).join("\n"),
        4000,
      );

      // message = exactly what the user typed (no scaffolding injected here anymore).
      ...
      const { data, error } = await supabase.functions.invoke('sophia-brain', {
        body: {
          ...
          contextOverride: moduleContextOverride,
        },
      ...
      // Extract and strip the hidden fil_rouge note before displaying.
      const filRougeMatch = rawText.match(/<!--fil_rouge:([\s\S]*?)-->/);
      if (filRougeMatch) {
        setFilRouge(filRougeMatch[1].trim());
      }
      const cleanedText = rawText.replace(/<!--fil_rouge:[\s\S]*?-->/g, "").trim();
      // Sanitize immediately when receiving (defense in depth, also cleaned on display)
      const assistantText = sanitizeBrokenGlyphs(cleanedText);
              ...
              <div className="md:hidden fixed inset-0 z-[100] bg-black/70 backdrop-blur-sm p-3">
                <div className="relative h-full w-full rounded-[28px] border border-emerald-800/60 bg-emerald-950 shadow-2xl overflow-hidden flex flex-col">
                  ...
                  <div className="p-4 border-t border-emerald-900 bg-emerald-950 shrink-0">
                    {activeQuestion && (() => {
                      const q = currentWeek.subQuestions.find(sq => sq.id === activeQuestion) ?? currentWeek.subQuestions[0];
                      return q ? (
                        <p className="text-emerald-600 text-xs mb-2 leading-snug">
                          On travaille sur : <span className="text-emerald-400 font-medium">{q.question}</span>
                        </p>
                      ) : null;
                    })()}
    ...
    <div className="min-h-screen bg-emerald-950 text-emerald-50 flex flex-col md:flex-row overflow-hidden font-sans">
      ...
      <div className="w-full md:w-[60%] flex flex-col h-screen overflow-y-auto border-r border-emerald-900 relative">
        ...
        <div className="sticky top-0 z-10 bg-emerald-950/90 backdrop-blur-md border-b border-emerald-900 p-4 md:p-6 flex items-center justify-between">
          <button 
            onClick={() => navigate('/dashboard?mode=architecte')}
