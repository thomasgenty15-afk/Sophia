...
function getPotionRemainingDays(
  session: UserPotionSessionRow | null,
): number | null {
...
function getAttackTechniques(
  ...
  const defaultsByKey = new Map(
    ATTACK_TECHNIQUE_PREVIEWS.map((
      technique,
    ) => [technique.technique_key, technique]),
  );
        ...
        ? (
          <div className="mt-4 rounded-2xl bg-white px-4 py-4">
            ...
            <ul className="mt-2 space-y-2 text-sm leading-6 text-stone-700">
              {technique.generated_result.supporting_points.map((
                point,
                index,
              ) => (
...
export function AttackTechniqueFlowModal({
  ...
  const [selectedTechniqueKey, setSelectedTechniqueKey] = useState<
    AttackTechniqueKey | null
  >(null);
  const [answers, setAnswers] = useState<string[]>([]);
  const [result, setResult] = useState<AttackTechniqueGeneratedResult | null>(
    null,
  );
  ...
  const selectedTechnique = selectedTechniqueKey
    ? techniques.find((technique) =>
      technique.technique_key === selectedTechniqueKey
    ) ?? null
    : null;
            ...
            ? (
              <div className="space-y-4">
                ...
                {questions.map((question, index) => (
                  <label
                    key={`${selectedTechnique.technique_key}-${index}`}
                    className="block"
                  >
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  <button
                    type="button"
                    disabled={generatingTechniqueKey ===
...
function AttackTechniqueAdjustmentModal({
  ...
  const [step, setStep] = useState<
    "feedback" | "choose" | "questions" | "result"
  >("feedback");
  ...
  const [analysis, setAnalysis] = useState<
    AttackTechniqueAdjustmentAnalysis | null
  >(null);
  const [selectedTechniqueKey, setSelectedTechniqueKey] = useState<
    AttackTechniqueKey | null
  >(null);
  ...
  const currentTechnique =
    techniques.find((technique) =>
      technique.technique_key === currentTechniqueKey
    ) ?? null;
              ...
              ? (
                <button
                  ...
                  onClick={() => {
                    setError(null);
                    setStep((current) =>
                      current === "questions" ? "choose" : "feedback"
                    );
                  }}
                  className="rounded-full border border-stone-200 bg-white px-4 py-3 text-xs font-semibold text-stone-700"
                >
  ...
  async function handleAnalyze() {
    ...
    const nextAnalysis = await onAnalyze(
      currentTechniqueKey,
      reasonKey,
      failureNotes,
    );
...
function PotionCard({
  ...
  const [potionScopeKind, setPotionScopeKind] = useState<PotionScopeKind | "">(
    "",
  );
  const [potionTargetScope, setPotionTargetScope] = useState<
    PotionTargetScope | ""
  >("");
  ...
  const followUpDefaults = useMemo(() => {
    ...
    return {
      ...
      time: latestSession?.follow_up_strategy?.scheduled_local_time_hhmm ??
        "09:00",
      duration: duration ? String(duration) : "7",
    };
  }, [latestSession]);
  ...
  }>({ sessionId: null, time: null, duration: null });
  const followUpDraftForSession =
    followUpDraft.sessionId === followUpDefaults.sessionId
      ? followUpDraft
      : null;
  const followUpTime = followUpDraftForSession?.time ?? followUpDefaults.time;
  const followUpDuration = followUpDraftForSession?.duration ??
    followUpDefaults.duration;

  const activePlanItems = planItems.filter((item) => item.status === "active");
  const requiresPotionScope = definition.type === "rappel" ||
    definition.type === "courage";
  const potionScopeReady = !requiresPotionScope
    ? true
    : potionScopeKind === "out_of_plan" ||
      (potionScopeKind === "plan_linked" &&
        (potionTargetScope === "whole_plan" ||
          potionTargetScope === "unknown" ||
          (potionTargetScope === "plan_item" &&
            Boolean(potionTargetPlanItemId))));
  ...
  const isScheduling = latestSession
    ? schedulingSessionId === latestSession.id
    : false;
  ...
  const scheduledSummary = isSeriesScheduled
    ? `${
      followUpStrategy.scheduled_message_count ??
        followUpStrategy.scheduled_duration_days ?? 0
    } message${
      (followUpStrategy.scheduled_message_count ??
          followUpStrategy.scheduled_duration_days ?? 0) > 1
        ? "s"
        : ""
    } prevu${
      (followUpStrategy.scheduled_message_count ??
      ...
      followUpStrategy.scheduled_local_time_hhmm
        ? ` a ${followUpStrategy.scheduled_local_time_hhmm}`
        : ""
    }`
    : null;

  async function handleActivateSubmit() {
    const selectedPlanItem = activePlanItems.find((item) =>
      item.id === potionTargetPlanItemId
    );
    const potionScope: PotionScopeSelection | null =
      requiresPotionScope && potionScopeKind
        ? {
          scope_kind: potionScopeKind,
          target_scope: potionScopeKind === "plan_linked"
            ? (potionTargetScope || "unknown") as PotionTargetScope
            : null,
          target_plan_item_id: potionScopeKind === "plan_linked" &&
              potionTargetScope === "plan_item"
            ? potionTargetPlanItemId || null
            : null,
          target_label: potionScopeKind === "plan_linked" &&
              potionTargetScope === "plan_item"
            ? selectedPlanItem?.title ?? null
            : potionScopeKind === "plan_linked" &&
                potionTargetScope === "whole_plan"
            ? "Tout mon plan / mon cap general"
            : null,
        }
        : null;
        ...
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0 flex flex-1 items-start gap-3">
            <div className="relative shrink-0">
              <div
                className={`relative flex h-12 w-12 items-center justify-center rounded-2xl border ${visual.iconWrapClass}`}
              >
                <PotionIcon className={`h-5 w-5 ${visual.iconClass}`} />
              </div>
              <span
                className={`absolute -left-1 top-1 h-2.5 w-2.5 rounded-full ${visual.dotClass}`}
              />
              <span
                className={`absolute -right-1 bottom-1 h-2 w-2 rounded-full ${visual.dotClass}`}
              />
              <span
                className={`absolute right-1 -top-1 h-1.5 w-1.5 rounded-full ${visual.dotClass}`}
              />
            </div>
            <div className="min-w-0 flex-1">
              {latestSession
          ...
          <div
            className={`rounded-2xl px-3 py-2 text-right ${visual.badgeClass}`}
          >
                              ...
                              <div className="mt-4 grid gap-3 md:grid-cols-[minmax(0,1fr)_160px_140px]">
                                <label className="text-sm text-stone-700">
                                  ...
                                  <input
                                    ...
                                    onChange={(event) =>
                                      setFollowUpDraft({
                                        sessionId: followUpDefaults.sessionId,
                ...
                : (
                  <div className="rounded-2xl border border-stone-200 bg-white p-4">
                    <p className="text-sm leading-6 text-stone-600">
                      Active cette potion une premiere fois pour avoir ton
                      texte, puis tu pourras le retrouver ici et la reactiver
                      sans le changer.
                    </p>
                  </div>
                )}
          ...
          <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
            ...
            <div className="relative z-10 w-full max-w-2xl overflow-hidden rounded-[28px] border border-stone-200 bg-white shadow-2xl">
              <div className="flex items-start justify-between gap-4 border-b border-stone-200 px-5 py-5">
                <div className="flex items-start gap-3">
                  <div
                    className={`relative flex h-11 w-11 items-center justify-center rounded-2xl border ${visual.iconWrapClass}`}
                  >
                    <PotionIcon className={`h-5 w-5 ${visual.iconClass}`} />
                  </div>
                  <div>
                  ...
                  ? (
                    <div className="rounded-2xl border border-stone-200 bg-stone-50 p-4">
                      ...
                      <div className="mt-3 grid gap-2 sm:grid-cols-2">
                        {([
                          ["plan_linked", "C'est lié à mon plan"],
                          ["out_of_plan", "C'est hors plan"],
                        ] as Array<[PotionScopeKind, string]>).map(
                          ([value, label]) => {
                            const selected = potionScopeKind === value;
                            return (
                              <button
                                key={value}
                                type="button"
                                onClick={() => {
                                  setPotionScopeKind(value);
                                  setPotionTargetScope("");
                                  setPotionTargetPlanItemId("");
                                }}
                        ...
                        ? (
                          <div className="mt-4 space-y-2">
                            {activePlanItems.map((item) => {
                              const selected =
                                potionTargetScope === "plan_item" &&
                                potionTargetPlanItemId === item.id;
                              return (
                                <button
                                  key={item.id}
                                  type="button"
                                  onClick={() => {
                                    setPotionTargetScope("plan_item");
                                    setPotionTargetPlanItemId(item.id);
                                  }}
                                  className={`block w-full rounded-2xl border px-3 py-2 text-left text-xs font-medium transition ${
                                    selected
                            ...
                            {([
                              ["whole_plan", "Tout mon plan / mon cap général"],
                              ["unknown", "Je ne sais pas exactement"],
                            ] as Array<[PotionTargetScope, string]>).map(
                              ([value, label]) => {
                                const selected = potionTargetScope === value;
                                return (
                                  <button
                                    key={value}
                                    type="button"
                                    onClick={() => {
                                      setPotionTargetScope(value);
                                      setPotionTargetPlanItemId("");
                                    }}
                                    className={`block w-full rounded-2xl border px-3 py-2 text-left text-xs font-medium transition ${
                                      selected
...
export function LabCardsPanel({
  ...
  const [adjustingTechniqueKey, setAdjustingTechniqueKey] = useState<
    AttackTechniqueKey | null
  >(null);
  ...
  const techniques = storedTechniques.length > 0
    ? storedTechniques
    : ATTACK_TECHNIQUE_PREVIEWS;
  const generatedTechniques = techniques.filter((technique) =>
    technique.generated_result
  );
  const remainingTechniques = techniques.filter((technique) =>
    !technique.generated_result
  );
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-amber-100 bg-amber-50/70 p-4">
              ...
              {isAttackIntroOpen
                ? (
                  <p className="mt-2 text-sm leading-6 text-amber-950">
                    Une carte d'attaque ne sert pas a te pousser plus fort au
                    dernier moment. Elle sert a agir en amont, pour que le bon
                    comportement devienne plus naturel quand le moment arrive.
                    En pratique, tu prepares ton terrain psychologique ou
                    materiel pour reduire la friction, diminuer l'hesitation, et
                    avoir besoin de moins de force sur le moment. Par exemple,
                    une ancre visuelle peut te remettre dans le bon axe sans
                    avoir a renegocier avec toi-meme, et un rituel de depart
                    peut rendre le passage a l'action beaucoup plus simple au
                    moment de commencer.
                  </p>
                )
                : null}
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-sm font-semibold text-stone-950">
                      Cartes d'attaque libres
                    </p>
                    <p className="mt-1 text-sm leading-6 text-stone-600">
                      Pour des actions qui peuvent t'aider, mais qui ne sont pas
                      dans le plan.
                    </p>
                  </div>
                </div>
                    ...
                    {loading
                        ...
                        <>
                          <div className="flex flex-wrap items-center justify-end gap-3">
                            <button
                              ...
                              disabled={remainingTechniques.length === 0 ||
                                generatingAttack}
                          ...
                          <div className="grid gap-4 xl:grid-cols-2">
                            {generatedTechniques.map((technique) => (
                              <GeneratedAttackCard
                                key={technique.technique_key}
                                technique={technique}
                                analyzing={analyzingTechniqueKey ===
                          ...
                          <div className="flex flex-wrap items-center gap-3">
                            {remainingTechniques.length === 0
                              ...
                              : (
                                <p className="text-xs text-stone-500">
                                  {remainingTechniques.length}{" "}
                                  technique{remainingTechniques.length > 1
                                    ? "s"
                                    : ""}{" "}
                                  restante{remainingTechniques.length > 1
                                    ? "s"
                                    : ""}.
                                </p>
                              )}
                      ...
                      : (
                        <div className="rounded-3xl border border-dashed border-stone-300 bg-white px-5 py-6 text-center">
                          <p className="text-sm leading-6 text-stone-600">
                            Tu n'as pas encore de carte d'attaque libre dans
                            cette section.
                          </p>
                          <button
                            type="button"
              ...
              <div className="mt-4 rounded-2xl border border-stone-200 bg-stone-50">
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-stone-950">
                        Cartes d'attaque du plan
                      </p>
                      <p className="mt-1 text-sm leading-6 text-stone-600">
                        Elles se rangent ici, par niveau, quand tu choisis de
                        les preparer pour une action du plan.
                      </p>
                    </div>
                  </div>
                  <ChevronDown
