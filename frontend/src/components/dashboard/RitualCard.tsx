import { Sparkles, CheckCircle2, Target, Lock, Play, Clock } from 'lucide-react';
...
export const RitualCard = ({ action }: { action: any }) => {
  ...
  if (isUpsell) {
    return (
      <div className={`bg-gradient-to-r from-indigo-900 to-violet-900 border border-indigo-800 rounded-xl p-4 mb-4 shadow-md relative overflow-hidden group ${isComingSoon ? 'opacity-90 grayscale-[20%]' : 'cursor-pointer hover:scale-[1.01]'} transition-transform`}>
        ...
        <div className="flex flex-col min-[400px]:flex-row items-start justify-between relative z-10 gap-3 min-[400px]:gap-0">
          <div className="flex flex-col min-[350px]:flex-row items-start min-[350px]:items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shadow-inner ${isComingSoon ? 'bg-white/5 text-indigo-300' : 'bg-white/10 text-amber-400'}`}>
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <h3 className={`font-bold text-sm min-[350px]:text-base leading-tight ${isComingSoon ? 'text-indigo-300' : 'text-indigo-200'}`}>{action.title}</h3>
              <p className="text-indigo-200 text-xs min-[350px]:text-sm mt-1 max-w-[200px] opacity-80">{action.description}</p>
            </div>
          </div>
          <div className="flex flex-col items-end self-end min-[400px]:self-auto">
            {isComingSoon ? (
               <div className="flex flex-col items-end gap-1">
                  <span className="bg-white/10 backdrop-blur-md border border-white/10 text-indigo-100 text-xs font-bold px-3 py-1.5 rounded-full flex items-center gap-2 shadow-sm">
                    <Clock className="w-3 h-3 text-indigo-300" /> Bientôt
                  </span>
               </div>
            ) : (
                <>
  ...
  return (
    <div className={`bg-white border border-gray-200 rounded-xl p-4 mb-3 flex items-center justify-between shadow-sm min-h-[80px] ${isComingSoon ? 'bg-slate-50/50' : ''}`}>
      <div className="flex flex-col min-[350px]:flex-row items-start min-[350px]:items-center gap-3 w-full">
        <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isComingSoon ? 'bg-slate-100 text-slate-400' : (action.subType === 'hypnose_daily' ? 'bg-indigo-100 text-indigo-600' : 'bg-blue-100 text-blue-600')}`}>
        ...
        <div className="min-w-0 flex-1 pr-0 min-[350px]:pr-2 w-full">
          <h3 className={`font-bold text-sm min-[350px]:text-base leading-tight mb-1.5 ${isComingSoon ? 'text-slate-500' : 'text-gray-900'}`}>{action.title}</h3>
          {action.subType === 'hypnose_daily' && !isComingSoon ? (
      ...
      {isComingSoon ? (
         <div className="ml-2 px-3 py-1.5 bg-slate-100 text-slate-400 rounded-lg text-xs font-bold border border-slate-200 flex items-center gap-1.5 shadow-sm whitespace-nowrap cursor-default">
           <Clock className="w-3 h-3 opacity-70" /> À venir
         </div>
      ) : isLimitReached ? (
