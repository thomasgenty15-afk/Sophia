...
describe("awayFrom — les deux sources d'une absence, séparées (D14)", () => {
  ...
  it("⛔ une ancienne entrée « dehors » ressort « absent », et le filtre de source tient", () => {
    // ⟳ 2026-09-24 — l'état « dehors » est retiré et ses entrées converties en
    // base. Une ligne lue avant la conversion doit repartir `kind: "away"`: la
    // grille réécrit ce qu'elle reçoit, et `keel_household_set_member_away`
    // refuse désormais `eating_out`.
    const OUT = [
    ...
    // La vue du maître n'hérite pas de ce que la personne a déclaré pour
    // elle-même, sinon la grille le recopierait dans sa colonne.
