...
//
// ⛔ LE DÉFAUT. L'étape 4 listait `facts.mouths` — dont `readFunnelFacts` le
// retire. Des absences posées sur SA ligne membre sortaient donc des repas du
// plan sans que le tunnel lui donne une seule case pour les contredire.
//
...
describe("D4 ② · le titulaire entre dans la liste des grilles", () => {
  ...
  it("⛔ SES ABSENCES ARRIVENT JUSQU'À LA GRILLE", () => {
    // Sans elles, la grille s'ouvre VIERGE sur une ligne qui porte cinq marques:
    // le premier enregistrement les effacerait toutes, en silence.
    const roster = presenceRoster({ self: self(), mouths: [] });
    expect(roster[0].away).toEqual(PREFILLED);
