...
const LOCK_PATH = "/tmp/sophia_tools_break_down_action_eval.lock";
...
function acquireLock() {
  try {
    ...
    throw new Error(
      `Another tools_break_down_action eval runner seems to be running (lock=${LOCK_PATH} pid=${pidRaw}). ` +
...
async function main() {
  ...
  const scenarioPath = path.join(process.cwd(), "eval", "scenarios", "tools", "tools_break_down_action_ai_user.json");
  const scenario = JSON.parse(fs.readFileSync(scenarioPath, "utf8"));

  const limits = {
    max_scenarios: 1,
    max_turns_per_scenario: 16,
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    ...
    try {
      ...
      // Retry only on transient infra-ish failures.
  ...
  const out = {
    ...
    eval_run_id: (Array.isArray(data?.results) ? data.results[0] : null)?.eval_run_id ?? null,
    test_user_id: (Array.isArray(data?.results) ? data.results[0] : null)?.test_user_id ?? null,
    issues_count: (Array.isArray(data?.results) ? data.results[0] : null)?.issues_count ?? null,
    suggestions_count: (Array.isArray(data?.results) ? data.results[0] : null)?.suggestions_count ?? null,
    ...(lastErr
      ? { error: String(lastErrBody?.error ?? lastErr?.message ?? lastErr).slice(0, 1600) }
      : (!data?.success
        ? { error: String(data?.error ?? data?.detail ?? JSON.stringify(data ?? {})).slice(0, 1600) }
        : {})),
  };
...
main().catch((e) => {
  console.log(JSON.stringify({ ok: false, request_id: null, eval_run_id: null, test_user_id: null, issues_count: null, suggestions_count: null, error: String(e?.message ?? e) }, null, 2));
  process.exitCode = 1;
});
