...
const LOCK_PATH = "/tmp/sophia_tools_update_action_eval.lock";
...
function acquireLock() {
  try {
    ...
    // If the lock is stale, clean it up automatically (common when a previous run crashed).
    // This keeps the "single-run" invariant without requiring manual /tmp cleanup.
    ...
    throw new Error(
      `Another tools_update_action eval runner seems to be running (lock=${LOCK_PATH} pid=${pidRaw}). ` +
...
function curlInvokeRunEvals({ url, anonKey, accessToken, body }) {
  ...
  try {
    ...
    if (String(httpCode) !== "200") {
      throw new Error(`run-evals http=${httpCode} body=${rawJson.slice(0, 1600)}`);
    }
...
function tryParseRunEvalsErrorBody(msg) {
  // We throw errors like: `run-evals http=500 body={"error":"...","request_id":"..."}`
  const m = String(msg ?? "");
...
async function main() {
  ...
  const scenarioPath = path.join(process.cwd(), "eval", "scenarios", "tools", "tools_update_action_ai_user.json");
  ...
  for (let attempt = 1; attempt <= 6; attempt++) {
    try {
      ...
      lastErr = e;
      lastErrBody = tryParseRunEvalsErrorBody(msg) ?? lastErrBody;

      // If run-evals propagated a meaningful error, don't "restart from beginning" on non-transient failures.
      // This is the common culprit behind "it keeps restarting": simulate-user can fail its rescue guard.
      const bodyErr = String(lastErrBody?.error ?? "");
      const nonRetryable =
        bodyErr.includes("simulate-user failed: invalid output after rescue") ||
        bodyErr.includes("Forbidden") ||
        bodyErr.includes("Unauthorized");

      const retryable =
        !nonRetryable && (
        msg.includes("__HTTP_CODE__:502") || msg.includes("http=502") || msg.includes(" 502") ||
        msg.includes("__HTTP_CODE__:500") || msg.includes("http=500") || msg.includes(" 500") ||
        msg.includes("__HTTP_CODE__:503") || msg.includes("http=503") || msg.includes(" 503") ||
        msg.includes("http=546") || msg.includes("__HTTP_CODE__:546") || msg.includes(" 546") ||
        msg.includes("WORKER_LIMIT"));
      if (retryable) {
        // Local edge runtime can hot-reload and briefly return 502/503. Resource limits can return 546 (WORKER_LIMIT).
        // Retry a few times with backoff.
        sleepMs(3000 * attempt);
        continue;
      }
      // Non-retryable: stop the loop and print a stable JSON summary below.
      break;
    }
  }
  if (!data) {
    // Always print a stable JSON summary for eval runners (even on failures),
    // so the caller can iterate on issues without guessing what happened.
    const requestId = lastErrBody?.request_id ?? null;
    const errMsg = String(lastErrBody?.error ?? (lastErr?.message ?? lastErr ?? "unknown error"));
