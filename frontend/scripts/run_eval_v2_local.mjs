/**
 * run_eval_v2_local.mjs
 *
 * LOCAL eval runner: resets a fixed user on a local Supabase instance, runs a scenario
 * via run-evals, then exports conversation transcript + brain trace for the exact
 * time interval of the run.
 *
 * Designed for deterministic, reproducible test runs:
 *   - Resets user DB state before each run (chat, actions, vitals, checkup logs)
 *   - Seeds plan + actions + vitals + investigation_state from scenario JSON
 *   - Exports initial investigation_state to bundle for verification
 *   - Restores DB state after run so the same test can be re-run identically
 *
 * Replicable to any state machine by changing --scenario.
 * Supports scenario variants (--variant <id> | all | random).
 *
 * Usage:
 *   cd frontend && \
 ...
 *   SOPHIA_MASTER_ADMIN_EMAIL="admin@test.local" \
 *   SOPHIA_MASTER_ADMIN_PASSWORD="123456" \
 *   node scripts/run_eval_v2_local.mjs \
 *     --scenario target-exceed-bilan-V4 \
 *     --variant yes_no_days \
 *     --turns 14 \
 *     --slot 1           # which fixed user slot (1-10, default random)
 *     --all-slots        # run all slots sequentially
 */
...
function parseArgs(argv) {
  const out = {
    ...
    slot: null,    // 1-20 or null for random
    allSlots: false,
    variant: null,
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    else if (a === "--slot") out.slot = Math.max(1, Math.min(20, Number(argv[++i]) || 1));
    ...
    else if (a === "--user-difficulty") {
      const d = String(argv[++i] ?? out.userDifficulty).trim().toLowerCase();
      out.userDifficulty = (d === "easy" || d === "mid" || d === "hard") ? d : out.userDifficulty;
    }
    else if (a === "--use-real-ai") {
      const v = String(argv[++i] ?? "true").trim().toLowerCase();
      out.useRealAi = !(v === "0" || v === "false" || v === "no");
    }
  }
  return out;
}
...
function materializeScenarioVariants({ scenario, variant }) {
  ...
  const pickOne = (v) => {
    ...
    if (v?.assertions && typeof v.assertions === "object") out.assertions = { ...(base.assertions ?? {}), ...v.assertions };
    if (v?.mechanical_assertions && typeof v.mechanical_assertions === "object") {
      out.mechanical_assertions = { ...(base.mechanical_assertions ?? {}), ...v.mechanical_assertions };
    }
    // Deep-merge setup: variant setup overrides base setup per-key, preserving shared fields.
    if (v?.setup && typeof v.setup === "object") {
      const baseSetup = base.setup && typeof base.setup === "object" ? base.setup : {};
      out.setup = { ...baseSetup, ...v.setup };
      // Special-case: preseed_actions and preseed_action_entries replace entirely (not merged)
      // because each variant typically defines its own full set.
    }
    if (Array.isArray(v?.tags)) out.tags = [...new Set([...(base.tags ?? []), ...v.tags])];
...
// ─── Resolve scenario file (dedicated folder or tools/ fallback) ────────────
// Lookup order:
//   1. eval/scenarios/<name>/scenario.json            (folder name = CLI arg)
//   2. eval/scenarios/<name_with_underscores>/scenario.json
//   3. Scan all dedicated folders — match scenario.json "id" field against CLI arg
//   4. eval/scenarios/tools/<name>.json               (legacy flat folder)
function resolveScenarioFile(evalScenariosRoot, scenarioName) {
  // 1. Dedicated folder (exact name match)
  const dedicatedFile = path.join(evalScenariosRoot, scenarioName, "scenario.json");
  if (fs.existsSync(dedicatedFile)) return dedicatedFile;

  // 2. Dashes → underscores
  const altName = scenarioName.replace(/-/g, "_");
  const altFile = path.join(evalScenariosRoot, altName, "scenario.json");
  if (altFile !== dedicatedFile && fs.existsSync(altFile)) return altFile;

  // 3. Scan dedicated folders — match by scenario "id" field inside scenario.json
  try {
    const dirs = fs.readdirSync(evalScenariosRoot).filter((f) => {
      try { return fs.statSync(path.join(evalScenariosRoot, f)).isDirectory(); } catch { return false; }
    });
    for (const dir of dirs) {
      const candidate = path.join(evalScenariosRoot, dir, "scenario.json");
      if (!fs.existsSync(candidate)) continue;
      try {
        const content = JSON.parse(fs.readFileSync(candidate, "utf8"));
        if (String(content?.id ?? "").trim() === scenarioName) return candidate;
      } catch {}
    }
  } catch {}

  // 4. Flat tools/ folder
  const toolsFile = path.join(evalScenariosRoot, "tools", `${scenarioName}.json`);
  if (fs.existsSync(toolsFile)) return toolsFile;

  // Not found — build a helpful error
  const toolsDir = path.join(evalScenariosRoot, "tools");
  const toolsAvailable = fs.existsSync(toolsDir)
    ? fs.readdirSync(toolsDir).filter((f) => f.endsWith(".json")).map((f) => f.replace(".json", ""))
    : [];
  const dedicatedAvailable = fs.readdirSync(evalScenariosRoot)
    .filter((f) => {
      try { return fs.statSync(path.join(evalScenariosRoot, f)).isDirectory() && fs.existsSync(path.join(evalScenariosRoot, f, "scenario.json")); } catch { return false; }
    });
  const all = [...new Set([...dedicatedAvailable, ...toolsAvailable])].sort();
  throw new Error(`Scenario not found: ${scenarioName}\nAvailable: ${all.join(", ")}`);
}

// ─── Per-scenario reset loader ──────────────────────────────────────────────────
// If a scenario lives in a dedicated folder with commands/reset.mjs, use it for
// post-run resets instead of the generic resetUserForEval + ensurePlanSeeded.
async function tryLoadScenarioReset(evalScenariosRoot, scenarioName) {
  // Build candidate paths (same logic as resolveScenarioFile: direct name, underscored, then scan by id)
  const candidates = [
    path.join(evalScenariosRoot, scenarioName, "commands", "reset.mjs"),
    path.join(evalScenariosRoot, scenarioName.replace(/-/g, "_"), "commands", "reset.mjs"),
  ];

  // Also scan dedicated folders by scenario id
  try {
    const dirs = fs.readdirSync(evalScenariosRoot).filter((f) => {
      try { return fs.statSync(path.join(evalScenariosRoot, f)).isDirectory(); } catch { return false; }
    });
    for (const dir of dirs) {
      const scenFile = path.join(evalScenariosRoot, dir, "scenario.json");
      if (!fs.existsSync(scenFile)) continue;
      try {
        const content = JSON.parse(fs.readFileSync(scenFile, "utf8"));
        if (String(content?.id ?? "").trim() === scenarioName) {
          const resetPath = path.join(evalScenariosRoot, dir, "commands", "reset.mjs");
          if (!candidates.includes(resetPath)) candidates.push(resetPath);
        }
      } catch {}
    }
  } catch {}
  ...
  for (const p of candidates) {
    if (fs.existsSync(p)) {
      try {
        ...
        // Accept either named export resetBilanExceedTarget or generic reset
        const resetFn = mod.resetBilanExceedTarget ?? mod.reset ?? null;
        const validateFn = mod.validateReset ?? null;
        if (typeof resetFn === "function") {
          console.log(`[V2 Eval] Loaded per-scenario reset from: ${p}`);
          return { resetFn, validateFn, path: p };
        }
      } catch (e) {
        console.warn(`[V2 Eval] Failed to load per-scenario reset from ${p}: ${e.message}`);
      }
    }
  }
  return null;
...
function tryAcquireLocalSlotLock(userId) {
  const lockPath = `/tmp/sophia_eval_slot_${String(userId).replace(/[^a-zA-Z0-9_-]/g, "_")}.lock`;
  const acquire = () => {
...
async function seedBilanVitalIfNeeded(admin, userId, planId, scenario) {
  ...
  // If scenario defines preseed_vitals, use them. Otherwise fall back to default "Sommeil".
...
async function seedInvestigationStateFixture(admin, userId, scenario) {
  ...
  // ── Fetch actions with ALL fields needed to build correct CheckupItems ──
  ...
  // ── Build CheckupItems with the same logic as getPendingItems (db.ts) ──
  const pendingActions = (actions ?? []).slice(0, activeCount).map((a) => {
    ...
    // Compute weekly_target_status (mirrors db.ts logic)
    ...
    return {
      ...
      is_scheduled_day: true, // In fixture mode, always consider it a scheduled day
      day_scope: "today",
  ...
  // Same sort as production: vitals first, then actions
...
async function ensurePlanSeeded(admin, userId, scenario) {
  const setup = scenario?.setup ?? {};
  ...
  if (existingPlans && existingPlans.length > 0) {
    ...
    if (preseedActions.length > 0) {
      ...
      const invState = await seedInvestigationStateFixture(admin, userId, scenario);
...
function exportBundleUsingProdScript({ url, serviceRoleKey, userId, sinceIso, untilIso, scenarioKey, slot }) {
  ...
  const isLocalUrl = /\b(127\.0\.0\.1|localhost|0\.0\.0\.0)\b/i.test(String(url ?? ""));
  const args = [
    exporter,
  ...
  if (!isLocalUrl) args.splice(1, 0, "--prod");
  const env = {
    ...process.env,
    SUPABASE_URL: url,
...
async function runOneSlot({ url, anonKey, serviceRoleKey, accessToken, admin, scenario, user, args }) {
  ...
  const limits = {
    ...
    test_post_checkup_deferral: Boolean(setup?.test_post_checkup_deferral),
    user_difficulty: args.userDifficulty || "hard",
    stop_on_first_failure: false,
    budget_usd: 0,
    model: args.model,
    use_real_ai: Boolean(args.useRealAi),
  ...
  const scenarioReset = args._scenarioReset;
  if (scenarioReset?.resetFn) {
    // Extract variant from scenario id (e.g. "target-exceed-bilan-V4__no" → "no")
    const variantId = String(scenario?.id ?? "").split("__").pop() || "no";
    await scenarioReset.resetFn(admin, variantId, userId);
    if (scenarioReset.validateFn) {
      const validation = await scenarioReset.validateFn(admin, variantId, userId);
      if (!validation.allOk) {
        console.warn(`[V2 Eval] ⚠ Post-run reset validation FAILED (${validation.passed}/${validation.total} passed)`);
      }
    }
  } else {
    await resetUserForEval(admin, userId, "web");
...
async function main() {
  ...
  // Load scenario — check both dedicated folders and tools/ fallback
  const evalScenariosRoot = path.join(process.cwd(), "eval", "scenarios");
  const scenarioFile = resolveScenarioFile(evalScenariosRoot, args.scenario);
  ...
  console.log(`[V2 Eval] Loaded scenario base: ${scenarioBase.id} (from ${scenarioFile})`);
  console.log(`[V2 Eval] Variants selected: ${scenarios.map((s) => s.id).join(", ")}`);

  // Try loading per-scenario reset (if dedicated folder exists)
  const scenarioReset = await tryLoadScenarioReset(evalScenariosRoot, args.scenario);
  // Attach to args so runOneSlot can use it
  args._scenarioReset = scenarioReset;
