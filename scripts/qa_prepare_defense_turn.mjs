...
const date = process.env.QA_DATE || "2026-05-15";
...
if (!jwt) throw new Error(`missing ${persona} JWT`);

const scope = `qa-prepare-defense-card-${persona}-${date}-${runId}`;
...
const requestId = `qa-prepare-defense-card-${persona}-${date}-${runId}-t${
...
const durable = {
  user_defense_cards: await restSelect(
    "user_defense_cards",
    `user_id=eq.${userId}&select=id,plan_item_id,status,source,scope_kind,transformation_id,cycle_id,content,metadata,generated_at,last_updated_at&order=generated_at.desc&limit=30`,
  ),
  user_plan_items: await restSelect(
    "user_plan_items",
    `user_id=eq.${userId}&select=id,title,dimension,kind,status,cards_status,defense_card_id,attack_card_id,description,target_reps,current_reps,cadence_label,time_of_day,updated_at&order=activation_order.asc`,
  ),
...
console.log(JSON.stringify({
  ...
  defense_card_id: operationFlowRun?.defense_card_id ?? null,
