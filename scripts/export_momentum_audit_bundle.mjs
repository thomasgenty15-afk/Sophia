...
async function main() {
  ...
  if (args.help) {
    console.log(
      [
        "Usage:",
        "  node ./scripts/export_momentum_audit_bundle.mjs --user-id <uuid> --from <ISO> --to <ISO> [--scope whatsapp]",
        "  node ./scripts/export_momentum_audit_bundle.mjs --user-id <uuid> --hours 24 [--scope whatsapp]",
        "",
        "Options:",
  ...
  const [traceRes, scorecardRes] = await Promise.all([
    postInternalJson(`${functionsBase}/get-momentum-trace`, internalSecret, body),
    postInternalJson(`${functionsBase}/get-momentum-scorecard`, internalSecret, body),
  ]);
  ...
  const defaultOutFile = path.join(
    outDir,
    `momentum_audit_${String(args.userId).slice(0, 8)}_${
  ...
  console.log(JSON.stringify({
    ...
    counts: {
      ...
      state_events: bundle.trace?.summary?.state_events_total ?? null,
      proactive_decisions: bundle.trace?.summary?.proactive_decisions_total ?? null,
      outreachs: bundle.trace?.summary?.outreachs_total ?? null,
