...
function getSupabaseConnection(repoRoot) {
  ...
  if (!url) {
    throw new Error("Missing SUPABASE_URL env and no local `supabase status --output json` available.");
  }
...
const FRESHNESS_WINDOWS = {
  ...
  trigger_daily_bilan: 20 * 3600_000,
  trigger_weekly_bilan: 6 * 24 * 3600_000,
...
async function supabaseGet(baseUrl, serviceKey, tablePath) {
  ...
  if (!res.ok) throw new Error(`GET ${tablePath.slice(0, 80)} → ${res.status}: ${text.slice(0, 800)}`);
...
async function loadActiveUserIds(baseUrl, serviceKey, fromIso, toIso) {
  const events = await supabaseGet(baseUrl, serviceKey,
...
function aggregateGroup(items) {
  let costUsd = 0, promptTokens = 0, outputTokens = 0, totalTokens = 0, latencySum = 0, latencyCount = 0;
  for (const it of items) {
    costUsd += it.cost_usd;
    promptTokens += it.prompt_tokens;
    outputTokens += it.output_tokens;
    totalTokens += it.total_tokens;
    if (it.latency_ms > 0) { latencySum += it.latency_ms; latencyCount++; }
  }
  return {
    calls: items.length,
...
function detectRedundantCalls(timeline) {
  ...
  const byUserAndSource = groupBy(candidates, (t) => `${t.user_id}::${getFreshnessKey(t.source)}`);
  const redundant = [];
...
function computeScorecard(timeline, dailySeries, redundantCalls, fallbacks) {
  ...
  if (dailySeries.length >= 4) {
    ...
    const firstHalf = dailySeries.slice(0, mid).reduce((s, d) => s + d.cost_usd, 0);
    const secondHalf = dailySeries.slice(mid).reduce((s, d) => s + d.cost_usd, 0);
    if (firstHalf > 0) {
      costTrend = {
  ...
  if (costPerUserPerDay > 0.50) {
    alerts.push({ type: "daily_budget_exceeded", detail: `$${costPerUserPerDay}/user/day exceeds $0.50 budget` });
  }

  if (redundantCalls.length > 0) {
    const pulseRedundant = redundantCalls.filter((r) => r.freshness_key === "conversation_pulse");
    if (pulseRedundant.length > 0) {
      alerts.push({ type: "redundant_pulse_generation", count: pulseRedundant.length });
    }
    const dailyRedundant = redundantCalls.filter((r) => r.freshness_key === "trigger_daily_bilan");
    if (dailyRedundant.length > 0) {
      alerts.push({ type: "redundant_daily_bilan", count: dailyRedundant.length });
    }
    const weeklyRedundant = redundantCalls.filter((r) => r.freshness_key === "trigger_weekly_bilan");
    if (weeklyRedundant.length > 0) {
      alerts.push({ type: "redundant_weekly_bilan", count: weeklyRedundant.length });
    }
  }
  ...
  if (expensiveTier3.length > 0) {
    ...
    alerts.push({ type: "tier3_using_expensive_model", count: expensiveTier3.length, models });
  }
  ...
  if (totalCalls > 0 && failedCount / totalCalls > 0.05) {
    alerts.push({ type: "high_fallback_rate", rate: round4(failedCount / totalCalls) });
  }

  if (totalCalls > 0 && unpricedCount / totalCalls > 0.10) {
    alerts.push({ type: "high_unpriced_rate", rate: round4(unpricedCount / totalCalls) });
  }

  // Tier 1 calls outside onboarding context
  const tier1Calls = timeline.filter((t) => t.tier === 1);
  if (tier1Calls.length > 6) {
    alerts.push({ type: "tier1_high_volume", count: tier1Calls.length, note: "Expected 1-3 per cycle" });
  }
  ...
  return {
    ...
    redundant_call_rate: totalCalls > 0 ? round4(redundantCalls.length / totalCalls) : 0,
...
async function main() {
  ...
  const targetLabel = args.allUsers ? "all-users" : `user=${args.userId.slice(0, 8)}…`;
  console.error(`Connecting to ${baseUrl} (${connection.source})…`);
  console.error(`Loading LLM cost audit for ${targetLabel} window=${args.hours}h`);

  // Load events
  const events = await loadLlmEvents(
    baseUrl, connection.serviceKey,
  ...
  const scorecard = computeScorecard(timeline, dailyCostSeries, redundantCalls, fallbacks);
  ...
  const defaultOutFile = path.join(
    outDir,
    `llm_cost_audit_${userLabel}_${compactTs(fromIso)}_${compactTs(toIso)}.json`,
  );
