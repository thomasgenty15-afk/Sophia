...
function getInternalSecret(connection) {
  ...
  throw new Error("Missing INTERNAL_FUNCTION_SECRET. Set it in env for remote/staging usage.");
}
    ...
    for (const row of messages ?? []) {
      ...
      const requestId = String(
        row?.metadata?.request_id ??
          ...
          row?.metadata?.router_decision_v1?.request_id ??
          "",
      ).trim();
...
async function main() {
  ...
  if (args.help) {
    console.log(
      [
        "Usage:",
        "  npm run coaching:audit:export -- --user-id <uuid> --from <ISO> --to <ISO> [--scope whatsapp]",
  ...
  const [traceRes, scorecardRes] = await Promise.all([
    postInternalJson(`${functionsBase}/get-coaching-intervention-trace`, internalSecret, body),
    postInternalJson(`${functionsBase}/get-coaching-intervention-scorecard`, internalSecret, body),
  ]);
  ...
  const defaultOutFile = path.join(
    outDir,
    `coaching_intervention_audit_${String(args.userId).slice(0, 8)}_${
  ...
  console.log(JSON.stringify({
    ...
    counts: {
      ...
      selector_runs: bundle.trace?.summary?.selector_runs_total ?? null,
      interventions: bundle.trace?.summary?.interventions_total ?? null,
      follow_ups: bundle.trace?.summary?.follow_ups_total ?? null,
      weekly_surfaces: bundle.trace?.summary?.weekly_surfaces_total ?? null,
