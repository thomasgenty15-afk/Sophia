...
// ══════════════════════════════════════════════════════════════════════════
// ⟳ 2026-09-14 · B4 — `--absences=` : UN JOUR SANS BOUCHE, MÊME RPC QUE
//                LA CAMPAGNE (`keel_household_set_member_away`)
// ══════════════════════════════════════════════════════════════════════════
//
//   --absences=,,tue        un jeton par bouche, ordre du roster. Vide =
//                           présent. Plusieurs jours : `tue+wed`.
//
// ⛔ CE N'EST PAS `--rythmes-bouches`. Le rythme retire un CRÉNEAU tous les
// jours ; l'absence retire une JOURNÉE entière. Le tir 9 de la campagne pose
// Nils (3ᵉ bouche) `away: tue`. Sans ce drapeau, `--bouches=4` nourrit
// tout le monde tous les jours et n'exerce pas B4 « présences variables ».
const ABSENCES = (Deno.args.find((a) => a.startsWith("--absences="))?.slice(11) ?? "")
  .split(",").map((x) => x.trim());
...
// ══════════════════════════════════════════════════════════════════════════
// ⟳ 2026-09-14 · BÊTA 2B — `--panne=<famille>[:tour]`
// ══════════════════════════════════════════════════════════════════════════
//
// ⛔ POURQUOI IL EXISTE. Le plan de bêta exige d'éprouver « erreur 546, 429,
// réponse fournisseur invalide, réseau coupé ». Aucune n'avait jamais été
// jouée: « le produit se comporte bien quand le fournisseur tombe » était une
// affirmation.
//
//   --panne=429            le premier jet est refusé pour débit
//   --panne=500            le fournisseur rend une erreur serveur
//   --panne=corps          200, corps illisible (le cas le plus traître)
//   --panne=coupure        la connexion tombe, `fetch` jette
//   --panne=429:1          la panne frappe le tour 1 (une RÉPARATION)
//   --panne=429:*          la panne est PERMANENTE — le fournisseur ne revient
//                          jamais. C'est le cas qui dit si l'issue est BORNÉE.
//
// ⚠️ LE 546 N'Y EST PAS, ET C'EST STRUCTUREL: c'est le worker TUÉ, pas une
// réponse du fournisseur. Aucun crochet ne peut le produire.
const PANNE_ARG = Deno.args.find((a) => a.startsWith("--panne="))?.slice(8) ?? "";
const [PANNE_FAMILLE, PANNE_TOUR_BRUT] = PANNE_ARG.split(":");
/** `*` = panne permanente. Sinon, le rang du tour frappé (0 = premier jet). */
const PANNE_TOUJOURS = PANNE_TOUR_BRUT === "*";
const PANNE_TOUR = PANNE_TOUJOURS ? -1 : (Number(PANNE_TOUR_BRUT ?? "0") || 0);
const PANNES: Record<string, { kind: string; status?: number }> = {
  "429": { kind: "rate_limited" },
  "500": { kind: "server_error", status: 500 },
  "503": { kind: "server_error", status: 503 },
  "corps": { kind: "invalid_body" },
  "coupure": { kind: "connection_cut" },
};
if (PANNE_FAMILLE && !PANNES[PANNE_FAMILLE]) {
  console.error(
    `⛔ --panne inconnue : « ${PANNE_FAMILLE} ». Attendu : ${
      Object.keys(PANNES).join(", ")
    }`,
  );
  Deno.exit(2);
}
if (PANNE_FAMILLE) {
  console.log(
    `\n   ⚡ PANNE PROGRAMMÉE : ${PANNE_FAMILLE} au tour ${
      PANNE_TOUJOURS ? "*" : PANNE_TOUR
    } ` +
      `(0 = premier jet, ≥1 = réparation, * = permanente)\n`,
  );
}
...
const transport = installControlledTransport({
  ...
  // ⟳ 2026-09-14 · BÊTA 2B — voir `--panne=`. `undefined` quand rien n'est
  // demandé: le banc se comporte alors exactement comme avant ce lot.
  providerFault: PANNE_FAMILLE
    ? ({ turn, isComposition }) =>
      isComposition && (PANNE_TOUJOURS || turn === PANNE_TOUR)
        ? (PANNES[PANNE_FAMILLE] as never)
        : null
    : undefined,
...
if (ABSENCES.some((x) => x !== "")) {
  const bouchesOrdonnees = [memberId, ...membresSecondaires];
  for (const [rang, jeton] of ABSENCES.entries()) {
    if (jeton === "" || rang >= bouchesOrdonnees.length) continue;
    const jours = jeton.split("+").map((d) => d.trim()).filter((d) => d !== "");
    const w = await rpc(me.token, "keel_household_set_member_away", {
      p_member: bouchesOrdonnees[rang],
      p_away: jours.map((d) => ({ day: d })),
    });
    const wb = w.body as Record<string, unknown>;
    if (wb?.ok !== true) {
      throw new Error(`absence de la bouche ${rang + 1} → ${JSON.stringify(wb)}`);
    }
    console.log(`   absences bouche ${rang + 1} → ${jours.join(", ")}`);
  }
}
