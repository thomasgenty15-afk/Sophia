...
const dir=new URL('../2026-09-11-ENQUETE-DEUX-DIRECTIONS/',import.meta.url);
...
await Deno.writeTextFile(new URL('apres-results.json',out),JSON.stringify(results,null,2));
const stages:any[]=[];
const ecarts:any[]=[];const identiques:string[]=[];
  ...
  if(step===2){
   ...
   for(const m of measure){
    ...
    if(m.standard.kcal!==expected.standard_kcal || m.standard.cookedG!==expected.standard_cooked_g || m.standard.densityPer100G!==expected.density){
     ecarts.push({plan:name,jour:m.day,moment:m.slot,titre:m.title,
      avant:{kcal:expected.standard_kcal,g:expected.standard_cooked_g,d:expected.density},
      apres:{kcal:m.standard.kcal,g:m.standard.cookedG,d:m.standard.densityPer100G}});
    } else { identiques.push(`${name} ${m.day}/${m.slot}`); }
   }
  }
...
await Deno.writeTextFile(new URL('apres-stages.json',out),JSON.stringify(stages,null,2));
...
await Deno.writeTextFile(new URL('apres-diagnostic.json',out),JSON.stringify({water:{components:parts,together:weighedReadyGrams(components.flatMap(c=>c.ingredients),index),separate:parts.reduce((n,c)=>n+(c.grams??0),0),factor:gain.measure[5].bounded.factor},food},null,2));
...
await Deno.writeTextFile(new URL('apres-mappings.json',out),JSON.stringify([...mappings.values()],null,2));
...
await Deno.writeTextFile(new URL('apres-fruit-counterfactual.json',out),JSON.stringify(counterfactual,null,2));

// ══════════════════════════════════════════════════════════════════════════
// L'ÉCART AVEC LE JOURNAL ARCHIVÉ — RAPPORTÉ, PAS LEVÉ.
// ══════════════════════════════════════════════════════════════════════════
// ⛔ `rejouer.ts` LÈVE sur le premier écart, et c'est sa raison d'être: il
// garde l'enquête. Ce jumeau-ci ne garde rien, il MESURE — il dit combien
// d'assiettes sur 18 ont changé de mesure après le chantier, et de combien.
console.log('\n=== ÉCART AVEC LE JOURNAL ARCHIVÉ (18 parts standards, après 2e réparation)');
console.log(`identiques: ${identiques.length}/18 · différentes: ${ecarts.length}/18`);
for(const e of ecarts){
 const dk=e.apres.kcal===null||e.avant.kcal===null?'—':(e.apres.kcal-e.avant.kcal).toFixed(0);
 const dg=e.apres.g===null||e.avant.g===null?'—':(e.apres.g-e.avant.g).toFixed(0);
 const dd=e.apres.d===null||e.avant.d===null?'—':(e.apres.d-e.avant.d).toFixed(1);
 console.log(` ${e.plan} ${e.jour}/${e.moment} — ${e.titre}`);
 console.log(`   kcal ${e.avant.kcal} → ${e.apres.kcal} (${dk})  g ${e.avant.g} → ${e.apres.g} (${dg})  d ${e.avant.d} → ${e.apres.d} (${dd})`);
}
await Deno.writeTextFile(new URL('apres-ecarts.json',out),JSON.stringify({identiques,ecarts},null,2));
