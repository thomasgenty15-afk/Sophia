...
import type { RepairUnitDish } from "./plan_repair_unit.ts";
import {
...
function pot(id: string, title: string, term = "courgette"): RepairPlanPreparation {
  return {
    id,
    title,
    servingsMade: 2,
...
function applique(
  ...
  const preparations = new Map<string, RepairPlanPreparation>(
    envelope.preparations.map((p) => [
      String(p.id ?? ""),
      pot(String(p.id ?? ""), String(p.title ?? "")),
    ]),
  );
  return {
    envelope,
...
Deno.test("③ bis — la casserole devenue SANS CONSOMMATEUR est retirée, celle encore tirée reste", () => {
  ...
  const { result } = applique({
    repair: {
      ...
      preparations: [{ id: "p2", title: "Autres légumes" }],
    },
  }, { plan: soloPlan, index: soloIndex, scope: soloScope });
  assertEquals(result.applied, true);
...
Deno.test("⑤ le jour, le moment et le propriétaire écrits par le modèle sont ÉCRASÉS", () => {
  ...
  assertEquals(pont.addressToUnit.get("sun|dinner|zoe"), U_ZOE.unitId);
