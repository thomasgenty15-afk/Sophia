...
const safetyConstraints=householdAllergyConstraints([{id:'qa-allergy',memberId:'qa-child',label:'lait'}],'fr-FR');
...
const rows=[];
for(const term of ['lait de vache','lait sans lactose','lactose-free milk','boisson de riz','lait sans lactose de vache']){
 const ingredient={term,quantity:'200 ml',amount:200,unit:'ml',state:'raw',group:term==='boisson de riz'?'plant_milk':'dairy_milk'};
 ...
 rows.push({term,expected_reject:term!=='boisson de riz',dishes_kept:parsed.dishes.length,shopping_kept:parsed.shopping_list.length,issues:parsed.issues});
}
