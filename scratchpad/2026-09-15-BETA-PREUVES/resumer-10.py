...
M="scratchpad/2026-09-15-BETA-PREUVES/mesure-10"; FX="scratchpad/2026-09-11-CLOTURE/fixtures"; SO="scratchpad/2026-09-11-FIABILITE-RECETTES/sorties-lot-F"
TIRS=sys.argv[1:] or sorted(os.path.basename(f)[:-4] for f in glob.glob(f"{M}/c41-*-p*.txt") if not f.endswith(".figer.txt"))
...
for n in TIRS:
    ...
    out.append({"tir":n,"titre":g.get("titre"),"request_id":g.get("request_id"),"bouches_n":len(dem.get("bouches") or []),
        ...
        "bouches":bouches})
json.dump(out,open("scratchpad/2026-09-15-BETA-PREUVES/mesures-campagne-10.json","w"),ensure_ascii=False,indent=2)
