...
| C15 | Calories : « jamais de chiffres » | **FAUX — le produit en affiche** | `plan/EnergyReadout.tsx:42-45` (`{n} kcal`) sur `/app/plan` et `/app/today`. **MAIS** `profiles.energy_display_enabled` **default false** (`20260812230000:60`) |
| C15b | ⚠️ **PRÉCISION — le compte exact des verrous** | **QUATRE pour afficher, un CINQUIÈME pour la cible** | `energy_gate.ts:232-250` : ① plancher TCA (`restriction_floor`), ② mineur, ③ méthode du coach (`no_counting`), ④ interrupteur élève. La **cinquième** porte (`target_off`, `energy_gate.ts:254+`) ne garde **que la cible**, pas l'affichage. Donc : « quatre verrous décident si on peut allumer les chiffres » est **exact** ; « cinq » ne l'est que si on parle de la cible. Une page qui ne veut pas trancher écrit « une chaîne de gardes » |
