...
export interface DraftEnvelope {
  ...
  draftId: string | null;
  /**
   * ⟳ 2026-09-08 — CE QUE LA PHRASE A FAIT, AVANT QUE LE PLAN NE SE COMPOSE.
   *
   * `null` quand aucune note n'a été lue (un aperçu sans phrase). Sinon
   * l'issue de `keel-read-note-v1`: les lignes DITES à la personne (les mêmes
   * que le chat reçoit), et les compteurs qui distinguent « rien à changer »
   * de « je n'ai pas compris de qui tu parles ». Le dialogue les rend SOUS LE
   * CHAMP: la personne ne doit pas aller chercher dans le chat ce qu'on vient
   * de faire de sa phrase.
   */
  noteOutcome: NoteOutcome | null;
}
...
export function readDraftEnvelope(raw: unknown): DraftEnvelope {
  ...
  return {
    // Le composeur ne lit plus la note: son enveloppe ne porte jamais d'issue.
    // `composeDraft` la pose APRÈS, depuis `readNote`.
    noteOutcome: null,
...
export async function composeDraft(input: ComposeDraftInput): Promise<PlanDraft> {
  // ⛔ LA NOTE D'ABORD, LE PLAN ENSUITE — et jamais la note DANS le plan.
  const noteOutcome = input.note !== null && hasNote(input.note)
    ? await readNote(input.note, input.window)
    : null;
  ...
  return {
    ...
    envelope: { ...readDraftEnvelope(payload), noteOutcome },
  };
}
...
export async function writeFromDraft(
  ...
  // ⚠️ MÊME RÈGLE QU'À L'APERÇU: une phrase posée à l'adoption est lue et
  // appliquée AVANT d'écrire, jamais donnée au composeur. L'adoption rejoue le
  // corps de l'aperçu (`604ce302`), donc la note y arrive rarement — mais
  // quand elle y arrive, elle suit le même chemin.
  if (input.note !== null && hasNote(input.note)) await readNote(input.note, input.window);
