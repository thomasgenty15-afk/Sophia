    ...
    <FunnelShell>
      <div className="space-y-6">
        ...
        <PlanDraftDialog
          ...
          noteOutcome={draft?.envelope.noteOutcome ?? null}
      ...
      <Card>
        ...
        <div className="mt-4 space-y-4">
          {/* ── LES DATES, ET C'EST LA QUESTION QUI MANQUAIT ────────────────
              ...
              ne doit pas exister. */}
          <div className="grid gap-4 sm:grid-cols-2">
            {/* ⚠️ LE `hint` DIT LA SEULE BORNE QUI RESTE. Il a porté pendant
                une heure l'explication du `max` (« passé dimanche les noms se
                répètent »); le `max` est parti, la phrase avec — une aide qui
                décrit une borne retirée est pire qu'une absence d'aide. */}
            <Field
              label={t("setup.request.from")}
              hint={t("plan.window.start_bound")}
