...
interface LoadedMember extends PortionMember {
   ...
   * ce lot.
   *
   * ⚠️ TROIS ÉTATS, comme `mealExtras`: clé absente / `false` / `true`. Voir
   * `parseMemberLight`.
   */
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    //
    // ⚠️ LES EXTRAS ATTEIGNENT LE PROMPT POUR LA PREMIÈRE FOIS. Jusqu'ici
    // « je prends du pain le soir » ne servait qu'à RETRANCHER des kcal de la
    // cible (`extrasFor`); le modèle ne l'a jamais su, et composait donc un
    // plat entier par-dessus. La carte le lui dit, avec la consigne qui va
    // avec: ne le compose pas, elle l'a déjà.
    //
    ...
    // ══════════════════════════════════════════════════════════════════════
    // ⟳ 2026-09-08 — CE BLOC A ÉTÉ HISSÉ ICI, ET LE POURQUOI EST DANS LE
    // BLOC SUIVANT: la densité requise se calcule AVANT le prompt, et elle a
    // besoin de ce que chaque bouche prend à côté du plat.
    //
    // ⚠️ IL ÉTAIT 2 300 LIGNES PLUS BAS, juste avant le dimensionnement. Rien
    // d'autre n'a bougé: il ne lit que `members`, `lineStructures`,
    // `composition` et `issues`, tous posés avant ce point. Effet mesurable
    // unique: `meal_extras_unresolved` entre dans `issues` avant les motifs de
    // voix, où il entrait après. Aucun test n'épingle cet ordre.
    // ══════════════════════════════════════════════════════════════════════
    // ══════════════════════════════════════════════════════════════════
    ...
    for (const m of platedMembers) {
      ...
      // ⚠️ `composition === null` ⇒ ON NE DIT RIEN. Sans référentiel, les
      // extras et le shaker ne se convertissent pas en kcal: la part d'un
      // moment serait celle d'une journée où personne ne boit rien, donc une
      // densité annoncée trop haute. On s'abstient, et on le compte.
      if (composition === null) {
      ...
      // qui déciderait.
      const density = requiredDensityFor({
        mouth: {
          memberId: m.memberId,
          // ⟳ 2026-09-08 — LE CRAN DE PART, SUR LA CIBLE DU JOUR. Sans lui, un
          // « ma mère ne mange pas autant » s'écrit en mémoire et ne déplace
    ...
    const v34CardFacts: Record<
      string,
      { diet: string | null; aside: string[]; requiredDensity: RequiredDensity | null }
    > = {};
    for (const m of platedMembers) {
      const aside: string[] = [];
      const extras = m.mealExtras ?? {};
      for (const slot of Object.keys(extras).sort()) {
        const what = (extras[slot] ?? []).slice().sort();
        if (what.length === 0) continue;
        aside.push(
          `already eats aside at ${slot}: ${what.join(", ")} — do NOT compose it`,
        );
      }
      ...
      result = await generateWithGemini(
        ...
        // et bœuf à un végétarien, sauce soja à un foyer sans gluten).
        {
          ...
          httpTimeoutMs: adoptingDraft
            ...
            : PLAN_COMPOSITION_HTTP_TIMEOUT_MS,
          // ⟳ 2026-09-08 — `high` SUR LA COMPOSITION SEULE. Les relances, plus
          // bas, gardent `PLAN_REASONING_EFFORT` : voir `generation_model.ts`.
          reasoningEffort: PLAN_COMPOSITION_REASONING_EFFORT,
        },
      );
    } catch (error) {
    ...
    if (
      swap.counters.flagrant && !adoptingDraft && strictestRegime !== null &&
      !swapRetrySkipped
    ) {
    ...
    const shadowSizing = (): Record<string, unknown> => {
      ...
      const bucketOf = (m: typeof platedMembers[number]): string => {
        const age = lineBodies.get(m.memberId)?.ageYears ?? null;
      ...
      const dayKcal = { rows: 0, within_5pct: 0, over_5pct: 0 };
      ...
      for (const [i, dish] of meal.dishes.entries()) {
        ...
        const atDish = [...eaters].sort().map((memberId) => {
          ...
          const shared = !p || p.dayKcal === null ? null : slotPlanTargets({
            targetKcal: p.dayKcal,
            coveredSlots: [slot],
            wholeSlots: [
              ...(p.declaredSlots.length > 0 ? p.declaredSlots : composedSlots),
              ...composedSlots,
            ],
            slotExtraKcal: extrasFor(memberId),
          ...
          return {
            ...
            targetKcal: shared?.bySlot.get(slot) ?? null,
            bounds,
          };
        });
      ...
      for (const acc of servedByMouthDay.values()) {
    ...
    const portionSizing = await (async () => {
      ...
      if (sizing.path !== "portion_v1" || composition === null) {
        ...
        splice: { fresh_spliced: 0, pots_spliced: 0, dish_missing: 0, pot_missing: 0, shopping_added: 0, shopping_pruned: 0 },
        skipped_stuck: 0,
            still_out: 0,
            skipped_budget: 0,
          },
      ...
      if (instruction && !adoptingDraft) {
        ...
        try {
          const retryResult = await generateWithGemini(
            built.systemPrompt + household.systemSuffix,
            householdRepairMessage(instruction),
            0.6,
            true,
            [],
            "auto",
      ...
      if (platedMembers.length > 1) {
        ...
        if (askable.length > 0) {
          ...
          const repaired = await runDensityRepair(askable, (k) => ({
          ...
          if (repaired) {
            ...
            const blinded = unmeasAfter > unmeasBefore || degraded;
            if (blinded) {
        ...
        const dedicated = {
          asked: 0,
          accepted: 0,
          rejected: { unparseable: 0, bearer_missing: 0, missing_pot: 0, degraded: 0, unmeasurable: 0 },
          skipped_budget: 0,
          missed_aim: 0,
          still_out: 0,
        };
        const stuckAsks = (() => {
          ...
          for (const { i, memberId, direction } of residualStuck) {
            ...
            const solo = repairDecisionForDish({
              standard: std,
              eaters: [{
                targetKcal: mine.targetKcal,
                bounds: mine.bounds,
                verdict: mine.verdict,
                isMinor: String(mine.bounds.source ?? "").startsWith("minor"),
              }],
            });
            if (solo.ask === null || solo.ask.direction !== direction) continue;
        ...
        if (dedicatedInstruction !== null && !adoptingDraft) {
              ...
              if (append.added.length > 0) {
                {
                  ...
                  const worse = blind || (pB !== null && pA !== null && pA < pB);
                  if (worse) {
                    if (blind) dedicated.rejected.unmeasurable++; else dedicated.rejected.degraded++;
                    restoreSnap();
                    const restored = shadowSizing();
                    const rr = (restored.applyRows ?? []) as EaterRowForApply[];
                    if (rr.length > 0) { rows.length = 0; rows.push(...rr); Object.assign(measured, restored); }
                  } else {
                    dedicated.accepted = append.added.length;
                    ...
                    for (const a of dedicatedAsks) {
                      const row = ((after.rows ?? []) as Record<string, unknown>[]).find((x) =>
                        ...
                        String(x.dish_title ?? "") !== "" && Number(x.factor) > 0 &&
                        append.added.some((d) => d.memberId === a.memberId && d.cell === `${a.day}/${a.slot}`)
                      );
      ...
      const body = lineBodies.get(mouth.memberId) ?? null;
      ...
      const measureDish = (
        ...
        // La cible de CE moment, CE jour — extras et shaker déjà retranchés,
        // moment léger déjà pesé. `slotPlanTargets` fait autorité.
        const shared = dayTarget.kcal === null ? null : slotPlanTargets({
          targetKcal: dayTarget.kcal,
          coveredSlots: [slot],
          wholeSlots: [
            ...(declaredSlots.length > 0 ? declaredSlots : composedSlots),
            ...composedSlots,
          ],
          slotExtraKcal: extrasFor(mouth.memberId),
          lightSlots,
          slotFixedKcal: fixedBySlotPerDay.get(day) ?? null,
        });
        const slotTarget = shared?.bySlot.get(slot) ?? null;
        const bounds = plateBoundsFor({
          ageYears: body?.ageYears ?? null,
          slot,
          appetite: body?.appetite ?? null,
        ...
        return { slot, day, standard, shared, slotTarget, bounds, sized, bounded };
      };
      ...
      if (await runDensityRepair(outOfBounds, (k) => ({
      ...
      measured.forEach((m, i) => {
        ...
        if (shared?.extrasFloored) extrasFloored++;
        if (shared?.fixedFloored) fixedFloored++;
    ...
    console.log(JSON.stringify({
      ...
      density: { ...densityCounters, by_slot: densityBySlot },
      ...portionSizing,
    }));
