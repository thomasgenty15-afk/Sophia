...
const SKIP_BLURBS: Readonly<Record<DraftNoteSkipReason, string>> = {
  ...
  degree:
    "\"too long to cook\", \"too complicated\", \"not varied enough\" — a DEGREE ABOUT THE WORK, not about a share. It is asked in a closed question after the plan, with the person in front of them, and it is a SETTING they can also change on their own screen. ⛔ NOT \"too much\" or \"not enough\" on someone's plate: that is a share, and it has its own drawer (4)",
  setting:
...
export const DRAFT_NOTE_CLASSIFY_SYSTEM_PROMPT = [
  ...
  "",
  '{ "preferences": [ ... ], "next_plan": [ ... ], "notes": [ ... ], "portions": [ ... ], "skipped": [ ... ], "clarify": [ ... ], "safety": [ ... ] }',
  "",
  ...
  }: those are not preferences, they go in "skipped" (see 5). NEVER a share that is too big or too small either — that has its own drawer (4),`,
  ...
  '  "member_id": see WHO below. ⛔ AND HERE IT MATTERS MORE THAN ANYWHERE: a share belongs to ONE person. When the note does not say clearly whose plate it is about, do NOT guess and do NOT use null — put it in "clarify" with "about": "who". Taking food off the whole table because one person said they eat less is the exact harm this drawer exists to avoid,',
  "}",
  ...
  '  ⛔ NOT a remark about the plan being long to cook, complicated, or repetitive. Those are degrees about the WORK, they have no drawer, and they go in "skipped" (5).',
  "",
  // ── CE QU'ON NE RANGE PAS — dit, et compté ──────────────────────────────
  '5. "skipped" — what you read and deliberately did NOT file, one entry each, so it can be counted. EVERY thing the note says that you did not file in 1, 2, 3 or 4 MUST appear here, once: never return five empty lists without saying why. Each entry is exactly:',
  ...
  '  ⛔ NEVER use "skipped" for something you could not attribute to a person or to a food. That is not a thing you chose not to file — it is a thing you could not file yet, and it goes in "clarify" (6).',
  "",
  ...
  '6. "clarify" — one thing the note says that you could NOT file because you do not know WHO it is about, WHICH food it means, or — for a diet-like rule — whether it holds ALWAYS or only SOMETIMES. One entry per thing. Everything you put here is filed NOWHERE ELSE — not in 1, 2, 3, 4, 5 or "safety". They will be asked, once, with buttons; if they do not answer, nothing is kept. Each entry is exactly:',
  "{",
  ...
  return {
    ...
    classification: {
      proposed: preferences.proposed + notes.proposed + nextPlan.proposed +
        portions.proposed + clarify.proposed,
