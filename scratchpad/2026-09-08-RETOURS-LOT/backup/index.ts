  ...
  try {
    ...
    const shadowSizing = (): Record<string, unknown> => {
      ...
      for (const [i, dish] of meal.dishes.entries()) {
        ...
        for (const r of rows) {
          rowsByDish.push({ dishIndex: i, memberId: r.memberId, verdict: r.verdict });
        }
    ...
    const portionSizing = await (async () => {
      ...
      if (sizing.path !== "portion_v1" || composition === null) {
        ...
        return {
          ...
          repairs: {
            asked: 0,
            accepted: 0,
            rejected: { belt: 0, unparseable: 0, no_cell: 0, title_changed: 0, unmeasurable: 0, missing_pot: 0, dedicated_lost: 0, degraded: 0 },
            still_out: 0,
            skipped_budget: 0,
          },
          rows,
      ...
      for (const { i, ask } of outOfBounds.slice(0, REPAIR_MAX_DISHES_PER_PLAN)) {
      ...
      if (instruction && !adoptingDraft) {
        ...
        try {
          ...
          if (typeof retryResult !== "string") {
            ...
            const porteursAttendus = new Map<string, Set<string>>();
            for (const d of meal.dishes) {
              const k = `${d.day ?? ""}/${d.slot ?? ""}`;
              const set = porteursAttendus.get(k) ?? new Set<string>();
              if (d.memberId) set.add(String(d.memberId));
              porteursAttendus.set(k, set);
            }
            // ⚠️ LE FILTRE PORTE SUR LA CLÉ DE CASE, parce que c'est l'unité que
            // `mergeRetryCells` fusionne. Retirer « le plat » ne veut rien dire
            // ici: on garde ou on jette la case entière.
            const dedicatedLost = new Set<string>();
            for (const k of keptCells) {
              const attendus = porteursAttendus.get(k) ?? new Set<string>();
              if (attendus.size === 0) continue;
              const rendus = new Set(
                retried.dishes
                  .filter((d) => `${d.day ?? ""}/${d.slot ?? ""}` === k && d.memberId)
                  .map((d) => String(d.memberId)),
              );
              for (const id of attendus) {
                if (!rendus.has(id)) dedicatedLost.add(k);
              }
            }
            if (dedicatedLost.size > 0) {
              repairs.rejected.dedicated_lost += dedicatedLost.size;
              for (let n = keptCells.length - 1; n >= 0; n--) {
                if (dedicatedLost.has(keptCells[n])) keptCells.splice(n, 1);
              }
              console.warn(JSON.stringify({
                tag: "keel.household_meal.density_repair_dedicated_lost",
                user_id: userId,
                request_id: requestId,
                cells_dropped: dedicatedLost.size,
                cells_kept: keptCells.length,
              }));
            }
            ...
            repairs.rejected.title_changed = Math.max(
              0,
              askedDishes - keptCells.length - repairs.rejected.dedicated_lost,
            );
            ...
            const merge = mergeRetryCells({ base: meal, retry: retried, cells: keptCells });
            if (merge.cells.length === 0) {
              repairs.rejected.no_cell++;
            } else {
              // ⛔ LA CASSEROLE PARTAGÉE EST DÉFOURCHÉE AVANT TOUT LE RESTE.
              // Mesuré sur `plan-L6` (une seule bouche): `prep_rice` ET
              // `prep_rice__r` citées, cuites toutes les deux dans la même
              // session. `mergeRetryCells` a raison de renommer — un plat non
              // repris cite encore l'ancienne recette —, mais une casserole
              // qu'on a jugée RÉÉCRIVABLE vaut pour tous ses mangeurs: la
              // garder en double fait cuire deux fois ce qu'on a demandé de
              // réécrire une fois. Personne ne le comptait.
              const unfork = unforkReworkedPots(merge, reworkable);
              // ══════════════════════════════════════════════════════════
              // ⛔ UN PLAN QUI CITE UNE CASSEROLE ABSENTE N'EST PAS FUSIONNÉ
              // ══════════════════════════════════════════════════════════
              //
              // ⛔ MESURÉ AU TIR `IDENTITE` DU 2026-09-08: après deux
              // réparations acceptées, quatre plats sont devenus IMMESURABLES —
              // ils citaient un `preparationId` absent de `meal.preparations`.
              // Les deux chemins qui peuvent le produire sont ici: la fusion
              // renomme les casseroles réécrites, et le défourchage abandonne
              // (`continue`) quand il ne retrouve pas la version réécrite.
              //
              // ⚠️ C'EST UN INVARIANT, PAS UNE HEURISTIQUE. Un plat dont on ne
              // sait pas retrouver la recette n'a pas de contenu connu: le
              // moteur ne peut ni le peser, ni l'acheter, ni le cuire. Le
              // vérifier ICI protège LES DEUX appelants — la table par sa
              // seconde mesure, le solo qui n'en a aucune.
              const potIds = new Set(unfork.meal.preparations.map((x) => String(x.id)));
              const orphelines = [
                ...new Set(
                  unfork.meal.dishes.flatMap((d) =>
                    (d.uses ?? [])
                      .map((u) => String(u?.preparationId ?? ""))
                      .filter((id) => id.length > 0 && !potIds.has(id))
                  ),
                ),
              ];
              if (orphelines.length > 0) {
                repairs.rejected.missing_pot += orphelines.length;
                console.warn(JSON.stringify({
                  tag: "keel.household_meal.density_repair_orphan_pot",
                  user_id: userId,
                  request_id: requestId,
                  orphan_pots: orphelines.length,
                  unforked: unfork.unforked.length,
                  forked: unfork.forked.length,
                }));
                // ⛔ `meal` N'EST PAS TOUCHÉ: le plan d'avant reste en place, et
                // la réparation n'a simplement pas eu lieu.
                return merged;
              }
              repairs.pot_unforked = unfork.unforked.length;
              // ⚠️ `pot_forked` DIT QUE LA CONSIGNE N'A PAS ÉTÉ TENUE: le modèle
              // a modifié une casserole qu'on lui avait nommée intouchable. On
              // garde les deux versions (les autres plats gardent la leur) et
              // on le compte, pour que le tir suivant le sache.
              repairs.pot_forked = unfork.forked.length;
              meal = unfork.meal;
              meal.empty_slots = slotsStillEmpty(meal.empty_slots, merge.cells);
              mealSourceText = retryResult;
              // ⛔ ON RE-MESURE TOUT, pas seulement les cellules réparées: une
              // casserole partagée qui change de recette déplace la part de
              // CHAQUE plat qui la tire, y compris ceux qu'on n'a pas nommés.
              // ⛔ LA RE-MESURE APPARTIENT À L'APPELANT. Le solo remesure par
              // `measureDish`, la table par une seconde passe de `shadowSizing`:
              // les deux lisent le MÊME plan réparé, chacune avec son
              // arithmétique. La mettre ici obligerait la fonction à connaître
              // les deux, c'est-à-dire à en devenir une troisième.
              repairs.accepted = merge.cells.length;
              merged = true;
            }
          }
        } catch (error) {
      ...
      if (platedMembers.length > 1) {
        ...
        if (outOfBoundsN.length > 0) {
          ...
          const repaired = await runDensityRepair(outOfBoundsN, (k) => ({
        ...
        // réparation a pu changer qui mange quoi, et donc ce qui est gelé.
        {
          ...
          for (const [i, verdicts] of verdictsFinal) {
            for (const [, verdict] of verdicts) {
              ...
              if (!freshOk && !potsOk) repairs.residual_stuck++;
            }
          }
        }
        // ══════════════════════════════════════════════════════════════
