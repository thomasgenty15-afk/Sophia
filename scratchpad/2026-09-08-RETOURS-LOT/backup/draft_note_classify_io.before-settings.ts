...
export async function classifyAndPersistDraftNote(args: {
  ...
  const appetite = { asked: 0, moved: 0, at_edge: 0, failed: 0 };
  /**
   * ⛔ LES DEUX CRANS, PAS LE SEUL NOUVEAU. La phrase que la personne lira est
   * « Appétit : de normal à petit »: sans le `previous`, l'accusé annoncerait
   * une valeur sans dire d'où elle vient — et un cran qui n'a pas bougé
   * (l'échelle au bout) serait indiscernable d'un cran qui a bougé.
   *
   * ⚠️ ILS VIENNENT DE LA RPC, PAS D'UNE RELECTURE. Relire la fiche après coup
   * serait une seconde lecture, donc une seconde chance de lire l'état d'après
   * l'écriture de quelqu'un d'autre.
   */
  const appetiteMoved: Array<{
    move: PortionMove;
    previous: string;
    next: string;
  }> = [];
  for (const move of classification.portions.moves) {
    ...
    try {
      ...
      if (error || row.ok !== true) {
        // ⚠️ « AU BOUT » N'EST PAS UN ÉCHEC, et les confondre effacerait le seul
        // cas où la personne mérite une phrase: elle a redemandé moins à
        // quelqu'un qui est déjà au plus bas de l'échelle.
        const reason = String(row.reason ?? "");
        if (reason === "at_floor" || reason === "at_ceiling") appetite.at_edge += 1;
        else appetite.failed += 1;
        continue;
      }
      appetite.moved += 1;
      appetiteMoved.push({
      ...
      // ⛔ ON NE LÈVE PAS. Le plan est déjà écrit; une part qui ne se déplace
      // pas ne doit pas emporter la note entière, qui a peut-être rangé trois
      // préférences justes.
      appetite.failed += 1;
    }
  }
  ...
  if (write.ok) {
    ...
    // ⛔ HORS DU `if (write.durableWritten > 0)`: une part ne passe PAS par ce
    // port, donc son annonce ne doit pas dépendre de ce qu'il a écrit. Les
    // lier ferait taire un appétit déplacé parce qu'aucune préférence ne l'a
    // été — deux écritures indépendantes, deux accusés indépendants.
    for (const moved of appetiteMoved) {
      // ⛔ PAR `settingRecapLine`, PAS UNE PHRASE ÉCRITE ICI. Ce module est du
      // Deno et ne connaît aucun libellé; `memory_recap` est le seul endroit
      // qui dit un champ dans les deux langues, et il tient déjà l'échelle de
      // l'appétit. Une seconde phrase ici en ferait deux qui divergeraient.
      const line = settingRecapLine(
        { field: "appetite", previous: moved.previous, next: moved.next },
        language,
      );
      ...
      announced.push({
        text: line,
        until: null,
        // ⚠️ UN RÉGLAGE, pas un goût: on a déplacé un cran, on n'a pas appris
        // quelque chose d'elle. Même règle que `keel-plan-feedback-v1`.
        kind: "setting",
        who: whoOf(memberSubject(moved.move.memberId) ?? ""),
      });
    }
