...
echo "══════ UN SEUL MOMENT — $TZ_CASE, il y est $(psqlq -c "select to_char(now() at time zone '$TZ_CASE','HH24:MI');")"
echo "   attendu: AUCUN retrait (le dîner est devant), 3 jours, et le premier"
echo "            jour ne porte QUE le dîner"
...
OUT="$EVAL_DIR/plan-UNSEUL-$(date +%Y%m%d-%H%M%S).json"
