...
export function planValidationRecord(args: {
  ...
  const notApplicable = ([
    ...
    // ⟳ 2026-09-13 · LOT 1 — LA CASE D'UNE BOUCHE SANS CIBLE. Une portion y
    // existe (la part de recette), et il n'y a rien à comparer: le contrat
    // d'énergie s'est abstenu pour une raison protégée (âge inconnu, corps
    // absent, ceinture illisible). Avant ce lot elle était comptée comme un
    // contrôle d'énergie RÉUSSI — six cases sur `perte-l1age`.
    ["cell_energy_no_target", checked.cell_energy_no_target],
  ] as const)
    .filter(([, n]) => n > 0)
