...
| **12** | ⛔ **La fixture porte 4 corps et la ligne `student_goals` du maître, AU-DELÀ des six conditions** | `V0-C` | Sans la ligne d'objectif, `generate-household-meal-v1` rend **`goal_required` (409)** et `V0-D` **ne démarre pas**. Sans les 4 corps, `householdAppetite` rend `null` *(il est tout-ou-rien)* et `V0-D` mesurerait **le repli sur le compte de têtes** en croyant mesurer l'appétit. ⇒ une fixture strictement limitée aux six conditions **aurait été inutilisable par le lot qu'elle existe pour servir** | **gratuit** — RPC idempotentes et un `upsert` `ignoreDuplicates`. Les retirer ne libère rien et casse `V0-D` | |
