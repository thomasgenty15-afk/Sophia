...
| **migration** | non |
| **mesure AVANT** | **n'existe pas.** Ce qu'il faut compter : combien de fonctions, entre un parseur de réponse de modèle et une écriture, **énumèrent leurs clés** au lieu d'étaler ou de typer exhaustivement · combien de ces énumérations sont **plus courtes** que le type qu'elles prétendent produire · et **lesquelles perdent un champ que le modèle déclare** |
| **direction** | le chiffre existe ; ⛔ **et il n'est PAS un correctif** — corriger les trois connues ne dit rien des autres |
| **mesure APRÈS** | ⛔ **la question à trancher, CHIFFRÉE, et elle appartient au propriétaire** : à partir de combien d'instances une **règle de dépôt** est-elle moins chère que sa redécouverte ? `X2′` a posé la même question pour les ligatures et l'a tranchée sur **8/8 = 100 %** contre *« une règle visant le digramme serait fausse 83,8 % du temps »*. **Ici on a 3 instances en un jour et aucun dénominateur.** |
