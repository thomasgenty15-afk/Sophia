...
| **coût** | **une commande** |
| **risque** | ⛔ ⟳ **CE QUE ÇA RÉVÈLE, ET QUI GOUVERNE TOUTES LES VAGUES SUIVANTES : la doctrine servie est celle du COACH MAISON.** `KEEL Discovery`, version 1, `content_locale = en-US`, **6 croyances génériques**, 2 arbitrages, **`foods` VIDE des deux côtés**, et ⛔ **`table_scope_beliefs: 0`** — aucune croyance ciblée par objectif de table n'a été tirée, **alors que la table porte `fat_loss` + `muscle_gain`**. Le correctif `LOT C ①` du 2026-08-19 est branché et **n'a rien à servir**, parce que la doctrine maison n'a **aucune variante par objectif**. ⇒ **toute mesure de vague sur cette fixture mesure un foyer SANS opinion de coach**, et il faut le savoir **avant** de lire un grammage |

## ⟳ V0-C-quater — `regime_belt.mouths` compte les régimes DÉCLARÉS, pas les bouches à table *(fiche NEUVE)*
...
| **direction** | ⛔ **le second régime ne doit être NI plus strict NI emboîté dans le végane** — `dietDiverges` sort `false` dès que `exclusionCount(own) >= exclusionCount(strictest)`. **Un `vegetarian` ou un `pescatarian` sur Camille ou Yanis ouvre la divergence ; un second `vegan` ne l'ouvrirait PAS.** |
| **mesure APRÈS** | `regime_belt.mouths >= 2` sur un plan neuf, **et** `checked = kept + refused` tenu |
| **armé par** | ⛔ **une génération, et il en faut une** — ce seuil ne se lit pas dans le code. **À imputer au plafond de la vague 1, pas au reliquat de la vague 0** |
| **coût** | **un petit lot + 1 génération** |
| **risque** | ⚠️ **Elle déplace la `mesure AVANT` de `L26`, `L9bis` et `D3′`.** Et ⛔ **elle change ce que `V0-D` a mesuré** : le plan du 21/08 restera **le seul plan foyer entièrement végane du corpus** |
...
| **pourquoi** | ⟳ **Mesuré sur le plan neuf** : `portion_note` portant un grammage passe de **181/340 à 181/344** — le dénominateur monte de 4, **le numérateur ne bouge pas**. Les 4 notes du plan neuf sont *« Mettre tous les composants ensemble sur l'assiette »* : **zéro chiffre, et c'est CORRECT** — v4 a mis le gramme **dans le bac**, pas dans la phrase *(cicatrice « la phrase de table contredit le couvercle »)*. ⇒ ⛔ **le compteur #8 va DESCENDRE à chaque plan v4, et sa baisse se lira comme une régression** alors qu'elle est la réussite de `L6′`. |
| **dépend de** | `V0-E′` |
...
| **mesure APRÈS** | ⟳ **avec les DEUX populations** : `belt{severity, bit, passed}`. Et un plan **sans contrainte** sort **byte-identique**. ⛔ ⟳ **~~Non couvertes : 0~~ — CE SEUIL EST INATTEIGNABLE TEL QU'ÉCRIT, mesuré le 2026-08-21 (`Q-S2`).** `safetyConstraintTokens` (`safety_constraints.ts:526-534`) ne rend **que** `allergenRef`, `substanceRef`, `medicationClass` — **jamais `dietRef`**. Les **8** des 14 `strict` qui sont `kind='diet'` *(vegan 2, vegetarian 3, pescatarian 3)* **n'entrent pas sous la ceinture même élargie**, et **c'est voulu** : c'est la cicatrice `diabetes`. ⇒ **le seuil se lit sur 6, pas sur 14** : `dairy`, `lactose`, `gluten`, `fructose`, `fruits_de_mer`, `mustard`. **Seuils : 6/6 couvertes ; `passed` non nul ; byte-identité prouvée** |
...
| **risque** | ⛔ ⟳ **`applyKeelOutputLocks` est importé par `sophia-brain/router/run.ts:292`** : élargir la sévérité fait mordre le verrou **dans la conversation**, sur une intolérance au lactose. **Même famille que le précédent `diabetes`.** Le lot doit décider explicitement s'il élargit **les trois** points d'entrée ou seulement les générateurs  ⛔ ⟳ **ET LE VRAI COÛT DU LOT EST AILLEURS, mesuré le 2026-08-21 : il n'existe qu'UN SEUL texte de repli.** `MEDICAL_BLOCK_FALLBACK_EN` (`keel_output_locks.ts:153`) — *« it touches something medical … that is one to put to a doctor »* — **remplace le message entier** (`:363`). Servi à un intolérant au lactose à qui on venait de parler de yaourt, il est **faux 100 % des fois où il sort**. *(La seule constante sœur, `DOCTRINE_BLOCK_FALLBACK_EN`, appartient à la branche doctrine, évaluée après.)* ⇒ **`S2` doit livrer un second texte, ou il dégrade la conversation qu'il prétend protéger.** ⚠️ Et `allergen_bridge.ts:115` porte **déjà** `{medical, strict}`, via `plan_question/swap_resolver.ts:93` : `strict` **bloque déjà un échange de plat**, jamais le texte visible. `S2` **referme une divergence existante**, il n'en crée pas une. ⚠️ Enfin `:408` **n'est pas un filtre de ceinture** mais un booléen de prompt, et `:541` (`medicalConstraintTokens`) a **zéro appelant de production** — prouvé, commentaires et tests écartés |
...
| **mesure AVANT** | mineurs portant `fat_loss`/`muscle_gain` : ~~**2**~~ ⟳ **3 depuis le 2026-08-21** — la troisième est **la fixture `V0-C`** (Anouk, `muscle_gain`), **pas une personne**, et `S4` ne doit pas la « migrer » avec les deux autres. ⟳ **Et les quatre surfaces d'écriture rendent `ok:true`**, mesuré |
...
| **mesure APRÈS** | ⟳ ~~**trois cas SQL**~~ ⛔ **QUATRE cas SQL**, en transaction `rollback`, parce que quatre surfaces acceptent aujourd'hui : **A** `add_member(date_mineur, objectif)` ⇒ refus · **B** objectif posé avant la date, puis date de mineur ⇒ refus ou effacement nommé · **C** `set_member_goal` sur bouche déjà mineure ⇒ refus · **D** `set_member_target(cible, rythme)` sur bouche déjà mineure ⇒ refus · **plus le cas qui PASSE** : objectif sur un majeur ⇒ accepté. **Seuil : 5/5** |
...
| **direction** | ⛔ **écrite d'avance : ce n'est PAS un défaut de modèle, c'est un défaut de CONSIGNE.** Le brief lui dit à la fois *« Anouk porte son plat »* *(`dish_bearing`, via `dietDiverges` sur ses `servingDemands` de `muscle_gain`)* **et ne lui interdit nulle part de la nommer sur le bac commun** |
