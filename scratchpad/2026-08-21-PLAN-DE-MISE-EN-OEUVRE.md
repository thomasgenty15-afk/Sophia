...
## ⛔ L-1-b — LA QUANTITÉ EST ÉCRITE, EN TOUTES LETTRES, ET L'ÉCRITURE N'EN GARDE QUE LA MOITIÉ QUI MANQUE  ✅ **LIVRÉ le 2026-08-22, ⛔ 1 SEUIL SUR 3 SUR LE DÉNOMINATEUR [A], ET LE TROISIÈME EST ARITHMÉTIQUEMENT INATTEIGNABLE** *(fiche NEUVE, ouverte par la contre-épreuve de `L2-lang` et confirmée par `L-1`)*
...
| **armé par** | ⟳ **LIVRÉ** : **quatre gardes qui rougissent (`rc=1`)**, dans le script — ⓐ **une ligne à libellé cuit SANS décision** *(c'est le « test qui refuse qu'une ligne entre dans la liste sans décision » de la fiche, et il travaille sur le référentiel VIVANT : une ligne neuve d'un import CIQUAL ne peut pas entrer en silence)* ; ⓑ une décision sur un slug absent ; ⓒ une décision **périmée** *(la base n'est ni « avant » ni « après »)* ; ⓓ **le patch en mémoire déplace exactement le nombre de lignes que la liste annonce** — et **quatre gardes de plus en SQL, dans la migration** : la garde d'ENTRÉE *(un voisin a-t-il déjà bougé une de ces 57 ?)*, les 57 sont `neutral`, **les 15 sèches n'ont pas bougé**, la distribution finale des six classes est exacte, et **le seuil ① lui-même est ré-exécuté en SQL**. ⛔ **SEPT MUTATIONS, SEPT ROUGES**, copie hors dépôt vérifiée NON MUTÉE **en premier** *(sortie strictement identique au dépôt, dry-run `UPDATE 11 + UPDATE 46`)* et **re-vérifiée à la fin** : `scratchpad/2026-08-22-L-C-journal-de-mutation.txt`. ⚠️ **Et la mutation ⑤ a trouvé une garde MANQUANTE** : `indexAvecDecisions` rendu no-op sortait `rc=0` avec un tableau de témoins entièrement à ×1,00 — **un patch désarmé passait pour un patch vert**. La garde ⓓ a été ajoutée pour ça, la copie re-synchronisée, la non mutée re-vérifiée, et la mutation re-jouée. |
