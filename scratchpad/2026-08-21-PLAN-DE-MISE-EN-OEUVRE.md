...
| ⛔ **provenance de la mesure** | ⟳ ⛔ **CES CHIFFRES NE SONT PAS MESURÉS SUR `HEAD`.** L'arbre de travail porte `_shared/keel/food_composition.ts` à **+404 lignes** de `HEAD` — le **volet ① de `L19b`**, livré exprès dans l'arbre et non commité. C'est **le module qui résout les ingrédients**, pas un détail d'environnement. Le volet ① retire `complet`/`complete`/`complets`/`completes` des réductions : un terme en `complet` **ne résout plus RIEN** au lieu d'atteindre le **raffiné**. ⛔ **Sur l'épreuve du nom nu c'est un progrès ; sur CE compteur c'est un coût** — la porte est binaire, donc une abstention éteint le plat, puis la journée. **Les 16 occurrences de `pain complet` (×9) et `pain complet grille` (×7) en tête des termes bloquants français SONT cet effet** : elles étaient calculables (à tort) avant. ⇒ ① reproduire ces chiffres exige **le même arbre de travail** ; ② au moment des dix générations, `food_composition.ts` doit être **dans le même état pour les deux bras** — la garde du `prompt_version` garde la **consigne**, pas le **référentiel** |
