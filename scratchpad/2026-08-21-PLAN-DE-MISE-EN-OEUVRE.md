...
## ⟳ S1c — le plancher de MALADIE porte la même blessure *(fiche NEUVE, ouverte par `S1`)*
...
| **migration** | non |
| **mesure AVANT** | ⟳ **4/4 formes mortes sous ligature** *(sortie datée du 2026-08-22 01:30:04)* |
| **direction** | 4/4 mordent sous ligature · **aucune régression sur le digramme** · un message neutre reste `null` |
| **mesure APRÈS** | balayage **sur la table `MEDICAL_CONDITION_SURFACE_FORMS`**, **avec assertion de cardinalité**. **Seuil : 100 % des formes concernées, et le compte des formes concernées est > 0** |
| **armé par** | ⛔ **le trépied de `S1`, recopié tel quel** : un cas qui **MORD** · un cas qui **PASSE** · un balayage **qui compte ses cas**. **Plus la mutation**, sur copie hors dépôt, **non mutée vérifiée d'abord** |
| **coût** | **une constante** |
| **risque** | ⚠️ ⛔ **Le vrai sujet n'est pas cette ligne, c'est que le motif est dans SEPT modules et que DEUX l'ont déjà.** Sans repli : `body_measure_floor`, `hunger_signal`, `daily_recommendation`, `slot_from_message`, `grocery_waves`, `planned_dish_match`, `medical_condition_floor`. Avec : `meal_declaration_floor`, `food_composition`. **La divergence est DÉJÀ installée.** Une fiche par module referme **un huitième** à chaque fois ⇒ **décider s'il faut une RÈGLE DE DÉPÔT** *(famille `X2′`)* **plutôt que huit correctifs est un arbitrage, pas un geste** |
