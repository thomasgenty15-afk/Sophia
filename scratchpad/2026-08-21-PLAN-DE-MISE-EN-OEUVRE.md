...
| **bloque** | le pilote payant |
| **fichiers** | ⟳ ⛔ **TROIS ouvreurs de déficit, pas deux — la fiche oubliait LE PLUS VIVANT.** ✅ **COMMITÉS** : `condition_energy_gate.ts` *(neuf, le module pur, `deno check` rc=0 seul)* · son test *(neuf)* · ⛔ **`mouth_anchor.ts` — l'ancre absolue, ABSENTE de la fiche, et c'est ELLE qui REMPLACE le facteur relatif quand elle tire** · `medical_condition_floor.ts` *(`breastfeeding` sort de `pregnancy`)*. ⛔ **NON COMMITÉS** : `household_portions.ts` *(portait **+1 532/−188** d'une autre session)*, les deux générateurs, 4 tests. ⟳ ⛔ **`meal_envelope.ts` et `weight_pace.ts` NE SONT PAS TOUCHÉS** *(§⑨ n° 42 et n° 43)* |
