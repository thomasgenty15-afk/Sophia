...
function verdictOf(i: CompositionInput): { resolved: boolean; weighed: boolean } {
  ...
  return { resolved: true, weighed: g !== null };
}

type Line = { norm: string; lang: Lang; lane: string; live: boolean; source: "dish" | "preparation"; resolved: boolean; weighed: boolean };
const lines: Line[] = [];
type DayKey = string;
const days = new Map<DayKey, { lane: string; live: boolean; n: number; allResolved: boolean; allWeighed: boolean }>();
...
for (const m of meals) {
  ...
  for (const p of preps) for (const i of p.ingredients) {
    const v = verdictOf(i);
    lines.push({ norm: normalizeTerm(i.term), lang: langOf(normalizeTerm(i.term)), lane, live, source: "preparation", ...v });
  }
  for (const d of (Array.isArray(m.dishes) ? m.dishes : [])) {
    for (const i of (Array.isArray(d?.ingredients) ? d.ingredients : []).map(asInput).filter((i: CompositionInput) => i.term)) {
      const v = verdictOf(i);
      lines.push({ norm: normalizeTerm(i.term), lang: langOf(normalizeTerm(i.term)), lane, live, source: "dish", ...v });
    }
  }
  ...
  for (const d of (Array.isArray(m.dishes) ? m.dishes : [])) {
    ...
    const e = days.get(key) ?? { lane, live, n: 0, allResolved: true, allWeighed: true };
...
console.log("\n═══ LA JOURNÉE ═══");
for (const [scope, live] of [["TOUS LES PLANS", false], ["PLANS VIVANTS SEULEMENT (retired_at is null)", true]] as [string, boolean][]) {
console.log(`\n### ${scope}`);
for (const lane of ["personal", "household"]) {
  const ds = [...days.values()].filter((d) => d.lane === lane && d.n > 0 && (!live || d.live));
  const ns = ds.map((d) => d.n);
  const ll = lines.filter((l) => l.lane === lane && (!live || l.live));
