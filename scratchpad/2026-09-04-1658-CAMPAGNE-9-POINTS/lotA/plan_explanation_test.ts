...
Deno.test("③ AUCUNE CULPABILISATION — la MÊME porte que la rationale", () => {
  ...
  // une phrase que `findGuiltTripping` attrape.
  const out = gate(["You said you would cook, and you didn't."]);
  assertEquals(out.refused, "guilt_tripping");
...
Deno.test("⚠️ LE MILLÉSIME DU PROMPT DIT CE LOT", () => {
  ...
  assertEquals(HOUSEHOLD_PROMPT_VERSION, "v26_the_plan_says_what_it_weighed");
});
