...
- **Statut:** `fix_applied` — chantier V2-F (2026-07-07, décision actée : compression incluse) : le prompt companion a été compressé (~400 chars de reformulations, AUCUNE règle supprimée, ancres du test de contrat conservées) pour loger la règle « Pont post-détresse : si ta dernière réponse était du soutien face à un creux, jamais de réponse 100% transactionnelle ce tour-ci — exécute la demande avec une vraie phrase de pont (pas juste un emoji) ». La même exigence est portée dans la GUIDANCE de commit (`outcomeGuidance` committed, `direct_effect_local_context.ts`) — c'est elle que le composeur suit sur les tours d'effet, la règle générique seule laissait passer des confirmations sèches. Budget < 13000 vérifié, contrat companion 15/15. **Probes live** (Nina, ×2) : creux medium → tour suivant « mets-moi un rappel demain 9h » → committed + vraie phrase de pont (« vu le creux que tu traverses, je garde ça simple »).
- **Fix reference:** —
