...
| R6 | **Abstention avant erreur** : résolution < 80 %, ou un seul ingrédient dense dont l'énergie manque — **non résolu OU résolu sans grammes** ⇒ `not_computable` | Une matière grasse absente de la somme déplace l'énergie d'un plat de plusieurs dizaines de pour cent. Un verdict rendu sur 95 % des ingrédients mais sans l'huile est plus dangereux qu'une abstention, parce qu'il a l'air d'un résultat. La seconde branche manquait : « a drizzle of olive oil » se résout, compte comme connu, et ne pèse rien — 82 occurrences sur 80 générations (2026-08-12). La résolution, elle, compte les termes **connus** (`coverage`), pas les termes **pesés** : le second ratio s'abstient sur du sel. |
| R6bis | Le verdict porte sur les plats **préparations pliées dedans**, au prorata `servings / servingsMade` | En batch cooking, le plat ne porte pas ses ingrédients — il cite une préparation. Mesuré le 2026-08-12 : **41 % de l'énergie et 51 % de la protéine** vivaient hors des plats. Le verdict rendait `below`/`under` sur des plans à 99 % de leur cible, et la boucle de correction y dépensait son unique relance. Le pliage est une fonction **pure et exportée** (`foldPreparationsIntoDishes`) : en fermeture chez l'appelant, il a perdu la moitié du plan sans qu'une assertion ne bouge. |
...
| R15 | Quand le **plancher protéine** et le **plafond énergie** ne peuvent pas être tenus ensemble, **la protéine gagne** — et le dépassement est toléré jusqu'à `ENERGY_DIRECTION_MARGIN` | ⟳ 2026-09-04, mesuré. `scaleFactorsFor` sert la protéine d'abord (« le plancher est un plancher : il ne se négocie pas »), puis comble l'énergie avec le reste. Quand la protéine monte, le `remaining` laissé au `other` devient négatif et `other` s'écrase à `MIN_SCALE` : le total dépasse alors le plafond — **2 444 kcal pour 2 426** sur le banc de `portion_scaling_test.ts`. ⛔ **CE N'EST PAS RÉPARABLE PAR UN SECOND RABOT**, et c'est la raison d'être de cette ligne : `other` est déjà au plancher, et la seule autre variable est la protéine. Le prochain qui verra le dépassement le « corrigera » par un rabot supplémentaire — et il violera R-protéine sans s'en apercevoir. La tolérance est la **bande de jugement** (×1,10), celle qui décide `above` : elle laisse passer les quelques pour cent d'un croisement de contraintes, et attrape le **×1,34 mesuré le 2026-09-03**. |
...
| Un ingrédient dense **résolu mais sans grammes** (« a drizzle of olive oil »), résolution à 100 % | `not_computable` quand même (R6). C'est la branche qui manquait, et elle coûtait plus cher que l'autre : l'aliment compte comme connu, donc rien ne signalait la perte. |
| Un plat qui n'a **que** des `uses` et aucun ingrédient propre | Il porte la masse de ses préparations au prorata (R6bis). Sans le pliage, il se lisait comme une assiette vide. |
| Une `uses` vers une préparation absente du plan | Ignorée, jamais devinée. Le plat garde ses propres ingrédients. |
