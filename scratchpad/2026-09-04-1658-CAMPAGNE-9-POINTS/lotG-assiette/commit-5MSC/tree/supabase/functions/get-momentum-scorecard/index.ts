...
import { buildMomentumTraceScorecard } from "../sophia-brain/lib/momentum_scorecard.ts";
import { loadMomentumTraceWindow } from "../sophia-brain/lib/momentum_trace.ts";
...
function deriveWindow(body: z.infer<typeof BodySchema>): { from: string; to: string } | null {
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const trace = await loadMomentumTraceWindow({
    ...
    const scorecard = buildMomentumTraceScorecard({
      trace,
      annotations: [],
    });
    ...
    console.error(`[get-momentum-scorecard] request_id=${requestId}`, error);
