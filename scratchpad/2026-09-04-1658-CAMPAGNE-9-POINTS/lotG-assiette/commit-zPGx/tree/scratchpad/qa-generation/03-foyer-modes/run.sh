#!/usr/bin/env bash
# ÉTAPE ③ — un run réel sur UN mode de cuisson, et LES TROIS FICHIERS.
#   ./run.sh <cooking_shape|none> <request-id> <dossier>
#
# Rend dans <dossier>:
#   inputs.json        les entrées (foyer + corps de la requête)
#   request-body.json  le corps POST tel qu'envoyé
#   dump/prompt-system.txt, dump/prompt-user.txt, dump/output.json
#   http-response.json le plan rendu
#   plan-written.json  `generated_from` du plan écrit (dont `household.cooking`)
#   model.txt          QUEL MODÈLE A RÉELLEMENT SERVI (source|model|status)
set -uo pipefail
ROOT="/Users/ahmedamara/Dev/Sophia 2"
ANON="eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZS1kZW1vIiwicm9sZSI6ImFub24iLCJleHAiOjE5ODM4MTI5OTZ9.CRXP1A7WOeoJeXxjNni43kdQwgnWNReilDMblYTn_I0"
SHAPE="$1"; RID="$2"; OUT="$3"
EMAIL="qa3a.foyer@keeltest.dev"
OWNER="3a000000-0000-4000-8000-000000000001"
FN="generate-household-meal-v1"
DAYS="${DAYS:-2}"
STARTS="${STARTS:-2026-08-19}"
if [ "$SHAPE" = "none" ]; then SHAPE_JSON="null"; else SHAPE_JSON="\"$SHAPE\""; fi
BODY="{\"window\":{\"kind\":\"exact\",\"starts_on\":\"$STARTS\",\"duration_days\":$DAYS},\"intent\":\"prepare_next\",\"replaces\":null,\"context\":\"the oven only holds one tray at a time this week\",\"cooking_shape\":$SHAPE_JSON}"
mkdir -p "$OUT"

# ── LA FENÊTRE EST LIBÉRÉE AVANT CHAQUE RUN ────────────────────────────────
# Un plan écrit par le run précédent rend `plan_overlaps_existing` (409). On
# RETIRE le plan (le geste du produit), on ne touche jamais à son contenu.
...
select jsonb_pretty(jsonb_build_object(
  ...
  'roster', (select jsonb_agg(jsonb_build_object(
       ...
       'has_account', r.user_id is not null,
...
echo "== POST $FN  cooking_shape=$SHAPE  request_id=$RID =="
START=$(date +%s)
...
# On PROUVE d'abord que la ligne de CE run existe, et on relève QUEL MODÈLE a
# servi (le compte OpenAI est à sec: la lane replie sur gemini-3-flash-preview).
docker exec supabase_db_Sophia_2 psql -U postgres -d postgres -tAc \
...
docker exec supabase_db_Sophia_2 psql -U postgres -d postgres -tAc \
  "select jsonb_pretty(jsonb_build_object(
      'plan_id', id, 'created_at', created_at,
      'cooking', generated_from->'household'->'cooking',
      'box_sizing', generated_from->'household'->'box_sizing',
      'attribution', generated_from->'household'->'attribution',
...
head -c 400 "$OUT/plan-written.json"; echo
