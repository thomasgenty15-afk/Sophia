...
- Statut: `fix_applied` (partiel) — chantier W5 (2026-07-06) + arbitrage produit : la persistance conversationnelle des préférences arrivera dans une version prochaine (consigne existante FO renforcée) ; doctrine FO bornée : appliquer immédiatement en session, engagement de SESSION explicite (« je le fais sur cette conversation »), jamais « à partir de maintenant » ; réglage durable = Preferences coach. **Volet cross-mode fermé** — chantier V2-F3 (2026-07-07, compression companion incluse) : règle companion ajoutée « une préférence de style exprimée en session (ton, emojis, longueur) s'applique à TOUS les tours suivants, y compris en mode soutien » (rattachée à la ligne Profil/préférences). Budget < 13000, contrat companion 15/15 (ancres F3 testées).
- Fix reference: chantier W (2026-07-06)
- Tests requis:
