-- Add JSON payload + server-side writer for per-turn debugging logs.
-- Goal: persist full turn summary (including brain trace events) into public.turn_summary_logs
-- so production debugging does not rely on conversation_eval_events (eval tooling).

alter table public.turn_summary_logs
...
begin
  ...
  -- Prevent forging logs for another user.
