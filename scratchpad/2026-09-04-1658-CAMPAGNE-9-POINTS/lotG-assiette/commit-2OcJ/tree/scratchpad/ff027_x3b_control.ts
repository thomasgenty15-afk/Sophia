...
for (const run of [0, 1, 2]) {
  const s = await makeStudent({ coach, fullName: `ff027ctl_${run}`, country: "FR", timezone: "Europe/Paris", locale: "en-US" });
  ...
  // AUCUN tap de faim: c'est le témoin.
  const res = await callAs(s, "generate-week-plan-v1", { local_date: nextMonday() });
  const { data } = await db.from("student_week_plans").select("items").eq("user_id", s.userId).eq("week_start", nextMonday()).maybeSingle();
  const items = JSON.stringify((data as any)?.items ?? []);
  const hit = REFUSED.test(items);
  console.log(`run ${run}: HTTP ${res.status} · aliment refusé ${hit ? "PRÉSENT ❌" : "absent ✅"}`);
